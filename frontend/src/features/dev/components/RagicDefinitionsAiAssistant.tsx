...
import {
  ...
  devAiKnowledgeSubmissionStatusText,
...
export function RagicDefinitionsAiAssistant({
  ...
  const readinessMessage = !readiness
    ? "正在檢查 AI 設定…"
    : readiness.chatAvailable
      ? null
      : readiness.reason === "conversation-disabled"
        ? "對話儲存目前停用；請設定 DEV_AI_CONVERSATION_HISTORY_ENABLED=true。"
        : readiness.reason === "provider-disabled"
          ? "AI provider 目前停用；請設定 DEV_AI_ENABLED=true。"
          : "AI provider 尚未設定 API key。";
  const knowledgeReadinessMessage =
    readiness && !readiness.knowledge.available
      ? readiness.knowledge.state === "missing"
        ? "本地 knowledge 索引尚未建立，已預設關閉；請先執行 knowledge:prepare。"
        : readiness.knowledge.state === "stale"
          ? "本地 knowledge 索引已過期，已預設關閉；請重新執行 knowledge:prepare。"
          : "本地 embedding／index 尚未完成可持久驗證，已預設關閉。"
      : null;
  const canSendThreadMessage =
    Boolean(composerText.trim()) && !loading && readiness?.chatAvailable === true;
  ...
  useEffect(() => {
    ...
    fetchDevAiReadiness(token)
      .then((next) => {
        if (!cancelled) {
          setReadiness(next);
          setIncludeKnowledge(next.knowledge.available);
        }
      })
      .catch((err) => {
        if (!cancelled) setError(onError(err, "讀取 AI readiness 失敗"));
      });
    return () => {
      cancelled = true;
    };
  ...
  async function handleThreadSend() {
    if (!readiness?.chatAvailable) {
      setError(readinessMessage ?? "AI 對話目前不可用");
      return;
    }
  ...
  async function handleStoreChatExample() {
    ...
    try {
      await submitDevAiKnowledgeCandidate(token, {
        ...
        domain: "ragic",
        sourceKey: `chat:${chatResult.chatId}`,
      });
  ...
  async function handleStoreFormulaExample() {
    ...
    try {
      await submitDevAiKnowledgeCandidate(token, {
        ...
        domain: "definitions",
        sourceKey: `formula:${activeResult.suggestionId}`,
      });
      setFeedbackState("saved");
    } catch (err) {
  ...
  const assistant = (
    <aside
      ...
      aria-label={mode === "formula" ? "AI 公式助手" : "DemoCo Dev AI 問答助手"}
    >
      {open ? (
        <section
          ref={panelRef}
        ...
        >
          <div className="ragic-defs-ai-bot__chrome">
            <div className="ragic-defs-ai-bot__identity">
              ...
              <div>
                <span className="ragic-defs-ai-bot__eyebrow">DemoCo Dev AI</span>
                <strong>AI 助手</strong>
          ...
          <div className="ragic-defs-ai-bot__conversation" aria-label="AI 對話內容">
            ...
            {readinessMessage ? (
              <p className="dev-mode-error ragic-defs-ai-bot__error">{readinessMessage}</p>
            ) : null}

            {knowledgeReadinessMessage ? (
              <p className="ragic-defs-ai-bot__knowledge-warning">{knowledgeReadinessMessage}</p>
            ) : null}
              ...
              <div className="ragic-defs-ai-bot__empty-state">
                ...
                <span>
                  {mode === "formula"
                    ...
                    : "把 DemoCo 或 definitions 問題丟進來；若本地 knowledge 不足，AI 會明確說缺哪些資料。"}
                </span>
              </div>
            ) : null}
          </div>
      ...
      <button
        ...
        aria-expanded={open}
        aria-label={open ? "收合 DemoCo Dev AI" : "開啟 DemoCo Dev AI"}
        title="DemoCo Dev AI · 可拖曳位置"
...
function AiFeedbackStatus({
  ...
  const text = devAiKnowledgeSubmissionStatusText(state);
  const tone = state === "saving" ? "is-saving" : state === "saved" ? "is-ok" : "is-blocked";
  return <span className={`ragic-defs-ai-bot__feedback ${tone}`}>{text}</span>;
}
