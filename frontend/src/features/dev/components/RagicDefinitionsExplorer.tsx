...
function formulaDryRunMatchesDraft(
  ...
  return (
    ...
    dryRun.position === (draft.position?.trim().toUpperCase() || null) &&
    dryRun.sourceLine === (draft.sourceLine ?? null) &&
...
export function RagicDefinitionsExplorer({ token, username, onAuthFailure }: Props) {
  ...
  const applyLoadedDetail = useCallback((
    ...
    if (
      ...
      next.formulas.some((formula) => matchesFormulaOccurrence(formula, preferredTarget))
    ) {
    ...
    if (!nextTarget) {
      nextTarget = fallbackFormula
        ? {
            ...
            position: fallbackFormula.position,
            sourceLine: fallbackFormula.sourceLine,
          }
        : next.fields[0]
    ...
    const draftFormula =
      nextTarget?.type === "formula"
        ? next.formulas.find((formula) => matchesFormulaOccurrence(formula, nextTarget)) ?? fallbackFormula
        : fallbackFormula;
  ...
  const selectedFormula = useMemo(() => {
    ...
    return currentDetail.formulas.find((formula) =>
      matchesFormulaOccurrence(formula, selectedTarget)
    ) ?? null;
  ...
  const dryRunMatchesDraft =
    ...
    dryRunResult.position === (dryRunDraft.position?.trim().toUpperCase() || null) &&
    dryRunResult.sourceLine === (dryRunDraft.sourceLine ?? null) &&
  ...
  const runFormulaDryRun = useCallback(async (draft: RagicFormulaPatchDryRunInput) => {
    ...
    try {
      ...
      setDryRunDraft((current) =>
        synchronizeFormulaDryRunDraft(current, requestDraft, result)
      );
    } catch (err) {
      if (controller.signal.aborted || dryRunRequestSeqRef.current !== requestSeq) return;
      setDryRunResult(null);
  ...
  async function handleApplySubmit() {
    ...
    try {
      ...
      setDryRunDraft((current) =>
        synchronizeFormulaDryRunDraft(current, mainTarget, finalDryRun)
      );
