...
function dryRunMatchesTarget(
  dryRun: RagicFormulaPatchDryRunResult | null,
  target: Pick<SiblingApplyTarget, "formPath" | "fieldId" | "formulaKind" | "newFormula"> &
    Partial<Pick<SiblingApplyTarget, "position" | "sourceLine">>
): boolean {
  return (
    dryRun !== null &&
    dryRun.formPath === target.formPath &&
    dryRun.fieldId === target.fieldId &&
    (target.position === undefined || dryRun.position === target.position) &&
    (target.sourceLine === undefined || dryRun.sourceLine === target.sourceLine) &&
...
export function FormulaSiblingsPanel({
  ...
  useEffect(() => {
    const targets: SiblingApplyTarget[] = (siblings ?? [])
      ...
      .flatMap((sibling): SiblingApplyTarget[] => {
        const dryRun = rows[sibling.formPath]?.dryRun ?? null;
        const target = {
        ...
        if (
          !dryRunMatchesTarget(dryRun, target) ||
          !dryRun?.position ||
          !dryRun.sourceLine
        ) {
          return [];
        }
        return [{ ...target, position: dryRun.position, sourceLine: dryRun.sourceLine }];
      })
      .filter((target) => {
