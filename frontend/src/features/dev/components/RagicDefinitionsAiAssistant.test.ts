...
describe("Dev AI source evidence", () => {
  it("definitions 來源會顯示表單、Field ID 與短版 revision", () => {
    expect(
      devAiKnowledgeSourceLabel({
        sourceId: "definitions:default/devtest/7:field:9001111:L6",
        title: "TestForm1 · Name",
        kind: "definitions",
        excerpt: "欄位設定：{\"l\":\"9001110\"}",
        score: 10,
    ...
    ).toContain("[default/devtest/7 · Field 9001111 · 欄位 · rev aaaaaaaa]");
  });
...
describe("Dev AI 知識送審狀態", () => {
  it("建立 pending candidate 後不會宣稱已發布到 knowledge", () => {
    expect(devAiKnowledgeSubmissionStatusText("saving")).toBe("正在建立知識候選…");
    const saved = devAiKnowledgeSubmissionStatusText("saved");
    expect(saved).toContain("已送交知識審核");
    expect(saved).toContain("核准後");
    expect(saved).not.toContain("已收錄");
    expect(saved).not.toContain("clean knowledge");
  });
});
