...
export function matchesFormulaOccurrence(
  formula: RagicDefinitionFormula,
  target: {
    fieldId: string;
  ...
  return (
    formula.fieldId === target.fieldId &&
    formula.formulaKind === target.formulaKind &&
    formula.position === target.position &&
    formula.sourceLine === target.sourceLine
  );
}

export function synchronizeFormulaDryRunDraft(
  current: RagicFormulaPatchDryRunInput,
  requested: RagicFormulaPatchDryRunInput,
  result: RagicFormulaPatchDryRunResult
): RagicFormulaPatchDryRunInput {
  if (
    current.formPath.trim() !== requested.formPath.trim() ||
    current.fieldId.trim() !== requested.fieldId.trim() ||
    current.formulaKind !== requested.formulaKind ||
    current.newFormula !== requested.newFormula
  ) {
    return current;
  }
