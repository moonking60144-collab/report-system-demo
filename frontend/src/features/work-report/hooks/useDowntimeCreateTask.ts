...
export interface UseDowntimeCreateTaskResult {
  ...
  submit: (payload: CreateActivityLogDowntimePayload) => Promise<{ ok: boolean; errorMessage?: string }>;
}
 ...
 * - backend 收到相同 key 會擋掉重複寫入（擋 retry 風暴造成的 duplicate activity log entry）
...
export function useDowntimeCreateTask(
  ...
  const submit = useCallback(
    async (payload: CreateActivityLogDowntimePayload) => {
      ...
      const result = await run(() =>
        createActivityLogDowntimeRecord({ ...payload, clientRowKey })
      );
      if (result.ok) {
