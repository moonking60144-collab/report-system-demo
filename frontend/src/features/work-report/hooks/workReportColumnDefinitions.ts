...
interface BuildWorkReportColumnDefinitionsArgs {
  ...
  renderPreviousMachineValue: (value: unknown) => ReactNode;
  ...
  renderPlannedEndDateValue: (value: unknown, record: WorkReportRecord) => ReactNode;
  renderStartSchedule: (value: unknown, record: WorkReportRecord) => ReactNode;
  renderUrgent: (value: unknown, record: WorkReportRecord) => ReactNode;
...
const FIT_WIDTH_OVERRIDES: Readonly<Record<string, number>> = {
  ...
  completedQty: 128,
  processLossPc: 901,
  finishedWireSize: 901,
  status: 88,
  sourceCloseStatus: 102,
  workOrderRemark: 124,
...
function reorderColumnsForForm902(
  ...
  return baseColumns.flatMap((column) => {
    ...
    if (key === "startSchedule") {
      return [modificationStatusColumn];
    }
    if (key === "machineCode") {
      return [{ ...column, render: renderMachineValue }];
    }
    if (key === "plannedEndDate") {
      return [plannedStartDateColumn, column];
    }
    return [column];
  });
}
...
export function buildFormAwareColumns(args: BuildWorkReportColumnDefinitionsArgs): ColumnsType<WorkReportRecord> {
  ...
  const baseColumns: ColumnsType<WorkReportRecord> = [
    {
      ...
      width: 90,
      align: "center",
      className: "work-report-boolean-column-cell",
      render: renderStartSchedule,
    },
    {
      title: "工令單號",
    ...
    {
      title: "上一站機台",
      dataIndex: "previousMachine",
      width: 176,
      align: "center",
      render: renderPreviousMachineValue,
    },
    {
      title: "前置母件",
    ...
    },
    {
      ...
      render: renderPlannedEndDateValue,
    },
    {
      title: "預估所需工時",
  ...
  const formAwareColumns =
    currentFormId === "902"
      ? reorderColumnsForForm902(baseColumns, renderValue, renderMachineValue, toDateValue)
      : baseColumns;
