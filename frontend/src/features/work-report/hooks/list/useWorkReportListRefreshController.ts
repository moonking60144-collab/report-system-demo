...
interface UseWorkReportListRefreshControllerArgs {
  ...
  invalidatePreviewCache: (formId?: WorkReportFormId) => void;
  hydrateAllRecords: (
    forceRefresh?: boolean,
    options?: {
      mode?: "foreground" | "background";
...
export function useWorkReportListRefreshController({
  ...
  const handleRefresh = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      if (shouldUseFullHydrationForList) {
        ...
        await loadReports(false, { throwOnError: true, invalidateCache: true });
      }
  ...
  const triggerAutoRefresh = useCallback(
    (options: { noticeMessage?: string; maxJitterMs?: number; resetPageToFirst?: boolean; requiredReadStartedAfter?: number } = {}): void => {
      pendingReadStartedAfterRef.current = autoRefreshTimerRef.current === null
        ? options.requiredReadStartedAfter
        : pendingReadStartedAfterRef.current === undefined || options.requiredReadStartedAfter === undefined
          ? undefined
  ...
  const fallbackRefreshStateRef = useRef({
    loading,
    isHydratingAllRecords,
    triggerAutoRefresh,
  });
  fallbackRefreshStateRef.current = {
    loading,
    isHydratingAllRecords,
    triggerAutoRefresh,
  };
  ...
  const handleRealtimeFormUpdated = useCallback(
    (event: { formId?: string }) => {
      ...
      triggerAutoRefresh({ maxJitterMs: 1200, resetPageToFirst: false, requiredReadStartedAfter: performance.now() });
    },
  ...
  useEffect(() => {
    ...
    return startFallbackRefreshScheduler({
      shouldDefer: () => {
        const latest = fallbackRefreshStateRef.current;
        return (
          latest.loading ||
          latest.isHydratingAllRecords ||
          autoRefreshInFlightRef.current
        );
      },
      refresh: () => {
        fallbackRefreshStateRef.current.triggerAutoRefresh({ maxJitterMs: 0 });
      },
    });
  }, [isStandaloneTopView, realtimeConnected]);
