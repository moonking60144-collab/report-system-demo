...
export function useColumnMenuState({ setPage, initialState }: UseColumnMenuStateOptions) {
  ...
  const [columnMenuOpenState, setColumnMenuOpenState] = useState<{
    columnKey: ColumnKey;
    ownerId: string;
  } | null>(null);
  const columnMenuOpenKey = columnMenuOpenState?.columnKey ?? null;
  const columnMenuOpenOwnerId = columnMenuOpenState?.ownerId ?? null;
  ...
  const handleColumnMenuOpenChange = useCallback((
    ...
    setColumnMenuOpenState((previous) => {
      if (nextOpen) {
        return { columnKey, ownerId };
      }
      if (columnMenuInteractRef.current || previous?.ownerId !== ownerId) {
        return previous;
      }
      return null;
    });
