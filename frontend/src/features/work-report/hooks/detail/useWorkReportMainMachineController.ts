...
export function resolveEditableMainMachineCode(
  ...
  const value = formId === "902" ? record?.filterMachineCode : record?.machineCode;
...
export function useWorkReportMainMachineController({
  ...
  const mainMachineBaselineRef = useRef<{ formId: WorkReportFormId | null; entryId: string | null; record: WorkReportRecord } | null>(null);
  ...
  const openMainMachineModal = useCallback(() => {
    const run = async () => {
      ...
      mainMachineBaselineRef.current = { formId, entryId: safeEntryId, record: { ...record } };
  ...
  const submitMainMachineUpdate = useCallback(async () => {
    ...
    const baseline = mainMachineBaselineRef.current;
    if (!baseline || baseline.formId !== formId || baseline.entryId !== safeEntryId) return;
    if (nextMachineCode === resolveEditableMainMachineCode(formId, baseline.record)) {
      setMainMachineModalOpen(false);
      setMainMachineDraft("");
      return;
    }
    ...
    try {
      ...
      const retryRecord = getOrCreateRetryableEntryFieldMutation({
        operation: "work-report-main-machine",
        formId,
        entryId: safeEntryId,
        value: nextMachineCode,
        previousValue: resolveEditableMainMachineCode(formId, baseline.record) || null,
        workOrderNo: record.workOrderNo,
        expectedEntryLastUpdatedAt: resolveExpectedEntryLastUpdatedAt(baseline.record),
      });
      ...
      await registerAcceptedMutationTask("update", accepted, undefined, {
        ...
        patch: { kind: "update-entry", patch: retryRecord.successPatch },
