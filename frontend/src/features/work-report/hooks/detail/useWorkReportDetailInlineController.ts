...
export function useWorkReportDetailInlineController({
  ...
  const getOrderedInlineKeysForRow = useCallback(
    (rowId: string | null): InlineEditableDetailKey[] => {
      const fallbackKeys = (INLINE_EDITABLE_DETAIL_KEYS_BY_FORM[formId ?? "901"] ?? []).filter((key) =>
  ...
  const startInlineRowEdit = useCallback(
    async (row: WorkReportItem) => {
      ...
      editingRowSnapshotRef.current = {
        formId, entryId: safeEntryId, rowId: row.rowId, hash: row.snapshotHash,
    ...
    async (row: DetailTableRow) => {
      ...
      try {
        ...
        if (isCreatePlaceholderRow(row)) {
          ...
          const startedAt = new Date().toISOString();
          const clientMutationId = createClientMutationId();
          const baseline = editingRowSnapshotRef.current;
          if (!baseline || baseline.formId !== formId || baseline.entryId !== safeEntryId || baseline.rowId !== row.rowId) throw new Error("缺少報工明細的編輯原值，請重新開啟編輯。");
          payload.expectedRowSnapshotHash = baseline.hash;
          const expectedEntryLastUpdatedAt = baseline.timestamp;
