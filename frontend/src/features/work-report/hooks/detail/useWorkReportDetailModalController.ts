...
export function useWorkReportDetailModalController({
  ...
  const editBaselineRef = useRef<{ formId: WorkReportFormId | null; entryId: string | null; rowId: string; timestamp?: string } | null>(null);
  ...
  const openEditModal = useCallback(
    (row: WorkReportItem) => {
      const run = async () => {
        ...
        editBaselineRef.current = { formId, entryId: safeEntryId, rowId: row.rowId, timestamp: expectedEntryLastUpdatedAt };
        setModalState({ open: true, mode: "edit", row: { ...row } });
      };
      void run();
    },
    [acquireRowEditLock, ensureOptionsLoaded, hasActiveMutationTask, hasBlockingMutationTask, formId, safeEntryId, expectedEntryLastUpdatedAt]
  );
  ...
  const handleSubmit = useCallback(
    async (
      ...
      try {
        ...
        if (!baseline || baseline.formId !== formId || baseline.entryId !== safeEntryId || baseline.rowId !== modalState.row.rowId) {
          throw new Error("缺少報工明細的編輯原值，請重新開啟編輯。");
        }
        const clientMutationId = createClientMutationId();
        const changedFields = summarizeChangedPayloadFields(payload);
        ...
        payload.expectedRowSnapshotHash = modalState.row.snapshotHash;
        writePendingMutationReplay({
          kind: "update",
          formId,
          entryId: safeEntryId,
