    ...
    return startFallbackRefreshScheduler({
      shouldDefer: () => {
        ...
        return (
          ...
          latest.modalOpen ||
          latest.editingRowId !== null ||
          latest.hasActiveMutationTask ||
          latest.submitting ||
          latest.loading ||
          latest.refreshing
        );
      ...
      refresh: async () => {
        ...
        latest.logDetailEvent("realtime", "fallback-refresh-triggered", "離線 fallback refresh 已觸發", {
          phase: "fallback",
          meta: {
        ...
        try {
          await latest.loadEntry({ mode: "background", forceRefresh: false });
        } finally {
