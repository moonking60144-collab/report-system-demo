...
export function isMutationTaskVerificationPending(
  ...
  if (isEntryFieldMutationSettlementTerminal(task)) {
    return false;
  }
  return (
    task.stale === true ||
...
export function hasPendingEntryFieldSettlement(
  task: Pick<
    CreateTaskMonitor,
    | "taskId"
    | "entryFieldOperation"
  ...
  const retryMutation = getRetryMutation(task.taskId);
  return (
    isEntryFieldMutationOperation(operation) &&
    !isEntryFieldMutationSettlementTerminal(task) &&
    (retryMutation === null || isRetryableEntryFieldMutationBlocking(retryMutation, task))
  );
}
...
export function useWorkReportDetailTaskController({
  ...
  const hasBlockingMutationTask =
    ...
    currentEntryTaskMonitors.some((task) => hasPendingEntryFieldSettlement(task));
  ...
  const buildTaskMonitor = useCallback(
    (
      ...
      const optimisticMutation = acceptedMutation
        ...
        : undefined;
      const entryFieldOperation = isEntryFieldMutationOperation(
        optimisticInput?.operation
      )
        ? optimisticInput.operation
        : undefined;
      return {
        ...
        ...(entryFieldOperation
          ? {
              entryFieldOperation,
              entryFieldClientMutationId: optimisticInput!.mutationId,
            }
          : {}),
