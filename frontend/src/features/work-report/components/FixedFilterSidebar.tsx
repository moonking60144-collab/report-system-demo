...
export const FixedFilterSidebar = memo(function FixedFilterSidebar({
  ...
  const visibleMachines = unfinishedMachineShortcuts.filter(preset =>
    preset.machineCode.toLocaleLowerCase().includes(machineSearch.trim().toLocaleLowerCase())
  );
  const changeMachineSearch = (value: string) => {
    setMachineSearch(value);
    if (machineListRef.current) machineListRef.current.scrollTop = 0;
  };
  ...
  const machineOpenDescription =
    currentFormId === "902"
      ? t("workReport:sidebar.machineOpenDesc902")
      ...
      <div className="fixed-filter-sidebar-header">
        {!effectiveCollapsed && (
          <div className="fixed-filter-sidebar-title">
            ...
            <p className="fixed-filter-current-view" title={currentViewLabel}>
              {t("workReport:sidebar.currentView")}<span>{currentViewLabel}</span>
            </p>
          </div>
        )}
        {!mobileMode && (
          ...
          >
            <LeftOutlined className="fixed-filter-collapse-icon" />
          </button>
        )}
      </div>
      ...
      <div className="fixed-filter-sidebar-body">
        ...
        {!effectiveCollapsed && unfinishedMachineShortcuts.length > 0 && (
          <section className="fixed-filter-machine-group" aria-label={t("workReport:sidebar.openMachines")}>
            <div className="fixed-filter-machine-group-header">
              <div className="fixed-filter-machine-group-title">{t("workReport:sidebar.openMachines")}</div>
              <div className="fixed-filter-machine-search">
                <SearchOutlined aria-hidden="true" />
                <input ref={machineSearchRef} type="text" value={machineSearch}
                  aria-label={t("workReport:sidebar.searchMachines")}
                  placeholder={t("workReport:sidebar.searchMachines")}
                  onChange={event => changeMachineSearch(event.target.value)}
                  onKeyDown={event => {
                    if (event.key === "Escape") { event.stopPropagation(); changeMachineSearch(""); }
                  }} />
                {machineSearch && <button type="button" aria-label={t("workReport:sidebar.clearMachineSearch")}
                  onClick={() => { changeMachineSearch(""); machineSearchRef.current?.focus(); }}>
                  <CloseOutlined />
                </button>}
              </div>
            </div>
            <div className="fixed-filter-machine-list" ref={machineListRef}>
              {visibleMachines.length === 0 && <p className="fixed-filter-machine-empty" role="status">{t("workReport:sidebar.noMatchingMachines")}</p>}
              {visibleMachines.map((preset) => (
                <button
                  key={preset.key}
                  type="button"
