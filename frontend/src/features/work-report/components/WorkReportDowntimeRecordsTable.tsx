...
import type { OptimisticActivityLogDowntimeRecord } from "../downtimeOptimisticMutation";
import {
...
export const WorkReportDowntimeRecordsTable = memo(function WorkReportDowntimeRecordsTable({
  ...
  if (!hasRenderableContent && !error) {
    return (
      <PageLoadingBoundary
        variant="compact"
        state={{ kind: "pending", message: loadingText }}
      />
    );
  }
  ...
  if (totalCount === 0) {
    return (
      <div className="downtime-records-section">
        {loading ? (
          <BackgroundLoadingIndicator label={t("common:states.backgroundLoading")} />
        ) : null}
        <div className="downtime-empty-state">{emptyText}</div>
      </div>
    );
  }
