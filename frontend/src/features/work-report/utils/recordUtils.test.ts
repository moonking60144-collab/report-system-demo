...
describe("getColumnCellValue", () => {
  ...
  test("machineCode 有值時回 machineCode", () => {
    const record = makeRecord({ id: "1", machineCode: "MB07", filterMachineCode: "F8" });
    expect(getColumnCellValue(record, "machineCode")).toBe("MB07");
  });
...
describe("matchesPageReportGroup", () => {
  test("prodType 等於目標回 true", () => {
    expect(matchesPageReportGroup(makeRecord({ id: "1", prodType: "PA" }), "PA")).toBe(true);
  });

  test("prodType 不等於目標回 false", () => {
    expect(matchesPageReportGroup(makeRecord({ id: "1", prodType: "PB" }), "PA")).toBe(false);
  });
...
describe("applyPageReportGroupFilter", () => {
  test("無任何 record 帶 prodType 時不過濾（避免空畫面）", () => {
    ...
    expect(applyPageReportGroupFilter(records, "PA")).toEqual(records);
  });

  test("有 prodType 時依 prodTypeCode 過濾", () => {
    const records = [
      makeRecord({ id: "1", prodType: "PA" }),
      makeRecord({ id: "2", prodType: "PB" }),
      makeRecord({ id: "3", prodType: "PA" }),
    ];
    const result = applyPageReportGroupFilter(records, "PA");
    expect(result.map((r) => r.id)).toEqual(["1", "3"]);
  });

  test("預設 prodTypeCode 為 PA", () => {
    const records = [
      makeRecord({ id: "1", prodType: "PA" }),
      makeRecord({ id: "2", prodType: "PB" }),
    ];
