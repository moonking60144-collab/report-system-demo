...
function getExpectedLandingPageKeyForForm(formId: WorkReportFormId): WorkReportLandingPageKey {
  return formId === "902" ? "line-b-902" : "line-a-901";
}
...
export const FORM_901_UNFINISHED_DEFAULT_SORT_RULES: readonly ColumnSortRule[] = Object.freeze([
  { key: "machineCode", direction: "asc", type: "text" },
  { key: "sortOrder", direction: "asc", type: "number" },
]);

export const FORM_902_UNFINISHED_DEFAULT_SORT_RULES: readonly ColumnSortRule[] = Object.freeze([
  { key: "filterMachineCode", direction: "asc", type: "text" },
  { key: "plannedStartDate", direction: "asc", type: "date" },
]);

export const FORM_902_MACHINE_SHORTCUT_SORT_RULES: readonly ColumnSortRule[] = Object.freeze([
  { key: "filterMachineCode", direction: "asc", type: "text" },
  { key: "sortOrder", direction: "asc", type: "number" },
]);
...
export function normalizeGlobalFiltersForForm(
  filters: GlobalFilters,
  formId: WorkReportFormId
): GlobalFilters {
  const normalized = normalizeFilters(filters);
...
export function normalizeColumnSortRulesForForm(
  rules: readonly ColumnSortRule[],
  formId: WorkReportFormId
): ColumnSortRule[] {
  const sourceKey = formId === "902" ? "machineCode" : "filterMachineCode";
  const targetKey = formId === "902" ? "filterMachineCode" : "machineCode";
  const hasNativeTarget = rules.some((rule) => rule.key === targetKey);
  const seenKeys = new Set<string>();

  return rules.flatMap((rule) => {
    if (rule.key === sourceKey && hasNativeTarget) {
      return [];
    }
    const key = rule.key === sourceKey ? targetKey : rule.key;
    if (seenKeys.has(key)) {
      return [];
    }
    seenKeys.add(key);
...
export function normalizeColumnFilterStateForForm(
  ...
  const sourceRule = state[sourceKey];
  const targetRule = state[targetKey];
  delete nextState[sourceKey];
  if (targetRule) {
    nextState[targetKey] = targetRule;
  } else if (sourceRule) {
    nextState[targetKey] = sourceRule;
  }
  return nextState;
}
...
export function buildUnfinishedMachineFilters(
  machineCode: string,
  formId: WorkReportFormId = "901"
): GlobalFilters {
  return {
    ...DEFAULT_GLOBAL_FILTERS,
    machineCode: formId === "902" ? ALL_FILTER_VALUE : machineCode,
    filterMachineCode: formId === "902" ? machineCode : ALL_FILTER_VALUE,
    status: "未結案",
    ragicUnfinishedStatus: ALL_FILTER_VALUE,
    startSchedule: formId === "902" ? "all" : "yes",
  };
}
...
export function buildUnfinishedMachineShortcut(
  ...
  const sortRules =
    formId === "902"
      ? FORM_902_MACHINE_SHORTCUT_SORT_RULES
...
export function getDefaultSortRulesForFixedPreset(
  ...
  if (formId === "901" && (presetId === "unfinished-runnable" || presetId === "unfinished-orders")) {
    return FORM_901_UNFINISHED_DEFAULT_SORT_RULES.map((rule) => ({ ...rule }));
  }

  if (formId === "902" && presetId === "unfinished-orders") {
    return FORM_902_UNFINISHED_DEFAULT_SORT_RULES.map((rule) => ({ ...rule }));
  }
...
export function getDefaultSortRulesForCurrentFilters(
  ...
  return (
    formId === "902"
      ? FORM_902_MACHINE_SHORTCUT_SORT_RULES
      : FORM_901_UNFINISHED_DEFAULT_SORT_RULES
  ).map((rule) => ({ ...rule }));
}
...
export function getDefaultSortBootstrapKey(
  ...
  const machineCode =
    formId === "902" ? normalizeFilters(filters).filterMachineCode : normalizeFilters(filters).machineCode;
...
export function getInitialGlobalFilters(
  landingPageKey: WorkReportLandingPageKey = "line-a-901"
): GlobalFilters {
  const formId = WORK_REPORT_LANDING_PAGE_CONFIGS[landingPageKey].formId;
  if (hasExplicitGlobalFilterParamsInUrl()) {
    return normalizeGlobalFiltersForForm(readFiltersFromUrl(), formId);
  }
  ...
  return getFixedFilterPresetFilters(preferredPresetId, formId);
}
...
export function applyFixedFilterPresetToGlobalFilters(
  presetId: FixedFilterPresetId,
  formId: WorkReportFormId
): GlobalFilters {
  return normalizeGlobalFiltersForForm(
    getFixedFilterPresetFilters(presetId, formId),
    formId
  );
}

export function applyMachineShortcutToGlobalFilters(
  shortcutFilters: GlobalFilters,
  formId: WorkReportFormId
): GlobalFilters {
  return normalizeGlobalFiltersForForm(shortcutFilters, formId);
}
