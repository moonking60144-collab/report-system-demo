...
export function writeHydrationCache(formId: string = FORM_ID, records: WorkReportRecord[]): void {
  ...
  if (records.length > HYDRATION_CACHE_MAX_RECORDS) {
    try {
      window.localStorage.removeItem(getHydrationCacheKey(formId));
    } catch {
      // NOTE: 清除大型舊快取失敗時不阻塞讀取流程
    }
    return;
  }
...
export function reconcileWorkReportTableLayout(
  ...
  const savedKeysInDefaultOrder = availableColumnKeys.filter((key) => savedOrder.includes(key));
  const hasCustomColumnOrder = savedOrder.some(
    (key, index) => key !== savedKeysInDefaultOrder[index]
  );
  ...
  return {
    ...
    columnOrder: hasCustomColumnOrder
      ? [...savedOrder, ...missingColumnKeys]
      : [...availableColumnKeys],
    columnWidths,
    columnColors,
  };
