...
import { normalizeWorkReportFilterGroup } from "./customFilterUtils";
...
export function normalizeWorkReportListViewState(value: unknown): WorkReportListViewState | undefined {
  ...
  const hasCustomFilterDraft = Object.prototype.hasOwnProperty.call(
    candidate,
    "customFilterDraft"
  );
  const customFilterDraft = normalizeWorkReportFilterGroup(candidate.customFilterDraft);
  const customFilters = normalizeWorkReportFilterGroup(candidate.customFilters);
  ...
  if (hasCustomFilterDraft) {
    normalizedState.customFilterDraft = customFilterDraft;
  }
  if (customFilters.conditions.length > 0) {
    normalizedState.customFilters = customFilters;
  }
  ...
  try {
    ...
    return {
      ...
      listAnchor: {
        ...
        outerScrollY: Number.isFinite(parsed.listAnchor.outerScrollY)
          ? Math.max(0, Number(parsed.listAnchor.outerScrollY))
          : 0,
