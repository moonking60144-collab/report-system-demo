...
describe("compareColumnCellValues / number", () => {
  ...
  test("非空白 invalid number 使用 0 sentinel", () => {
    expect(compareColumnCellValues("abc", "2", "number")).toBeLessThan(0);
  });
});
...
describe("compareColumnCellValues / date", () => {
  ...
  test("非空白 invalid date 使用 0 sentinel", () => {
    expect(compareColumnCellValues("invalid", "2026-04-22", "date")).toBeLessThan(0);
  });
});
...
describe("sortRecordsByColumnRules / shared sentinel policy", () => {
  test("desc 仍將 blank 放最後，invalid number 使用 0", () => {
    const records = [
      { id: "valid", estimatedHours: "10" },
      { id: "invalid", estimatedHours: "abc" },
      { id: "blank", estimatedHours: "" },
    ] as Parameters<typeof sortRecordsByColumnRules>[0];

    expect(
      sortRecordsByColumnRules(records, [
        { key: "estimatedHours", direction: "desc", type: "number" },
      ]).map((record) => record.id)
    ).toEqual(["valid", "invalid", "blank"]);
  });
});
...
describe("buildBackendColumnFilters", () => {
  ...
  test("active rule 會帶上 type 欄位", () => {
    const result = buildBackendColumnFilters(
      { machineCode: { textQuery: "MB07" } },
      { machineCode: "text" }
    );
    expect(result).toEqual({
      machineCode: { type: "text", textQuery: "MB07" },
    });
  });
