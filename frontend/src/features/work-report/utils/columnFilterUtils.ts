...
export function compareColumnCellValues(
  ...
  if (type === "number") {
    const leftBlank = normalizeColumnText(leftValue) === "";
    const rightBlank = normalizeColumnText(rightValue) === "";
    ...
    return (toSortableNumber(leftValue) ?? 0) - (toSortableNumber(rightValue) ?? 0);
  }
  ...
  if (type === "date") {
    ...
    return (toSortableDate(leftValue) ?? 0) - (toSortableDate(rightValue) ?? 0);
  }
  ...
  return records
    ...
    .sort((left, right) => {
      for (const rule of rules) {
        const leftValue = getColumnCellValue(left.record, rule.key);
        const rightValue = getColumnCellValue(right.record, rule.key);
        const isBlank = (value: unknown) =>
          rule.type === "boolean"
            ? parseSemanticBoolean(value) === null
            : normalizeColumnText(value) === "";
