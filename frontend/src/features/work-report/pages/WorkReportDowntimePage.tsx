import { buildDowntimeEditPatch, downtimeEditableFields } from "../downtimeEditPatch";
import { ReloadOutlined } from "@ant-design/icons";
import { createTaskEventWakeup, subscribeWorkReportTaskEvents } from "../workReportTaskEvents";
...
import {
  createActivityLogDowntimeRecord,
  deleteActivityLogDowntimeRecord,
  exportActivityLogAnalysisXlsx,
  exportActivityLogDowntimeMonthlyCsv,
  fetchActivityLogDowntimeOptions,
  fetchActivityLogDowntimeRecords,
  fetchDowntimeTask,
  fetchDowntimeTasks,
  fetchPlannedIdleSummary,
  updateActivityLogDowntimeRecord,
  type CreateActivityLogDowntimePayload,
  type DowntimeQueueTask,
  type DowntimeTaskScope,
  type DowntimeTaskAccepted,
  type ActivityLogDowntimeRecord,
...
function getDowntimeTaskTypeLabel(
  taskType: DowntimeQueueTask["taskType"],
  t: (key: string) => string
): string {
  if (taskType === "update-downtime") {
    return t("workReport:downtimePage.tasks.updateTitle");
  }
  if (taskType === "delete-downtime") {
    return t("workReport:downtimePage.tasks.deleteTitle");
  }
  return t("workReport:downtimePage.tasks.createTitle");
}

function formatDowntimeTaskActor(task: DowntimeQueueTask): string {
  const parts = [
    task.actorLabel,
    task.actorIp,
...
const DOWNTIME_TASK_STATUS_PROGRESS: Record<DowntimeQueueTask["status"], number> = {
  pending: 0,
  running: 1,
  success: 2,
  failed: 2,
...
function compareDowntimeTaskRecency(
  left: DowntimeQueueTask,
  right: DowntimeQueueTask
): number {
...
function mergeDowntimeTaskList(
  previous: DowntimeQueueTask[],
  incoming: DowntimeQueueTask[]
): DowntimeQueueTask[] {
  const previousById = new Map(previous.map((task) => [task.taskId, task]));
  return incoming.map((task) => {
    const existing = previousById.get(task.taskId);
    if (!existing) {
      return task;
    }
    ...
    if (
      ...
      DOWNTIME_TASK_STATUS_PROGRESS[existing.status] >
        DOWNTIME_TASK_STATUS_PROGRESS[task.status]
    ) {
      return existing;
    }
    return task;
...
export function WorkReportDowntimePage() {
  ...
  const [records, setRecords] = useState<ActivityLogDowntimeRecord[]>([]);
  ...
  const editingRecordBaselineRef = useRef<ActivityLogDowntimeRecord | null>(null);
  ...
  const { activeKey: exportKey, runDownload } = useReportDownloadFeedback();
  const exporting = exportKey === "csv";
  const exportingAnalysis = exportKey === "analysis";
  ...
  const [efficiencyStatsBusy, setEfficiencyStatsBusy] = useState(false);
  ...
  const [downtimeTaskScope, setDowntimeTaskScope] = useState<DowntimeTaskScope>("all");
  ...
  const downtimeTaskListRequestGateRef = useRef(new LatestRequestGate());
  const downtimeTaskListAbortRef = useRef<AbortController | null>(null);
  const downtimeTaskListInvalidatedRef = useRef(false);
  ...
  useEffect(() => {
    const taskListRequestGate = downtimeTaskListRequestGateRef.current;
    return () => {
      ...
      taskListRequestGate.start();
      downtimeTaskListAbortRef.current?.abort();
      downtimeTaskListAbortRef.current = null;
    };
  }, []);
  useEffect(() => {
  ...
  const handleExportCsv = useCallback(() => runDownload(
    "csv", `${lastMonthInfo().label} CSV`, async () => {
      ...
      triggerBlobDownload(download.blob, download.filename ?? `c1-6-${lastMonthInfo().label}.${isXlsx ? "xlsx" : "csv"}`);
    }
  ), [runDownload]);
  ...
  const handleExportAnalysis = useCallback((attendanceDays?: number) => runDownload(
    "analysis", `${lastMonthInfo().label} Excel`, async () => {
      const download = await exportActivityLogAnalysisXlsx(attendanceDays);
      triggerBlobDownload(download.blob, download.filename ?? `c1-6-分析表-${lastMonthInfo().label}.xlsx`);
    }
  ), [runDownload]);
  ...
  const loadOptions = useCallback(async () => {
    ...
    try {
      const options = await fetchActivityLogDowntimeOptions();
  ...
  const loadRecords = useCallback(
    async (
      ...
      try {
        const result = await fetchActivityLogDowntimeRecords({
  ...
  const upsertDowntimeTaskState = useCallback((task: DowntimeQueueTask) => {
    setDowntimeTasks((previous) => {
      ...
      return next.sort(compareDowntimeTaskRecency).slice(0, DOWNTIME_TASK_LIST_LIMIT);
    });
  }, []);
  ...
  const trackDowntimeTask = useCallback(
    (taskId: string) => {
      ...
      void (async () => {
        ...
        const wakeup = createTaskEventWakeup("903", taskId);
        let first = true;
        ...
        try {
          while (
            ...
            if (!first) await wakeup.wait(wakeup.connected() ? 30_000 : DOWNTIME_TASK_POLL_INTERVAL_MS);
            first = false;
  ...
  const loadDowntimeTasks = useCallback(async (options: { background?: boolean; invalidate?: boolean } = {}): Promise<void> => {
    if (options.background && downtimeTaskListAbortRef.current) {
      if (options.invalidate) downtimeTaskListInvalidatedRef.current = true;
      return;
    }
    const requestId = downtimeTaskListRequestGateRef.current.start();
    const controller = new AbortController();
    downtimeTaskListAbortRef.current?.abort();
    downtimeTaskListAbortRef.current = controller;
    ...
    try {
      const tasks = await fetchDowntimeTasks({
        scope: downtimeTaskScope,
        actorClientId,
        limit: DOWNTIME_TASK_LIST_LIMIT,
        signal: controller.signal,
      });
      if (!downtimeTaskListRequestGateRef.current.isLatest(requestId)) {
        return;
      }
      setDowntimeTasksError(null);
      setDowntimeTasks((previous) => mergeDowntimeTaskList(previous, tasks));
    } catch (error) {
      if (
        downtimeTaskListRequestGateRef.current.isLatest(requestId) &&
        !options.background
      ) {
      ...
      if (downtimeTaskListRequestGateRef.current.isLatest(requestId)) {
        if (downtimeTaskListAbortRef.current === controller) {
          downtimeTaskListAbortRef.current = null;
          if (downtimeTaskListInvalidatedRef.current) {
            downtimeTaskListInvalidatedRef.current = false;
            void loadDowntimeTasks({ background: true });
          }
        }
        setDowntimeTasksLoading(false);
      }
    }
  }, [actorClientId, downtimeTaskScope]);

  const handleDowntimeTaskScopeChange = useCallback(
    (scope: DowntimeTaskScope) => {
      if (scope === downtimeTaskScope) {
        return;
      }
      downtimeTaskListRequestGateRef.current.start();
      downtimeTaskListAbortRef.current?.abort();
      downtimeTaskListAbortRef.current = null;
      downtimeTaskListInvalidatedRef.current = false;
      setDowntimeTaskScope(scope);
    },
    [downtimeTaskScope]
  );
  ...
  useEffect(() => {
    ...
    const refresh = () => {
      ...
      debounce = setTimeout(() => {
        debounce = undefined;
        void loadDowntimeTasks({ background: true, invalidate: true });
      }, 100);
    };
    const unsubscribe = subscribeWorkReportTaskEvents((event) => {
      if (event.formId === "903") refresh();
    }, (connected) => { setTaskEventsConnected(connected); if (connected) refresh(); });
    return () => { clearTimeout(debounce); unsubscribe(); };
  }, [loadDowntimeTasks]);

  useEffect(() => {
    const timer = window.setInterval(() => {
      if (!document.hidden) void loadDowntimeTasks({ background: true });
    }, taskEventsConnected ? 30_000 : DOWNTIME_TASK_POLL_INTERVAL_MS);
    return () => window.clearInterval(timer);
  }, [taskEventsConnected, loadDowntimeTasks]);
  ...
  const handleRetryDowntimeTask = useCallback(
    async (task: DowntimeQueueTask) => {
      ...
      try {
        const accepted = await createActivityLogDowntimeRecord(retryRecord.payload);
        const acceptedAt =
  ...
  const handleCreateRecord = useCallback(async () => {
    ...
    setSubmitting(true);
    try {
      const payload: CreateActivityLogDowntimePayload & { clientRowKey: string } = {
      ...
      const accepted = await createActivityLogDowntimeRecord(payload);
      const createdAt = accepted.createdAt ?? new Date().toISOString();
      const optimisticMutation = createDowntimeOptimisticMutation({
  ...
  const handleEditStart = useCallback((record: ActivityLogDowntimeRecord) => {
    setEditingRecordId(record.id);
    editingRecordBaselineRef.current = { ...record };
  ...
  const handleEditCancel = useCallback(() => {
    ...
    editingRecordBaselineRef.current = null;
  ...
  const handleEditSave = useCallback(async () => {
    ...
    const previousSnapshot = editingRecordBaselineRef.current;
    if (!previousSnapshot || previousSnapshot.id !== editingRecordId) return;
    const payload = buildDowntimeEditPatch(previousSnapshot, editingDraft);
    const changedFields = downtimeEditableFields.filter((key) => payload[key] !== undefined);
    ...
    try {
      const accepted = await updateActivityLogDowntimeRecord(editingRecordId, payload, {
        async: ACTIVITY_LOG_OPTIMISTIC_MUTATIONS_ENABLED,
        clientMutationId,
      });
      ...
      const optimisticMutation = createDowntimeOptimisticMutation({
        ...
        patch: {
          ...
          record: {
            ...previousSnapshot,
            ...Object.fromEntries(changedFields.map((key) => [key, payload[key] === "" ? null : payload[key]])),
            ...(payload.operatorId !== undefined ? {
              operatorName: payload.operatorId ? operatorOptionMap.get(payload.operatorId)?.label ?? null : null,
            } : {}),
    ...
    (record: ActivityLogDowntimeRecord) => {
      Modal.confirm({
        ...
        onOk: async () => {
          ...
          try {
            const accepted = await deleteActivityLogDowntimeRecord(record.id, {
  ...
  return (
    <main className="page downtime-page">
              ...
              <div className="toolbar-title-side">
                <div className="toolbar-title-actions">
                  ...
                  <button
                    ...
                    disabled={optionsLoading || submitting || headerRefreshing || exportKey !== null || efficiencyStatsBusy}
            ...
            <div className="page-view-toolbar">
              <div className="page-view-switch" role="tablist" aria-label={t("workReport:page.viewSwitchAria")}>
                <button
                  ...
                  onClick={() => navigate("/?landingPage=line-a-901&topView=report")}
                >
                  {t("workReport:page.views.lineA901")}
                </button>
                <button
                  type="button"
                  role="tab"
                  aria-selected={false}
                  className="page-view-chip"
                  onClick={() => navigate("/?landingPage=line-b-902&topView=report")}
                >
                  {t("workReport:page.views.lineB902")}
                </button>
                <button type="button" role="tab" aria-selected className="page-view-chip is-active">
                  {t("workReport:page.views.downtimeActivity")}
                </button>
                <button
                  type="button"
                  role="tab"
                  ...
                  disabled={efficiencyStatsBusy || exportKey !== null}
                  aria-busy={efficiencyStatsBusy || exportKey !== null}
                >
                  {efficiencyStatsBusy || exportKey !== null ? <LoadingSpinner size="small" /> : <CsvIcon size="1.05em" />}
                  {t(efficiencyStatsBusy || exportKey !== null ? "workReport:efficiencyStats.downloadBusy" : "workReport:efficiencyStats.entry")}
                </button>
                <button
                  type="button"
                  role="tab"
        ...
        >
          ...
          <div className="local-settings-card">
            ...
            <WorkReportDowntimeRecordsTable
              loading={recordsLoading || refreshingRecords}
              error={recordsError}
              hasRenderableContent={recordsLoaded}
          ...
          >
            {downtimeTaskSidebarCollapsed ? (
              ...
              <div className="downtime-task-sidebar-panel">
                ...
                <div
                  className="downtime-task-scope"
                  role="group"
                  aria-label={t("workReport:downtimePage.tasks.scope.label")}
                >
                  {(["all", "mine"] as const).map((scope) => (
                    <button
                      key={scope}
                      type="button"
                      className={downtimeTaskScope === scope ? "is-active" : ""}
                      aria-pressed={downtimeTaskScope === scope}
                      onClick={() => handleDowntimeTaskScopeChange(scope)}
                    >
                      {t(`workReport:downtimePage.tasks.scope.${scope}`)}
                    </button>
                  ))}
                </div>
                ...
                {downtimeTasks.length > 0 ? (
                  <div className="downtime-task-list" aria-live="polite">
                    {downtimeTasks.map((task) => {
                      ...
                      const isLocalTask = task.actorClientId === actorClientId;
                      const actorText = formatDowntimeTaskActor(task);

                      return (
                        <article
                          key={task.taskId}
                          className={`downtime-task-item is-${task.status} ${
                            isLocalTask ? "is-local-device" : "is-other-device"
                          }`}
                        >
                          <div className="downtime-task-item-head">
                            <div className="downtime-task-primary">
                              <h3>{getDowntimeTaskTypeLabel(task.taskType, t)}</h3>
                          ...
                          <div className="downtime-task-actor">
                            <span
                              className={`downtime-task-device-badge ${
                                isLocalTask ? "is-local" : "is-other"
                              }`}
                            >
                              {t(
                                isLocalTask
                                  ? "workReport:downtimePage.tasks.device.local"
                                  : "workReport:downtimePage.tasks.device.other"
                              )}
                            </span>
                            <span className="downtime-task-device-detail">
                              {actorText || t("workReport:downtimePage.tasks.device.unknown")}
                            </span>
                          </div>
      ...
      <EfficiencyStatsModal
        ...
        onBusyChange={setEfficiencyStatsBusy}
      />
      <Modal
