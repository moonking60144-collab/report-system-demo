...
import { useWorkReportReadinessProbe } from "../hooks/useWorkReportReadinessProbe";
import { isWorkReportReadyForReentry } from "../../../api/workReportReadiness";
...
export function WorkReportSystemUnavailablePage() {
  ...
  const readiness = useWorkReportReadinessProbe();
  const readyForReentry = isWorkReportReadyForReentry(readiness.snapshot);
  const readinessMode = readiness.snapshot?.mode;
  ...
  const handleRetry = async () => {
    const next = await readiness.checkNow();
    if (isWorkReportReadyForReentry(next)) {
      window.location.replace(returnTo);
    }
  };

  const readinessMessage = readiness.checking
    ? t("workReport:systemUnavailable.readinessChecking")
    : readyForReentry
      ? readinessMode === "degraded"
        ? t("workReport:systemUnavailable.readinessDegraded")
        : t("workReport:systemUnavailable.readinessReady")
      : readiness.connectionError
        ? t("workReport:systemUnavailable.readinessConnectionFailed")
        : t("workReport:systemUnavailable.readinessWaiting");
  const readinessTone = readyForReentry
    ? readinessMode === "degraded"
      ? "#fde68a"
      : "#86efac"
    : "#cbd5e1";
  const lastCheckedLabel = readiness.lastCheckedAt
    ? new Date(readiness.lastCheckedAt).toLocaleTimeString("zh-TW", { hour12: false })
    : "--";
  const retryButtonLabel = readiness.checking
    ? t("workReport:systemUnavailable.readinessCheckingAction")
    : readyForReentry
      ? t("workReport:systemUnavailable.reenter")
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            padding: "0.95rem 1rem",
            borderRadius: "14px",
            border: `1px solid ${readyForReentry ? "rgba(34,197,94,0.3)" : "rgba(148,163,184,0.2)"}`,
            background: readyForReentry
              ? "rgba(20,83,45,0.22)"
              : "rgba(15,23,42,0.78)",
            color: readinessTone,
            lineHeight: 1.6,
        ...
        >
          ...
          <span style={{ color: "#94a3b8", fontSize: "0.82rem" }}>
            {t("workReport:systemUnavailable.lastChecked", {
        ...
        <div style={{ display: "flex", justifyContent: "flex-end" }}>
          <button
            ...
            style={{
              ...
              borderRadius: "12px",
              border: readyForReentry
                ? "1px solid #16a34a"
                : "1px solid #d97706",
              background: readyForReentry ? "#15803d" : "#b45309",
              color: "#fffbeb",
              cursor: readiness.checking ? "wait" : "pointer",
              fontWeight: 700,
              opacity: readiness.checking ? 0.72 : 1,
            }}
          >
            {retryButtonLabel}
          </button>
