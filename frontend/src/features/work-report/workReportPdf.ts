...
function appendRenderMachineSection(
  ...
  const sourceColumnGroup = sourceTable?.querySelector<HTMLTableColElement>("colgroup");
  ...
  if (includeMachineHeader) {
    ...
    if (machineHeader) {
      sheet.append(renderDocument.importNode(machineHeader, true));
    }
  } else {
  ...
  if (sourceColumnGroup) {
    table.append(renderDocument.importNode(sourceColumnGroup, true));
  }
      ...
      do {
        ...
        while (rowIndex < sourceRows.length) {
          ...
          if (page.scrollHeight > page.clientHeight) {
            ...
            if (rowsInSection > 0 || includeMachineHeader) {
              await queueCurrentPage();
              includeMachineHeader = false;
              retryOnNewPage = true;
              break;
            }
            const workOrderNo = sourceRows[rowIndex]
              .querySelector<HTMLTableCellElement>('td[data-column-key="workOrderNo"]')
              ?.textContent?.trim();
            throw new Error(
