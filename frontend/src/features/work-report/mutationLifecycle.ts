...
export interface OptimisticMutationTarget {
  domain: "work-report" | "activityLog-downtime";
  formId: "903" | "901" | "902";
  entryId: string;
  rowId?: string;
  clientRowKey?: string;
...
export function isStoredMutationLifecycle<TPreviousSnapshot = unknown>(
  ...
  if (
    ...
    !target ||
    (target.domain !== "work-report" && target.domain !== "activityLog-downtime") ||
    (target.formId !== "903" && target.formId !== "901" && target.formId !== "902") ||
    (target.domain === "work-report" && target.formId === "903") ||
    (target.domain === "activityLog-downtime" && target.formId !== "903") ||
