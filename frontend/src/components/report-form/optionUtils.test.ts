...
function makeMachineOption(
  ...
  return {
    value,
    label: label ?? `${value} - ${value}製程 A機`,
    display: label ?? `${value} - ${value}製程 A機`,
...
describe("filterActiveMachineOptionsWithCurrentValue", () => {
  const inUse1 = makeMachineOption("MB50", "使用中");
  ...
  test("僅保留狀態為「使用中」的機台；無狀態視為可選（防禦性）", () => {
    ...
    expect(result.map((o) => o.value)).toEqual(["MB50", "P50", "P20"]);
  });

  test("當前值為使用中時不動清單", () => {
    const result = filterActiveMachineOptionsWithCurrentValue([inUse1, retired, inUse2], "P20");
    expect(result.map((o) => o.value)).toEqual(["MB50", "P20"]);
  });
  ...
  test("當前值為報廢時，補回清單頂部並加 status suffix 到 label 跟 display", () => {
    ...
    expect(result.map((o) => o.value)).toEqual(["P30", "MB50", "P20"]);
    expect(result[0]?.label).toBe("P30 - P30製程 A機（報廢）");
    expect(result[0]?.display).toBe("P30 - P30製程 A機（報廢）");
  });

  test("當前值為售出時也加 status suffix 到兩個欄位", () => {
    const result = filterActiveMachineOptionsWithCurrentValue([inUse1, sold], "P40");
    expect(result[0]?.label).toBe("P40 - P40製程 A機（售出）");
    expect(result[0]?.display).toBe("P40 - P40製程 A機（售出）");
  });

  test("當前值空字串時不補位", () => {
    const result = filterActiveMachineOptionsWithCurrentValue([inUse1, retired], "   ");
    expect(result.map((o) => o.value)).toEqual(["MB50"]);
  });
  ...
  test("當前值不在原始清單中，fallback 用純字串 token 補位（罕見情境）", () => {
    ...
    expect(result[1]?.value).toBe("MB50");
  });

  test("options undefined 時不爆，單純用當前值 fallback 或回空陣列", () => {
    expect(filterActiveMachineOptionsWithCurrentValue(undefined, "")).toEqual([]);
    const withCurrent = filterActiveMachineOptionsWithCurrentValue(undefined, "MB50");
    expect(withCurrent).toEqual([{ value: "MB50", label: "MB50", display: "MB50" }]);
  });
...
describe("buildMachineStatusSuffix", () => {
  test("使用中 / 無狀態回空字串（單一 source of truth）", () => {
    expect(buildMachineStatusSuffix({
      ...
      label: "MB50",
      display: "MB50",
      machineDefault: { machineCode: "MB50", status: "使用中" },
    })).toBe("");
    expect(buildMachineStatusSuffix({ value: "MB50", label: "MB50", display: "MB50" })).toBe("");
  });
...
describe("decorateMachineOptionLabel", () => {
  test("使用中 / 無狀態回原 label", () => {
    expect(decorateMachineOptionLabel(makeMachineOption("MB50", "使用中"))).toBe(
      "MB50 - MB50製程 A機"
    );
    expect(decorateMachineOptionLabel(makeMachineOption("MB50", undefined))).toBe(
      "MB50 - MB50製程 A機"
    );
  });
...
describe("isActiveMachineOption", () => {
  test("使用中 / 無狀態 / null 資料都視為 active（防禦性 - null 回 false 不過濾光）", () => {
    expect(isActiveMachineOption(makeMachineOption("MB50", "使用中"))).toBe(true);
    expect(isActiveMachineOption(makeMachineOption("MB50", undefined))).toBe(true);
