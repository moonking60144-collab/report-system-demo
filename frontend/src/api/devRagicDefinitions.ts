...
export type DevAiKnowledgeCandidateSubmitInput = DevAiKnowledgeCandidateSubmitRequest;
export type DevAiCreateThreadInput = DevAiCreateThreadRequest;
export type DevAiSendMessageInput = DevAiSendMessageRequest;

export async function fetchDevAiReadiness(token: string): Promise<DevAiReadiness> {
  const response = await api.get<{ data: DevAiReadiness }>("/dev/ai/readiness", {
...
export async function submitDevAiKnowledgeCandidate(
  token: string,
  input: DevAiKnowledgeCandidateSubmitRequest,
  options: { signal?: AbortSignal } = {}
): Promise<DevAiKnowledgeCandidateSubmissionResult> {
  const response = await api.post<{ data: DevAiKnowledgeCandidateSubmissionResult }>(
    "/dev/ai/knowledge/candidates",
    input,
    { headers: authHeaders(token), signal: options.signal }
  );
  return response.data.data;
...
export async function fetchDevAiKnowledgeCandidates(
  token: string,
  params: {
  ...
  const response = await api.get<{ data: DevAiKnowledgeCandidateListResult }>(
    "/dev/ai/knowledge/candidates",
    { headers: authHeaders(token), params, signal: options.signal }
  );
  return response.data.data;
...
export async function updateDevAiKnowledgeCandidate(
  token: string,
  candidateId: string,
  input: DevAiKnowledgeCandidateUpdateRequest
): Promise<DevAiKnowledgeCandidate> {
  const response = await api.patch<{ data: DevAiKnowledgeCandidate }>(
    `/dev/ai/knowledge/candidates/${candidateId}`,
    input,
    { headers: authHeaders(token) }
  );
  return response.data.data;
...
export async function approveDevAiKnowledgeCandidate(
  token: string,
  candidateId: string,
  note = ""
): Promise<DevAiKnowledgeCandidate> {
  const response = await api.post<{ data: DevAiKnowledgeCandidate }>(
    `/dev/ai/knowledge/candidates/${candidateId}/approve`,
    { note },
    { headers: authHeaders(token) }
  );
  return response.data.data;
...
export async function rejectDevAiKnowledgeCandidate(
  token: string,
  candidateId: string,
  note = ""
): Promise<DevAiKnowledgeCandidate> {
  const response = await api.post<{ data: DevAiKnowledgeCandidate }>(
    `/dev/ai/knowledge/candidates/${candidateId}/reject`,
    { note },
    { headers: authHeaders(token) }
  );
  return response.data.data;
...
export async function fetchDevAiKnowledgeRuntimeStatus(
  token: string
): Promise<DevAiKnowledgeRuntimeStatus> {
  const response = await api.get<{ data: DevAiKnowledgeRuntimeStatus }>(
    "/dev/ragic-definitions/ai/knowledge/runtime-status",
    { headers: authHeaders(token) }
  );
  return response.data.data;
}
