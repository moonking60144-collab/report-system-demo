...
export interface ActivityLogDowntimeRecord {
  id: string;
...
export interface CreateActivityLogDowntimePayload {
  date: string;
  machineId: string;
  processCode: string;
  operatorId?: string;
...
export interface FetchActivityLogDowntimeRecordsOptions {
  limit?: number;
  offset?: number;
  refresh?: boolean;
}

export interface FetchActivityLogDowntimeRecordsResult {
...
export async function fetchActivityLogDowntimeOptions(): Promise<FormOptionMap> {
  const response = await api.get<{ data: FormOptionMap }>("/downtime/options");
  return response.data.data;
}

export async function fetchActivityLogDowntimeRecords(
  options: FetchActivityLogDowntimeRecordsOptions = {}
): Promise<FetchActivityLogDowntimeRecordsResult> {
  const response = await api.get<{
    data: ActivityLogDowntimeRecord[];
    meta: FetchActivityLogDowntimeRecordsResult["meta"];
  }>("/downtime/records", {
    params: {
...
export async function exportActivityLogDowntimeMonthlyCsv(): Promise<ApiDownload> {
...
export async function exportActivityLogAnalysisXlsx(attendanceDays?: number): Promise<ApiDownload> {
...
export type CreateActivityLogDowntimeAccepted = DowntimeTaskAccepted;
...
export type DowntimeTaskScope = "mine" | "all";
...
export async function createActivityLogDowntimeRecord(
  payload: CreateActivityLogDowntimePayload & { clientRowKey: string }
...
export interface UpdateActivityLogDowntimePayload {
  date?: string;
  machineId?: string;
  processCode?: string;
  /** 傳空字串代表清空操作者 */
  ...
  expectedValues?: Partial<Pick<ActivityLogDowntimeRecord, "date" | "machineId" | "processCode" | "operatorId" | "plannedIdleMinutes" | "remark">>;
}

export async function updateActivityLogDowntimeRecord(
  entryId: string,
  payload: UpdateActivityLogDowntimePayload,
...
export async function deleteActivityLogDowntimeRecord(
  entryId: string,
  options: {
    expectedSnapshotHash?: string | null;
    async?: boolean;
