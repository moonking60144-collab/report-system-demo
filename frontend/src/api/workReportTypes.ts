...
export interface WorkReportReadMeta {
...
export interface WorkReportFacetResponse {
  data: Record<string, WorkReportFacetCount[]>;
  meta: WorkReportReadMeta & {
    formId: string;
    fields: string[];
  };
...
export interface WorkReportAnalysisResponse {
  data: WorkReportAnalysisSummary;
  meta: WorkReportReadMeta & {
    formId: string;
    field: string;
...
export interface WorkReportEntryResponse {
  data: WorkReportRecord;
  meta: WorkReportReadMeta;
}
...
export interface WorkReportQueueTask extends MutationLifecycleTiming {
  ...
  /** 系統事件來源（e.g. demo-activity-callback、ragic-form-save）；跟 actorLabel 分流 */
  source: string | null;
