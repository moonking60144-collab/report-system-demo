...
const SAMPLE_ENTRIES = [
  ...
  },
  {
    ...
    subtableKey: "9001042",
    fieldPos: "E1",
    fieldName: "demo_work_order_type",
    fieldId: "9001051",
    fieldType: "選項",
  },
  {
  ...
  {
    formPath: "default/demo/work-orders/line-b",
    formName: "[902] 報工表",
    scope: "main" as const,
    subtableKey: "999",
    fieldPos: "A1",
...
test("replaceAll 寫入後 countAll 回正確統計", async () => {
  ...
  assert.equal(counts.totalForms, 2); // 901, 902
...
test("純讀查詢走 read provider，不佔用 write provider", async () => {
  ...
  try {
    ...
    readProviderCalls = 0;
    await repo.search({ q: "901" });
    await repo.countAll();
    await repo.getState();
    await repo.listFormFieldPositions("default/demo/work-orders/line-a");
    await repo.listVersionSiblingForms("default/demo/work-orders/line-a");
...
test("pending generation 未 promote 前不會被讀取，promote 後才切換", async () => {
  ...
  assert.equal(afterPromote[0]?.formPath, "default/forms999/9099");
  assert.equal((await repo.countAll()).totalFields, 1);
});
...
test("search 用 q LIKE 比對 form 名 / 欄位名 / id", async () => {
  ...
  // 搜 form 名（"901"）
  const r1 = await repo.search({ q: "901" });
  assert.equal(r1.length, 3); // 901 主 2 + 子 1
  ...
  // 搜 field id（明確）
  const r3 = await repo.search({ q: "9001062" });
  assert.equal(r3.length, 1);
  assert.equal(r3[0]?.fieldId, "9001062");
});

test("search 用 formPath 過濾", async () => {
  const { repo } = await buildRepo();
  await repo.replaceAll(SAMPLE_ENTRIES, "2026-05-08T00:00:00.000Z");
  const r = await repo.search({ formPath: "default/demo/work-orders/line-b" });
...
test("search 用 fieldId 過濾（精確比對）", async () => {
  const { repo } = await buildRepo();
  await repo.replaceAll(SAMPLE_ENTRIES, "2026-05-08T00:00:00.000Z");
  const r = await repo.search({ fieldId: "9001051" });
  assert.equal(r.length, 1);
  assert.equal(r[0]?.fieldName, "demo_work_order_type");
});
...
test("search 3+ 字元 query 走 FTS5 trigram、結果跟 LIKE 一致", async () => {
  const { repo } = await buildRepo();
  await repo.replaceAll(SAMPLE_ENTRIES, "2026-05-08T00:00:00.000Z");
  // 3 字元 → 走 FTS path（內部 query `"901"` MATCH trigram index）
  const r = await repo.search({ q: "901" });
...
test("search < 3 字元 query 走 LIKE fallback、不是 FTS", async () => {
  ...
  const r = await repo.search({ q: "04" }); // 應仍命中 901
  assert.ok(r.length > 0);
});

test("search 含 bracket query '[901]' 實際命中 row，不是只 doesNotReject", async () => {
  const { repo } = await buildRepo();
  await repo.replaceAll(SAMPLE_ENTRIES, "2026-05-08T00:00:00.000Z");
  // bare '[901]' 不 phrase wrap 會炸 FTS syntax error；repository 應該 phrase
  // wrap 後安全命中 form_name '[901] 工令單製程 A 報工+排程'
  const r = await repo.search({ q: "[901]" });
  assert.equal(r.length, 3, "[901] 應命中 form 901 的 3 筆");
});
...
test("search 含 double-quote query 'a\"b\"c' 不只不炸、也能命中包含相同 substring 的內容", async () => {
  ...
  await repo.replaceAll(
    [
      ...SAMPLE_ENTRIES,
      {
        formPath: "default/forms999/9099",
...
test("search FTS operator (AND / OR / NEAR / *) 被當 literal substring，不解讀為 operator", async () => {
  ...
  await repo.replaceAll(
    [
      {
        formPath: "default/forms999/90037",
      ...
      {
        formPath: "default/forms999/90038",
...
test("replaceAll 中途 INSERT 失敗 → active 不切換，staged generation 會清掉", async () => {
  ...
  const validBulk = Array.from({ length: 1000 }, (_, i) => ({
    formPath: `default/forms999/${i + 2000}`,
  ...
  // FTS 仍搜得到舊 active 資料。
  const stillSearchable = await repo.search({ q: "9001062" });
...
test("replaceAll 大量資料 (batch chunk) 仍能正確查回", async () => {
  ...
  const bulk = Array.from({ length: 1200 }, (_, i) => ({
    formPath: `default/forms999/${i + 1000}`,
