...
import { createActivityLogClientRowKeyRepository } from "../../src/storage/sqlite/activityLogClientRowKeyRepository";
...
async function buildRepo() {
  ...
  const repo = createActivityLogClientRowKeyRepository(async () => db);
  return { db, repo };
}
...
test("reservePending 新 key 會建立 pending 紀錄", async () => {
  ...
  assert.equal(result.record?.source, "work-report-901");
...
test("cleanupOlderThan 依 createdAt 閾值刪舊資料", async () => {
  ...
  await db.run(
    `INSERT INTO activityLog_client_row_keys (client_row_key, entry_id, source, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?)`,
    "old-key",
    "E-OLD",
    "downtime",
