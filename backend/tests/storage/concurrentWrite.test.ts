...
// repo（activityLog / workReport snapshot，由 auto-sync 觸發）在同一連線發 BEGIN，舊架構
...
async function buildSharedConnection(): Promise<Database> {
  // 一條連線同時帶 read-model schema（activityLog_downtime_*）與 field-index schema，
...
// 模擬 activityLogDowntimeSqliteRepository.replaceSnapshot 的交易（DELETE 全表 → INSERT →
// state upsert），但走呼叫端注入的 serializer，這樣它與 field-index repo 共用同連線的
// writeChain。structurally 等同 production replaceSnapshot。
async function replaceActivityLogSnapshot(
  ...
  await withWriteTransaction(async (db) => {
    ...
    for (const record of records) {
      await db.run(
        `INSERT INTO activityLog_downtime_records (entry_id, machine_id, raw_json, synced_at)
         VALUES (?, ?, ?, ?)`,
        record.entryId,
        record.machineId,
    ...
    await db.run(
      `INSERT INTO activityLog_downtime_state (id, snapshot_at, total_records, updated_at)
       VALUES (1, ?, ?, ?)
...
test("兩個 repo 同連線 concurrent 交易寫入：都不丟 'cannot start a transaction within a transaction' 且都完成", async () => {
  ...
  const activityLogSerializer = createConnectionSerializer(async () => db);
  ...
  const results = await withTimeout(
    Promise.allSettled([
      ...
      replaceActivityLogSnapshot(
        activityLogSerializer.withWriteTransaction,
        downtimeRecords,
  ...
  const downtimeCount = await db.get<{ c: number }>(
    "SELECT COUNT(*) AS c FROM activityLog_downtime_records"
  );
  assert.equal(downtimeCount?.c, 2, "activityLog snapshot replace 應完成且寫入 2 筆");
  const stateRow = await db.get<{ total_records: number }>(
    "SELECT total_records FROM activityLog_downtime_state WHERE id = 1"
  );
...
test("兩筆 replaceAll-like 交易透過兩個獨立 serializer 同連線 concurrent fire：不 nested BEGIN、都完成", async () => {
  ...
  const results = await withTimeout(
    Promise.allSettled([
      replaceActivityLogSnapshot(serializerA.withWriteTransaction, recordsA, "2026-06-02T01:00:00.000Z"),
      replaceActivityLogSnapshot(serializerB.withWriteTransaction, recordsB, "2026-06-02T02:00:00.000Z"),
    ]),
    2000,
  ...
  const count = await db.get<{ c: number }>("SELECT COUNT(*) AS c FROM activityLog_downtime_records");
  assert.ok(
