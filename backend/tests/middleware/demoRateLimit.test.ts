...
test("Demo 業務寫入維持每分鐘 30 次上限", () => {
  ...
  for (let index = 0; index < 30; index += 1) {
    assert.equal(invokeRateLimit("/api/forms/901/reports/E-901").nextCalled, true);
  }

  const rejected = invokeRateLimit("/api/forms/901/reports/E-901");
...
test("editing presence 使用獨立額度，不會耗掉業務寫入限流", () => {
  ...
  for (let index = 0; index < 40; index += 1) {
    const result = invokeRateLimit(
      "/api/forms/901/reports/E-901/editing-presence"
    );
  ...
  assert.equal(invokeRateLimit("/api/forms/901/reports/E-901").statusCode, 429);
});
...
test("editing presence 超過獨立額度仍會被保護", () => {
  ...
  for (let index = 0; index < 300; index += 1) {
    assert.equal(
      invokeRateLimit("/api/forms/901/reports/E-901/editing-presence").nextCalled,
      true
    );
  }
