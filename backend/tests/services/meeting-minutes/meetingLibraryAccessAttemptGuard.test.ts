...
test("Meeting 錄音庫存取碼：低門檻只限制同一個 browser client", () => {
  ...
  const firstClient = { clientId: "client-browser-a", ip: "192.0.2.28" };
  const secondClient = { clientId: "client-browser-b", ip: "192.0.2.28" };
...
test("Meeting 錄音庫存取碼：高門檻 IP bucket 可阻擋輪換 client id", () => {
  ...
  guard.recordFailure({ clientId: "client-browser-a", ip: "192.0.2.28" });
  guard.recordFailure({ clientId: "client-browser-b", ip: "192.0.2.28" });
  assert.throws(
    () =>
      guard.recordFailure({ clientId: "client-browser-c", ip: "192.0.2.28" }),
    isRateLimited
  );
});
...
test("Meeting 錄音庫存取碼：成功授權只清除該 browser 的失敗紀錄", () => {
  ...
  const identity = { clientId: "client-browser-a", ip: "192.0.2.28" };
...
test("Meeting 錄音庫存取碼：單一 browser 成功不會清除共用 IP 防繞過計數", () => {
  ...
  guard.recordFailure({ clientId: "client-browser-a", ip: "192.0.2.28" });
  guard.recordSuccess({ clientId: "client-browser-a", ip: "192.0.2.28" });
  assert.throws(
    () => guard.recordFailure({ clientId: "client-browser-b", ip: "192.0.2.28" }),
    isRateLimited
  );
