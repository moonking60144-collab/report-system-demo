...
async function buildDryRunFixtureWithOptions(
  ...
  const line = `D,7,6,9001108,測試,text=1&f=${lineFormula}\n`;
  const lineBytes =
...
test("dry-run：UTF-8 舊公式含中文與智慧引號仍可通過比對", async () => {
  ...
  await writeFile(
    join(builderRoot, sourceRelativePath),
    `D,7,6,9001108,測試,text=1&f=${oldFormula}\n`,
    "utf-8"
  );
  const definitionsService = {
    getState: async () => ({
  ...
  try {
    ...
    assert.equal(result.oldLinePreview, `D,7,6,9001108,測試,text=1&f=${oldFormula}`);
...
async function buildDuplicateOccurrenceFixture() {
  const root = await mkdtemp(join(tmpdir(), "ragic-dryrun-occurrence-"));
  const builderRoot = join(root, "builder");
  const sourceRelativePath = "default/devtest/51_Sheet51_index.nui";
  await mkdir(join(builderRoot, "default", "devtest"), { recursive: true });
  await writeFile(
    join(builderRoot, sourceRelativePath),
    [
      "D,1,1,9001108,測試,text=1&f=C3",
      "D,3,3,9001108,測試,text=1&f=D3",
      "D,2,1,1002,來源一,text=1&f=1",
      "D,4,3,1004,來源二,text=1&f=1",
    ].join("\n"),
    "utf-8"
  );
  const definitionsService = {
    ...
    readForm: async () => ({
      form: {
        ...
        sourceEncoding: "utf-8",
        sourceRelativePath,
        counts: { fields: 4, formulas: 4, workflows: 0 },
      },
      fields: [
        { fieldId: "9001108", fieldName: "測試", kind: "D", position: "A1", sourceLine: 1, attrs: {} },
        { fieldId: "9001108", fieldName: "測試", kind: "D", position: "C3", sourceLine: 2, attrs: {} },
        { fieldId: "1002", fieldName: "來源一", kind: "D", position: "B1", sourceLine: 3, attrs: {} },
        { fieldId: "1004", fieldName: "來源二", kind: "D", position: "D3", sourceLine: 4, attrs: {} },
      ],
      formulas: [
        { fieldId: "9001108", fieldName: "測試", position: "A1", formulaKind: "formula" as const, nuiFormula: "C3", displayFormula: "C3", sourceLine: 1 },
        { fieldId: "9001108", fieldName: "測試", position: "C3", formulaKind: "formula" as const, nuiFormula: "D3", displayFormula: "D3", sourceLine: 2 },
        { fieldId: "1002", fieldName: "來源一", position: "B1", formulaKind: "formula" as const, nuiFormula: "1", displayFormula: "1", sourceLine: 3 },
        { fieldId: "1004", fieldName: "來源二", position: "D3", formulaKind: "formula" as const, nuiFormula: "1", displayFormula: "1", sourceLine: 4 },
      ],
      workflows: [],
    }),
  } as unknown as RagicDefinitionsReadService;
  return {
    root,
    service: createRagicFormulaPatchDryRunService({ definitionsService, builderRoot }),
  };
}

test("dry-run：legacy request 遇到同 fieldId 多 occurrence 時 fail closed", async () => {
  const fixture = await buildDuplicateOccurrenceFixture();
  try {
    const result = await fixture.service.dryRunFormulaPatch({
      formPath: "default/devtest/51",
      fieldId: "9001108",
...
test("dry-run：position + sourceLine 精確命中同 fieldId 的第二個 occurrence", async () => {
  const fixture = await buildDuplicateOccurrenceFixture();
  try {
    const result = await fixture.service.dryRunFormulaPatch({
      formPath: "default/devtest/51",
    ...
    assert.equal(result.allowed, true);
    assert.equal(result.position, "C3");
    assert.equal(result.sourceLine, 2);
    assert.equal(result.oldFormula, "D3");
    assert.match(result.oldLinePreview ?? "", /D,3,3,9001108/);
  } finally {
    await rm(fixture.root, { recursive: true, force: true });
  }
});

test("dry-run：position 正確但 sourceLine 不同時視為 stale occurrence", async () => {
  const fixture = await buildDuplicateOccurrenceFixture();
  try {
    const result = await fixture.service.dryRunFormulaPatch({
      formPath: "default/devtest/51",
...
test("dry-run：同 fieldId 不同 position 的循環路徑不會被合併", async () => {
  const fixture = await buildDuplicateOccurrenceFixture();
  try {
    const result = await fixture.service.dryRunFormulaPatch({
      formPath: "default/devtest/51",
