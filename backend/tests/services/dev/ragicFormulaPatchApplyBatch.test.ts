...
const FORMS = [
  { id: "51", fieldId: "9001108", oldFormula: "F6*D6+1" },
  { id: "56", fieldId: "9001108", oldFormula: "F6*C6+2" },
];
...
test("同 fieldId 多 occurrence 可精確套用，且同檔批次 rollback 回到批次前內容", async () => {
  const fixture = await buildFixture();
  const formDir = join(fixture.root, "forms", "default", "devtest", "51");
  const nuiPath = join(fixture.nuiDir, "51_Sheet51_index.nui");
  try {
    await writeJson(join(formDir, "fields.json"), [
      { fieldId: "9001108", fieldName: "測試", kind: "D", position: "A1", sourceLine: 10, attrs: { text: "1" } },
      { fieldId: "9001108", fieldName: "測試", kind: "D", position: "C3", sourceLine: 24, attrs: { text: "1" } },
    ]);
    await writeJson(join(formDir, "formulas.json"), [
      { fieldId: "9001108", fieldName: "測試", position: "A1", formulaKind: "formula", nuiFormula: "B1", displayFormula: "B1", sourceLine: 10 },
      { fieldId: "9001108", fieldName: "測試", position: "C3", formulaKind: "formula", nuiFormula: "D3", displayFormula: "D3", sourceLine: 24 },
    ]);
    const lines = Array.from({ length: 26 }, (_, index) => `# filler ${index + 1}`);
    lines[9] = "D,1,1,9001108,測試,text=1&f=B1";
    lines[23] = "D,3,3,9001108,測試,text=1&f=D3";
    await writeFile(nuiPath, `${lines.join("\n")}\n`, "utf-8");
    await syncFixtureDefinitionCounts(fixture);

    const exportImpl = async () => {
      const currentLines = (await readFile(nuiPath, "utf-8")).split(/\r?\n/);
      const entries = [
        { position: "A1", sourceLine: 10 },
        { position: "C3", sourceLine: 24 },
      ].map(({ position, sourceLine }) => {
        const formula = (currentLines[sourceLine - 1] ?? "")
          .split("&")
          .find((part) => part.startsWith("f="))
          ?.slice(2) ?? "";
        return {
      ...
      await writeJson(join(formDir, "formulas.json"), entries);
      return { stdout: "[test] exported occurrences", stderr: "" };
    };
    ...
    assert.equal(result.applied, true);
    assert.equal(result.verifiedFormula?.position, "C3");
    assert.equal(result.verifiedFormula?.sourceLine, 24);
    const nui = await readFile(nuiPath, "utf-8");
    assert.match(nui, /D,1,1,9001108,測試,text=1&f=B1/);
    assert.match(nui, /D,3,3,9001108,測試,text=1&f=D3\*2/);
    const audit = await readFile(join(fixture.root, "audit.jsonl"), "utf-8");
    assert.match(audit, /"position":"C3"/);
    assert.match(audit, /"sourceLine":24/);

    const firstRollback = await applyService.rollbackLatestFormulaPatch();
    assert.equal(firstRollback.rolledBack, true);

    const batch = await applyService.applyFormulaPatchBatch([
      {
        formPath: "default/devtest/51",
        fieldId: "9001108",
        formulaKind: "formula",
    ...
    assert.deepEqual(
      batch.results.map((target) => target.verifiedFormula?.position),
      ["A1", "C3"]
    );
    const patchedBoth = await readFile(nuiPath, "utf-8");
    assert.match(patchedBoth, /D,1,1,9001108,測試,text=1&f=B1\*2/);
    assert.match(patchedBoth, /D,3,3,9001108,測試,text=1&f=D3\*3/);

    const batchRollback = await applyService.rollbackLatestFormulaPatch();
    assert.equal(batchRollback.rolledBack, true);
    assert.equal(batchRollback.restoredCount, 1);
    const restored = await readFile(nuiPath, "utf-8");
    assert.match(restored, /D,1,1,9001108,測試,text=1&f=B1/);
    assert.match(restored, /D,3,3,9001108,測試,text=1&f=D3/);
  } finally {
    await rm(fixture.root, { recursive: true, force: true });
  }
});
...
test("批次套用：兄弟表欄位存在但沒有既有公式時可新增 f= attr", async () => {
  ...
  try {
    ...
    nui56Lines[23] = "D,7,6,9001108,測試,text=1";
    await writeFile(
...
test("單筆 apply：顯示用逗號公式會以 Ragic .nui backtick canonical 寫入並驗證", async () => {
  ...
  try {
    ...
    const nuiLines = Array.from({ length: 26 }, (_, index) => `# filler ${index + 1}`);
    nuiLines[23] = "D,7,6,9001108,測試,text=1&f=IF(A1`B1`C1)";
    await writeFile(
...
test("單筆 apply：UTF-8 old formula raw 含非 ASCII 時可套用", async () => {
  ...
  try {
    ...
    const nuiLines = Array.from({ length: 26 }, (_, index) => `# filler ${index + 1}`);
    nuiLines[23] = `D,7,6,9001108,測試,text=1&f=${oldFormula}`;
    await writeFile(
      join(fixture.nuiDir, "51_Sheet51_index.nui"),
      `${nuiLines.join("\n")}\n`,
      "utf-8"
...
test("單筆 apply：Big5 sourceEncoding 可成功 patch 中文公式且不動其他 attr bytes", async () => {
  ...
  try {
    ...
    const big5NuiLine = `D,7,6,9001108,測試,text=1&note=KEEP&f=${oldFormula}`;
...
test("單筆 apply：Big5 不可表示字元應阻擋 patch，避免被寫成 ?", async () => {
  ...
  try {
    ...
    const big5NuiLine = `D,7,6,9001108,測試,text=1&f=${oldFormula}`;
    await writeFile(nuiPath, buildNuiFileWithLine(big5NuiLine, "big5"));
...
test("單筆 apply：非 UTF-8 下 old attr 位元組找不到時安全失敗", async () => {
  ...
  try {
    ...
    const mismatchLine = `D,7,6,9001108,測試,text=1&note=KEEP&f=${"中"}`;
...
test("非 UTF-8：f 殘留在 dv_f 前綴中時仍只會替換正確 f attr", async () => {
  ...
  try {
    ...
    const line = "D,7,6,9001108,測試,text=1&dv_f=A1&f=A1&note=KEEP";
...
test("非 UTF-8：new formula 只對 &,\r,\n 編碼，% 保留字面", async () => {
  ...
  try {
    ...
    const line = 'D,7,6,9001108,測試,text=1&f=A1&note=KEEP';
    await writeFile(nuiPath, buildNuiFileWithLine(line, "big5"));
