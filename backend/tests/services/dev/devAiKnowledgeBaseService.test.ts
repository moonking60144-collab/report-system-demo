...
test("Dev AI knowledge search 會從本地 markdown/jsonl 找相關來源", async () => {
  ...
  await mkdir(path.join(root, "democo"), { recursive: true });
  await writeFile(
    path.join(root, "democo", "company.md"),
    "# DemoCo 內部口徑\nDemoCo 公司問題只能依據內部文件回答，不可編造。",
    "utf8"
  );
  await writeFile(
  ...
  const result = await service.search({ query: "DemoCo 公司 怎麼回答" });

  assert.equal(result.length >= 1, true);
  assert.equal(result[0].kind, "curated");
  assert.equal(result[0].path, "democo/company.md");
...
test("Dev AI knowledge search 缺資料夾時回空陣列", async () => {
  ...
  const result = await service.search({ query: "DemoCo" });
...
test("Dev AI knowledge search 一般 DemoCo 問答不會誤撈官方 Ragic seed", async () => {
  const service = createDevAiKnowledgeBaseService({
...
test("Dev AI knowledge search 會跳過讀取失敗的單一檔案並保留其他來源", async () => {
  ...
  await writeFile(path.join(root, "good.md"), "# DemoCo good\nusable knowledge marker", "utf8");
