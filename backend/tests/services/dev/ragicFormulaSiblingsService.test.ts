...
// 兩張多版本工令單（同 mainKey 9001042）+ 一張不相干表單（不同 mainKey 但故意
// 撞同號 fieldId，驗證不會被誤抓）
const ENTRIES = [
  {
  ...
  {
    formPath: "default/demo/work-orders/line-a",
    formName: "[901] 工令單製程 A 報工+排程",
    scope: "main" as const,
    subtableKey: "9001042",
    fieldPos: "C3",
    fieldName: "規格",
  ...
  {
    formPath: "default/forms999/9003",
...
test("同 mainKey 找齊兄弟表單、排除自己，不同 mainKey 同號 fieldId 不誤抓", async () => {
  ...
  assert.deepEqual(paths, ["default/demo/work-orders/line-a", "default/forms999/9003"].sort());
...
test("includeCurrent=true 會把目前表單納入同一套 freshness 判定", async () => {
  ...
  await writeFile(root + "/x.nui", "D,1,6,9001106,規格,f=C3+D2\n", "utf-8");
  ...
  try {
    ...
    assert.deepEqual(
      paths,
      ["default/forms999/9001", "default/demo/work-orders/line-a", "default/forms999/9003"].sort()
    );
    const current = result.siblings.find((sibling) => sibling.formPath === "default/forms999/9001");
...
test("hasField 正確標示；現行公式從 definitions 取得", async () => {
  ...
  const form901 = result.siblings.find((s) => s.formPath === "default/demo/work-orders/line-a");
  const form77 = result.siblings.find((s) => s.formPath === "default/forms999/9003");
  assert.equal(form901?.hasField, true);
  assert.equal(form901?.fieldPosition, "D2");
  // 77 沒有「數量」欄位
...
test("帶 newFormula 時附位置翻譯：92 的 A6+B6 → 901 的 C3+D2", async () => {
  const { db, service } = await buildService();
  const result = await service.listSiblings({
    formPath: "default/forms999/9001",
    fieldId: "9001106",
  ...
  const form901 = result.siblings.find((s) => s.formPath === "default/demo/work-orders/line-a");
  assert.equal(form901?.translation?.translated, "C3+D2");
  assert.equal(form901?.translation?.mapping.length, 2);
...
test("跨版本公式推估只用 main 欄位，主表與子表同位置時不被子表覆蓋", async () => {
  ...
  assert.equal(form901?.translation?.translated, "C3+D2");
  assert.deepEqual(
    form901?.translation?.mapping.map((item) => item.fieldId),
    ["9001106", "9001107"]
  );
  await db.close();
});
...
test("目標表同 fieldId 對到多個位置時拒絕自動推估，避免靜默取錯位置", async () => {
  ...
  const form901 = result.siblings.find((s) => s.formPath === "default/demo/work-orders/line-a");
  assert.equal(form901?.translation?.translated, null);
  assert.ok(
    form901?.translation?.untranslatable.some((item) => /多個位置/.test(item.reason))
  );
  await db.close();
});
...
test("definitions 缺某版本表單匯出檔 → definitionsMissing 標記、查詢不炸", async () => {
  const { db, service } = await buildService({
    readForm: async (formPath: string) => {
      if (formPath === "default/forms999/9003") {
  ...
  const form901 = result.siblings.find((s) => s.formPath === "default/demo/work-orders/line-a");
  assert.equal(form901?.definitionsMissing, false);
  await db.close();
});
...
test("欄位存在但沒有既有公式且 live 也無公式 → freshness fresh，可由 apply 新增公式", async () => {
  ...
  await mkdir(root, { recursive: true });
  await writeFile(root + "/x.nui", "D,3,3,9001106,規格,text=1\n", "utf-8");
  const { db, service } = await buildService({
    builderRoot: root,
  ...
  try {
    ...
    const form901 = result.siblings.find((s) => s.formPath === "default/demo/work-orders/line-a");
    assert.equal(form901?.hasField, true);
    assert.equal(form901?.currentNuiFormula, null);
    assert.equal(form901?.freshness.checked, true);
    assert.equal(form901?.freshness.fresh, true);
    assert.equal(form901?.freshness.baselinePosition, "C3");
    assert.equal(form901?.freshness.actualPosition, "C3");
    assert.deepEqual(form901?.freshness.staleReasons, []);
  } finally {
    await db.close();
    await rm(root, { recursive: true, force: true });
  }
...
test("live .nui 欄位位置與 definitions 不一致時標記 stale，避免跨版本推估吃舊 baseline", async () => {
  const root = await mkdtemp(join(tmpdir(), "ragic-siblings-live-"));
  await mkdir(root, { recursive: true });
  await writeFile(root + "/x.nui", "D,4,14,9001106,規格,f=C3+D2\n", "utf-8");
  const { db, service } = await buildService({
    builderRoot: root,
    readForm: async (formPath: string) =>
      formDetail(formPath, [
  ...
  try {
    ...
    const form901 = result.siblings.find((s) => s.formPath === "default/demo/work-orders/line-a");
    assert.equal(form901?.freshness.checked, true);
    assert.equal(form901?.freshness.fresh, false);
    assert.equal(form901?.freshness.baselinePosition, "C3");
    assert.equal(form901?.freshness.actualPosition, "D14");
    assert.ok(
      form901?.freshness.staleReasons.some((reason) =>
...
test("freshness baseline position 使用 definitions，不被已刷新 field-index 覆蓋", async () => {
  ...
  try {
    await repo.replaceAll(
      ENTRIES.map((entry) =>
        entry.formPath === "default/demo/work-orders/line-a" && entry.fieldId === "9001106"
          ? { ...entry, fieldPos: "D14" }
          : entry
      ),
...
test("freshness 使用 definitions sourceLine，不 fallback 掃描同 fieldId", async () => {
  ...
  try {
    ...
    assert.equal(form901?.freshness.checked, true);
    assert.equal(form901?.freshness.fresh, false);
    assert.equal(form901?.freshness.actualPosition, "I9");
    assert.ok(
      form901?.freshness.staleReasons.some((reason) =>
...
test("includeFreshness=false 只回 definitions baseline，不讀 live .nui", async () => {
  ...
  const form901 = result.siblings.find((s) => s.formPath === "default/demo/work-orders/line-a");
  assert.equal(form901?.freshness.checked, false);
  assert.equal(form901?.freshness.fresh, true);
  assert.equal(form901?.freshness.baselinePosition, "C3");
  assert.deepEqual(form901?.freshness.warnings, []);
  await db.close();
});
...
test("live .nui 公式含 redaction 形狀時不可判定 fresh，避免遮蔽後 false fresh", async () => {
  ...
  await writeFile(
    root + "/x.nui",
    "D,3,3,9001106,規格,f=sk-1234567890123456\n",
    "utf-8"
  );
  const { db, service } = await buildService({
    builderRoot: root,
  ...
  try {
    ...
    assert.equal(form901?.freshness.checked, true);
    assert.equal(form901?.freshness.fresh, false);
    assert.equal(form901?.freshness.actualFormula, "sk-***REDACTED***");
    assert.ok(
      form901?.freshness.staleReasons.some((reason) =>
        /已遮蔽敏感片段/.test(reason)
      )
...
test("live .nui UTF-8 非 ASCII 公式含中文與智慧引號時不誤判 stale", async () => {
  ...
  await writeFile(
    root + "/x.nui",
    `D,3,3,9001106,規格,f=${encodeURIComponent(sourceFormula)}\n`,
    "utf-8"
  );
  const { db, service } = await buildService({
    builderRoot: root,
...
test("live .nui cache 會依 mtime/size 失效，不會在 Builder 寫入後讀舊公式", async () => {
  ...
  await mkdir(root, { recursive: true });
  await writeFile(nuiPath, "D,3,3,9001106,規格,f=C3+D2\n", "utf-8");
  const { db, service } = await buildService({
    builderRoot: root,
    readForm: async (formPath: string) =>
      formDetail(formPath, [
  ...
  try {
    ...
    const firstForm901 = first.siblings.find((s) => s.formPath === "default/demo/work-orders/line-a");
    assert.equal(firstForm901?.freshness.fresh, true);

    await writeFile(nuiPath, "D,3,3,9001106,規格,f=C3+D9\n", "utf-8");
    await utimes(
      nuiPath,
    ...
    const secondForm901 = second.siblings.find((s) => s.formPath === "default/demo/work-orders/line-a");
    assert.equal(secondForm901?.freshness.fresh, false);
    assert.equal(secondForm901?.freshness.actualFormula, "C3+D9");
  } finally {
    await db.close();
    await rm(root, { recursive: true, force: true });
  }
