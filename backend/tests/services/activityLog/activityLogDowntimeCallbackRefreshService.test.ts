...
test("activity log callback queue 關閉 admission 後拒絕新任務且不留下 pending work", () => {
  const registry = createRegistry();
  const service = new ActivityLogDowntimeCallbackRefreshService({
    delayMs: 0,
...
test("activity log callback drain 會等待已接受的 refresh 完成", async () => {
...
test("activity log callback recovery 不受一般 task list 的 200 筆上限影響", async () => {
  const recoveredTasks = Array.from({ length: 201 }, (_, index) => {
    const entryId = String(1_600_000_002 + index);
    return {
      taskId: `activityLog-callback-before-restart-${index}`,
  ...
  assert.equal(
    recoveredTasks.every(
      (task) =>
        ...
        "ACTIVITY_LOG_CALLBACK_REPLAY_SCHEDULED"
    ),
    true
  );
