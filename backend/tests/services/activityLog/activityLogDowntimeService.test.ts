...
import { activityLogDowntimeSqliteRepository } from "../../../src/storage/sqlite/activityLogDowntimeSqliteRepository";
import { buildActivityLogDowntimeRecordSnapshotHash } from "../../../src/storage/sqlite/activityLogDowntimeSqliteRepository";
import { activityLogClientRowKeyRepository } from "../../../src/storage/sqlite/activityLogClientRowKeyRepository";
import { activityLogPlannedIdleSqliteRepository } from "../../../src/storage/sqlite/activityLogPlannedIdleSqliteRepository";
import {
...
test("停機紀錄 snapshot hash 相同時允許 mutation", async (t) => {
  ...
  await activityLogDowntimeService.assertRecordSnapshotUnchanged("123", "same-hash");
...
test("停機紀錄 snapshot hash 不同時擋下 stale mutation", async (t) => {
  ...
  await assert.rejects(
    () => activityLogDowntimeService.assertRecordSnapshotUnchanged("123", "old-hash"),
    (error: unknown) => {
...
test("停機紀錄沒有 expected snapshot hash 時維持舊流程", async (t) => {
  ...
  await activityLogDowntimeService.assertRecordSnapshotUnchanged("123", null);

  assert.equal(getHashMock.mock.callCount(), 0);
});

test("同一 activity log entry 的背景 projection 依排入順序執行，較舊讀取不會最後覆蓋", async (t) => {
  ...
  const service = activityLogDowntimeService as unknown as {
  ...
  t.mock.method(
    activityLogDowntimeSqliteRepository,
    "upsertRecord",
    async (record: ActivityLogDowntimeRecord) => {
    projectedRemarks.push(record.remark ?? "");
    }
  );

  const first = activityLogDowntimeService.refreshEntrySnapshotFromRagic("123490");
  await new Promise((resolve) => setImmediate(resolve));
  const second = activityLogDowntimeService.refreshEntrySnapshotFromRagic("123490");
  await new Promise((resolve) => setImmediate(resolve));
...
test("停機紀錄 create payload 必須直寫 demo_report_type（Ragic 必填驗證先於公式，缺了會被 202 拒絕）", async (t) => {
  const service = activityLogDowntimeService as unknown as {
  ...
  t.mock.method(service, "resolveActivityLogRequiredFields", async () => ({
    depUnit: "P01加工一組",
    prodType: "PA",
    source: "test",
  }));
  ...
  t.mock.method(ragicClient, "getEntry", async () => ({
    ...
    [env.UPSTREAM_ACTIVITY_LOG_TYPE_FIELD_ID]: "PROC-A",
  }));
  t.mock.method(ragicClient, "executeActionButton", async () => ({}));
  t.mock.method(ragicClient, "clearFormCache", () => {});

  const result = await activityLogDowntimeService.createRecord({
    date: "2026/07/06",
    machineId: "A1",
    processCode: "A03",
  });
  ...
  assert.equal(
    (createPayload as Record<string, unknown>)[env.UPSTREAM_ACTIVITY_LOG_TYPE_FIELD_ID],
    "PROC-A"
  );
  assert.equal(
    (createPayload as Record<string, unknown>)[env.UPSTREAM_ACTIVITY_LOG_DEP_FIELD_ID],
    "P01加工一組"
  );
  assert.equal(
    (createPayload as Record<string, unknown>)[env.UPSTREAM_ACTIVITY_LOG_PROD_TYPE_FIELD_ID],
    "PA"
  );
...
test("停機紀錄 confirmed durable mapping 會在 duplicate guard 前回收 persisted 與 legacy fingerprint", async (t) => {
  ...
  const resolverMock = t.mock.method(service, "resolveActivityLogRequiredFields", async () => {
  ...
  t.mock.method(activityLogDowntimeSqliteRepository, "listRecords", async () =>
  ...
  const reserveMock = t.mock.method(activityLogClientRowKeyRepository, "reservePending", async () => {
  ...
  const persisted = await activityLogDowntimeService.createRecord({
    date: "2026/07/20",
    machineId: "MB50",
    processCode: "A01",
    clientRowKey: "confirmed-persisted",
  });
  const legacy = await activityLogDowntimeService.createRecord({
    date: "2026/07/20",
    machineId: "MB50",
    processCode: "A01",
...
test("停機紀錄 update 變更 processCode 時必須同步寫回 demo_report_type與 lookup 欄位", async (t) => {
  const service = activityLogDowntimeService as unknown as {
  ...
  t.mock.method(ragicClient, "getEntry", async () => ({
    [FORM_901_CONFIG.writeConfig.subtableWriteFields.date]: "2026/07/20",
    [FORM_901_CONFIG.writeConfig.subtableWriteFields.machineId]: "MB50",
    [FORM_901_CONFIG.writeConfig.subtableWriteFields.processCode]: "A03",
  ...
  const result = await activityLogDowntimeService.updateRecord("990001", {
    processCode: "A03",
  });
  ...
  assert.equal(
    (updatePayload as Record<string, unknown>)[env.UPSTREAM_ACTIVITY_LOG_TYPE_FIELD_ID],
    "PROC-A"
  );
  assert.equal(
    (updatePayload as Record<string, unknown>)[env.UPSTREAM_ACTIVITY_LOG_DEP_FIELD_ID],
    "P01加工一組"
  );
  assert.equal(
    (updatePayload as Record<string, unknown>)[env.UPSTREAM_ACTIVITY_LOG_PROD_TYPE_FIELD_ID],
    "PA"
  );
  assert.equal(bumpRevisionMock.mock.callCount(), 1);
});
...
test("停機紀錄 update 日期或機台時排除自己，不會把原紀錄判成重複", async (t) => {
  ...
  const currentRow = {
    ...
    [writeFields.machineId]: "MB50",
    [writeFields.processCode]: "A01",
    [writeFields.plannedIdle]: "Yes",
  };
  t.mock.method(service, "assertRecordSnapshotUnchanged", async () => {});
  t.mock.method(service, "refreshEntrySnapshotFromRagic", async () => {});
  t.mock.method(activityLogPlannedIdleSqliteRepository, "bumpProjectionRevision", async () => {});
...
test("停機紀錄 update 的部分 patch 會用現況補齊日期，並擋下其他 entry 的同日同機台", async (t) => {
  ...
  await assert.rejects(
    () => activityLogDowntimeService.updateRecord("990001", { machineId: "MA51" }),
    (error: unknown) => {
      assert.ok(error instanceof HttpError);
      assert.equal(error.statusCode, 409);
  ...
  assert.ok(duplicateWhere.includes("9001030,eq,2026/07/20"));
...
test("停機紀錄同一個舊 snapshot hash 只能成功使用一次", async (t) => {
  ...
  const buildCurrentRow = () => ({
    ...
    [writeFields.processCode]: "A01",
    [writeFields.plannedIdle]: "Yes",
    [env.UPSTREAM_ACTIVITY_LOG_REMARK_FIELD_ID]: currentRemark,
  });
  const oldSnapshotHash = buildActivityLogDowntimeRecordSnapshotHash({
    id: "990001",
    snapshotHash: null,
    date: "2026/07/20",
    machineId: "MB50",
  ...
  await activityLogDowntimeService.updateRecord(
    "990001",
    { remark: "after" },
  ...
  await assert.rejects(
    () =>
      activityLogDowntimeService.updateRecord(
        "990001",
        { remark: "second" },
        { expectedSnapshotHash: oldSnapshotHash }
      ),
...
test("停機紀錄 delete 成功後會 bump 計畫停機 projection revision", async (t) => {
  ...
  const result = await activityLogDowntimeService.deleteRecord("990001");
...
test("計畫停機 refresh=1 立即回舊 aggregate，背景 singleflight commit 後可讀到新快照", async (t) => {
  ...
  t.mock.method(activityLogPlannedIdleSqliteRepository, "getState", async () => ({
  ...
  t.mock.method(activityLogPlannedIdleSqliteRepository, "getRefreshBarrier", async () => ({
    fullRevision: 1,
    projectionRevision: 1,
    monthRevision: 1,
  }));
  t.mock.method(activityLogPlannedIdleSqliteRepository, "getMonthSyncedAt", async () =>
  ...
  t.mock.method(activityLogPlannedIdleSqliteRepository, "aggregateByMonth", async () =>
    committed
      ? [{ machineId: "MB50", prodType: "PA", totalMinutes: 480, count: 1 }]
      : [{ machineId: "MB50", prodType: "PA", totalMinutes: 60, count: 1 }]
  );

  const first = activityLogDowntimeService.summarizePlannedIdleByMachine("2026/07", true);
  const second = activityLogDowntimeService.summarizePlannedIdleByMachine("2026/07", true);
  ...
  const committedResult = await activityLogDowntimeService.summarizePlannedIdleByMachine(
    "2026/07",
    false
  );
...
test("計畫停機首次即時撈取若被較新 revision 取代，回傳已提交的 SQLite aggregate", async (t) => {
  ...
  t.mock.method(activityLogPlannedIdleSqliteRepository, "getState", async () => null);
  t.mock.method(activityLogPlannedIdleSqliteRepository, "getRefreshBarrier", async () => ({
    fullRevision: 1,
  ...
  t.mock.method(activityLogPlannedIdleSqliteRepository, "replaceMonth", async () =>
    "stale" as const
  );
  t.mock.method(activityLogPlannedIdleSqliteRepository, "aggregateByMonth", async () => [
    { machineId: "P20", prodType: "PA", totalMinutes: 480, count: 1 },
  ]);
  t.mock.method(activityLogPlannedIdleSqliteRepository, "getMonthSyncedAt", async () =>
    "2026-07-20T00:30:00.000Z"
  );

  const result = await activityLogDowntimeService.summarizePlannedIdleByMachine("2026/07", false);
...
test("停機全量同步遇到 stale revision 會重抓並只在 applied 後完成", async (t) => {
  ...
  let revision = 10;
  t.mock.method(activityLogDowntimeSqliteRepository, "getSnapshotState", async () => ({
  ...
  await activityLogDowntimeService.refreshSqliteSnapshotFromRagic();
...
test("背景停機全量同步在報工寫入等待時會讓位，且重抓後才套用 snapshot", async (t) => {
  ...
  const refreshPromise = activityLogDowntimeService.refreshSqliteSnapshotFromRagic({
...
test("停機全量同步連續 stale revision 會回 typed conflict，不會假報成功", async (t) => {
  ...
  t.mock.method(activityLogDowntimeSqliteRepository, "syncSnapshot", async () => "stale" as const);

  await assert.rejects(
    () => activityLogDowntimeService.refreshSqliteSnapshotFromRagic(),
    (error: unknown) => {
      assert.ok(error instanceof HttpError);
      assert.equal(error.code, "ACTIVITY_LOG_DOWNTIME_SNAPSHOT_CONFLICT");
      return true;
    }
  );
});
...
test("半年計畫停機同步遇到 stale revision 會重抓並只在 applied 後完成", async (t) => {
  ...
  const fetchMock = t.mock.method(service, "fetchPlannedIdleRowsFromRagic", async () => []);
  t.mock.method(activityLogPlannedIdleSqliteRepository, "getProjectionRevision", async () => 3);
  ...
  const result = await activityLogDowntimeService.syncPlannedIdleHalfYear();
...
for (const scenario of ["unrelated", "same-field", "already-intended"] as const) {
  test(`ActivityLog sparse 備註更新 ${scenario} 保護原值且只寫入備註`, async (t) => {
    const fields = FORM_901_CONFIG.writeConfig.subtableWriteFields;
    t.mock.method(ragicClient, "getEntry", async () => ({
      [fields.date]: "2026/09/16", [fields.machineId]: "MA51", [fields.processCode]: "A01",
      [fields.plannedIdle]: "Yes", [env.UPSTREAM_ACTIVITY_LOG_REMARK_FIELD_ID]: scenario === "unrelated" ? "before" : scenario === "same-field" ? "third" : "after",
    }));
    const refresh = activityLogDowntimeService as unknown as { refreshEntrySnapshotFromRagic: (id: string) => Promise<void> };
    t.mock.method(refresh, "refreshEntrySnapshotFromRagic", async () => {});
    t.mock.method(activityLogPlannedIdleSqliteRepository, "bumpProjectionRevision", async () => {});
    const write = t.mock.method(ragicClient, "updateEntry", async (_path: string, _id: string, payload: Record<string, unknown>) => {
      assert.deepEqual(payload, { [env.UPSTREAM_ACTIVITY_LOG_REMARK_FIELD_ID]: "after" }, "SPARSE_WRITE_ONLY_INTENT"); return {};
    });
    const run = () => activityLogDowntimeService.updateRecord("990001", { remark: "after" }, {
      fieldPreconditionVersion: 1, expectedValues: { remark: "before" }, expectedSnapshotHash: "unrelated-old-hash",
    });
    if (scenario === "same-field") {
      await assert.rejects(run, (error: unknown) => error instanceof HttpError && error.code === "DOWNTIME_RECORD_STALE", "ACTIVITY_LOG_SAME_FIELD_CONFLICT");
    } else await run();
    assert.equal(write.mock.callCount(), scenario === "unrelated" ? 1 : 0);
  });
}

test("ActivityLog 新欄位契約缺原值不得退回 legacy hash", async (t) => {
  const fields = FORM_901_CONFIG.writeConfig.subtableWriteFields;
  t.mock.method(ragicClient, "getEntry", async () => ({ [fields.date]: "2026/09/16", [fields.machineId]: "MB50", [fields.plannedIdle]: "Yes" }));
  const write = t.mock.method(ragicClient, "updateEntry", async () => ({}));
  await assert.rejects(() => activityLogDowntimeService.updateRecord("990001", { remark: "new" }, { fieldPreconditionVersion: 1, expectedValues: {} }),
