...
async function waitForCondition(
  ...
  while (!condition()) {
    if (Date.now() >= deadline) {
      throw new Error(`等待逾時：${description}`);
    }
...
async function createHarness(options: {
  ...
  let sequence = 0;
  const service = new ActivityLogEfficiencyReportArchiveService({
    repository,
...
test("同月份並發操作共用一次 Ragic 來源抓取", async () => {
  ...
  try {
    ...
    await waitForCondition(() => fetchCount === 1, "Ragic 來源抓取開始");
    assert.equal(fetchCount, 1);

    assert.ok(fetchGate.release);
    fetchGate.release();
...
test("缺失的歷史分析檔若 template version 不同則 fail closed", async () => {
  ...
  let replacementService: ActivityLogEfficiencyReportArchiveService | null = null;
  try {
    ...
    let buildCount = 0;
    replacementService = new ActivityLogEfficiencyReportArchiveService({
