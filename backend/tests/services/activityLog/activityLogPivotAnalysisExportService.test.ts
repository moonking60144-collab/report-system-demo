...
} from "../../../src/services/activityLog/activityLogPivotAnalysisExportService";
...
test("real template 的 table、autoFilter 與 worksheet dimension 跟實際資料列同步", async (t) => {
  ...
  assert.equal(ACTIVITY_LOG_PIVOT_CALCULATION_VERSION, "v2");
  ...
  for (const { name, dataRows } of [
    ...
    await t.test(name, async () => {
      ...
      const workbook = await activityLogPivotAnalysisExportService.buildWorkbook(csvText, templateBody);
