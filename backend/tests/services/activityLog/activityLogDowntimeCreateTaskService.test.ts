...
import {
  ACTIVITY_LOG_DOWNTIME_CREATE_QUEUE_KEY,
  ActivityLogDowntimeCreateTaskService,
  type ActivityLogDowntimeCreateTaskPayload,
} from "../../../src/services/activityLog/activityLogDowntimeCreateTaskService";
import {
...
function payload(clientRowKey: string): ActivityLogDowntimeCreateTaskPayload {
  return {
    date: "2026-07-06",
    machineId: "MB50",
    processCode: "A03",
...
test("停機 create task 會 pending -> running -> success 並同步 registry entryId", async () => {
  ...
  const service = new ActivityLogDowntimeCreateTaskService({
    registry,
  ...
  assert.equal(successTask.queueKey, ACTIVITY_LOG_DOWNTIME_CREATE_QUEUE_KEY);
...
test("真實 registry 先 recovery 時仍保留 downtime running create 的結果不明契約", async (t) => {
  ...
  workReportTaskRegistryService.upsertTask({
    ...
    queueKey: "901:E-REGISTRY-FIRST-BATCH",
  });
  workReportTaskRegistryService.upsertTask({
