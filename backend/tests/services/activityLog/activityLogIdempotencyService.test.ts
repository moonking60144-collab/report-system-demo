...
} from "../../../src/storage/sqlite/activityLogClientRowKeyRepository";
...
function buildRecord(
  overrides: Partial<ActivityLogClientRowKeyRecord> = {}
): ActivityLogClientRowKeyRecord {
  return {
...
function mockReservePending(
  t: TestContext,
  result: Awaited<ReturnType<typeof activityLogClientRowKeyRepository.reservePending>>
) {
  return t.mock.method(activityLogClientRowKeyRepository, "reservePending", async () => result);
}
...
test("clientRowKey 為空字串 → 直接 create、不 reserve、不 confirm", async (t) => {
  ...
  const confirmMock = t.mock.method(activityLogClientRowKeyRepository, "confirm", async () => 1);
  const createMock = t.mock.fn(async () => ({ entryId: "E-NEW" }));

  const result = await checkOrCreateActivityLogEntry({
    clientRowKey: "",
    source: "downtime",
    create: createMock,
  });
...
test("clientRowKey null / undefined 都當空 key 處理", async (t) => {
  ...
  await checkOrCreateActivityLogEntry({
    clientRowKey: null,
    source: "downtime",
    create: createMock,
  });
...
test("部署前映射缺 fingerprint 仍不可跨 source 重用", async (t) => {
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof HttpError && error.code === "ACTIVITY_LOG_IDEMPOTENCY_KEY_CONFLICT"
  );
  assert.equal(createMock.mock.callCount(), 0);
});
...
test("部署前 pending 映射缺 fingerprint 時仍回寫入不可確認、不重新 create", async (t) => {
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof HttpError && error.code === "ACTIVITY_LOG_WRITE_INDETERMINATE"
  );
  assert.equal(createMock.mock.callCount(), 0);
});
...
test("相同 clientRowKey 但 operation fingerprint 不同時拒絕重用 activity log 映射", async (t) => {
  mockReservePending(t, {
    record: buildRecord({
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      ...
      error.code === "ACTIVITY_LOG_IDEMPOTENCY_KEY_CONFLICT"
  );
...
test("Ragic 已建立但 confirm 不再擁有 reservation 時回不可確認", async (t) => {
  ...
  t.mock.method(activityLogClientRowKeyRepository, "confirm", async () => 0);
...
test("create 拋出 RAGIC_WRITE_FAILED 5xx → mark indeterminate，後續同 key 不重送", async (t) => {
  let reserveCalls = 0;
  t.mock.method(activityLogClientRowKeyRepository, "reservePending", async () => {
    reserveCalls += 1;
    if (reserveCalls === 1) {
      return {
        record: buildRecord({
...
test("create 拋出 RAGIC_ACTION_BUTTON_INDETERMINATE → mark indeterminate，後續同 key 不重送", async (t) => {
  ...
  const createMock = t.mock.fn(async () => {
    throw new HttpError(
      502,
      "activity log action button 48 失敗，且 rollback delete 未確認",
    ...
    await assert.rejects(
      () =>
        checkOrCreateActivityLogEntry({
          ...
          source: "downtime",
          create: async () => {
            throw new HttpError(502, `activity log 寫入結果不可確認：${code}`, code);
          },
        }),
      new RegExp(code)
    );
...
test("同 key 並發（TOCTOU）→ 只 create 一次，後到者共享結果且 reused=true", async (t) => {
  ...
  const first = checkOrCreateActivityLogEntry({
    clientRowKey: "uuid-race",
    source: "downtime",
    create: createMock,
  });
  const second = checkOrCreateActivityLogEntry({
    clientRowKey: "uuid-race",
    source: "downtime",
    create: createMock,
  });
...
test("並發時先到者 create 拋一般錯誤 → 等待者收到同一錯誤，之後 retry 會重新 create", async (t) => {
  ...
  t.mock.method(activityLogClientRowKeyRepository, "confirm", async () => 1);
  t.mock.method(activityLogClientRowKeyRepository, "releasePending", async () => 1);
  let createCalls = 0;
  const createMock = t.mock.fn(async () => {
    createCalls += 1;
    if (createCalls === 1) {
  ...
  const retried = await checkOrCreateActivityLogEntry({
    clientRowKey: "uuid-race-fail",
    source: "downtime",
    create: createMock,
  });
