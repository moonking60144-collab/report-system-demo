...
} from "../../../src/services/activityLog/activityLogPublishedCsv";
...
test("效率報表來源 hash 不受 CSV 資料列順序影響", () => {
  const first = inspectActivityLogPublishedCsv(
    csv([
      { id: "A", value: "10" },
      { id: "B", value: "20" },
    ]),
  ...
  const second = inspectActivityLogPublishedCsv(
    csv([
      { id: "B", value: "20" },
      { id: "A", value: "10" },
    ]),
...
test("效率報表來源 hash 維持既有 JSON canonicalization 相容性", () => {
  ...
  assert.equal(inspectActivityLogPublishedCsv(source, "2026-06", 100).sourceHash, expected);
});

test("效率報表來源內容變更會產生不同 hash", () => {
  const first = inspectActivityLogPublishedCsv(csv([{ id: "A", value: "10" }]), "2026-06", 100);
  const second = inspectActivityLogPublishedCsv(csv([{ id: "A", value: "11" }]), "2026-06", 100);
...
test("效率報表來源新增欄位的內容變更也會建立不同 hash", () => {
  ...
  const first = inspectActivityLogPublishedCsv(withExtraA, "2026-06", 100);
  const second = inspectActivityLogPublishedCsv(withExtraB, "2026-06", 100);
...
test("效率報表拒絕歸屬到其他月份的 CSV", () => {
  assert.throws(
    () => inspectActivityLogPublishedCsv(csv([{ id: "A", date: "2026/05/31" }]), "2026-06", 100),
    (error: unknown) =>
      typeof error === "object" &&
      error !== null &&
      "code" in error &&
...
test("效率報表拒絕不存在的日期", () => {
  ...
  assert.throws(
    () => inspectActivityLogPublishedCsv(invalidCsv, "2026-06", 100),
    (error: unknown) =>
      typeof error === "object" &&
      error !== null &&
      "code" in error &&
