...
import {
  ...
  buildMutationCommandReadOptions,
...
test("mutation command current 使用 mutation timeout 與有限 read retry", () => {
  assert.deepEqual(buildMutationCommandReadOptions("current"), {
    priority: "mutation",
    timeoutMs: env.RAGIC_MUTATION_READ_TIMEOUT_MS,
    totalBudgetMs: env.RAGIC_MUTATION_READ_TOTAL_BUDGET_MS,
...
test("mutation command verify 只有一個 bounded read attempt", () => {
  assert.deepEqual(buildMutationCommandReadOptions("verify"), {
    priority: "mutation",
    timeoutMs: env.RAGIC_MUTATION_VERIFY_TIMEOUT_MS,
    totalBudgetMs: env.RAGIC_MUTATION_VERIFY_TIMEOUT_MS,
    maxRetries: 0,
  });
});
