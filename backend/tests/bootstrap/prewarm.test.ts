...
test("options prewarm 901/902 串行執行，中間套用 between forms delay", async (t) => {
  stopFormOptionsPrewarm();
  ...
  assert.equal(finished[0]?.formId, "901");
  assert.equal(started[1]?.formId, "902");
  assert.ok(
    (started[1]?.at ?? 0) - (finished[0]?.at ?? 0) >= 15,
    "902 應在 901 完成後再等待 configured delay 才開始"
  );
});
