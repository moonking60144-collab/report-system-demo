...
test("queued waiter 依 priority 放行，且相同 priority 維持 FIFO", async () => {
  const bucket = new TokenBucket({ refillPerSecond: 50, capacity: 1 });
  await bucket.acquire();

  const settled: string[] = [];
  const low = bucket.acquire({ priority: 2 }).then(() => settled.push("low"));
  const highFirst = bucket.acquire({ priority: 0 }).then(() => settled.push("high-1"));
  const highSecond = bucket.acquire({ priority: 0 }).then(() => settled.push("high-2"));
  const normal = bucket.acquire({ priority: 1 }).then(() => settled.push("normal"));

  await Promise.all([low, highFirst, highSecond, normal]);
  assert.deepEqual(settled, ["high-1", "high-2", "normal", "low"]);
});
