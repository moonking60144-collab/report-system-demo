...
test("runRead 成功與失敗都回報本機 lane wait、upstream 與 outcome", async () => {
  const scheduler = new RagicRequestScheduler({
    globalRatePerSecond: 100,
    globalBurstCapacity: 2,
  });
  const successTimings: Array<{
  ...
  await scheduler.runRead(
    ...
    async () => "ok",
    "mutation",
    { onTiming: (timing) => successTimings.push(timing) }
  );
  assert.equal(successTimings.length, 1);
  assert.equal(successTimings[0]?.requestStarted, true);
  assert.equal(successTimings[0]?.outcome, "success");
  assert.ok((successTimings[0]?.laneWaitMs ?? -1) >= 0);
  assert.ok((successTimings[0]?.upstreamMs ?? -1) >= 0);
  assert.ok(
    (successTimings[0]?.totalMs ?? -1) >=
      (successTimings[0]?.laneWaitMs ?? 0) + (successTimings[0]?.upstreamMs ?? 0)
  );

  const failureTimings: typeof successTimings = [];
  await assert.rejects(
    () =>
      scheduler.runRead(
        "timed-failure",
        ...
        "mutation",
        { onTiming: (timing) => failureTimings.push(timing) }
      ),
    /timed boom/
  );
  assert.equal(failureTimings.length, 1);
  assert.equal(failureTimings[0]?.requestStarted, true);
  assert.equal(failureTimings[0]?.outcome, "failure");
  assert.ok((failureTimings[0]?.upstreamMs ?? -1) >= 0);
});

test("runWrite 也回報 lane wait 與 upstream timing", async () => {
  const scheduler = new RagicRequestScheduler({
    globalRatePerSecond: 100,
    globalBurstCapacity: 2,
  });
  ...
  assert.equal(observed?.requestStarted, true);
  assert.ok((observed?.laneWaitMs ?? -1) >= 0);
  assert.ok((observed?.upstreamMs ?? -1) >= 0);
});

test("runRead admission timeout 回報 requestStarted=false 且不執行 upstream callback", async () => {
  const scheduler = new RagicRequestScheduler({
    globalRatePerSecond: 0.0001,
    globalBurstCapacity: 1,
  });
  await scheduler.runRead("consume-global-token", async () => undefined);
  ...
  await assert.rejects(
    scheduler.runRead(
      ...
      "background",
      {
        overallTimeoutMs: 1,
        onTiming: (timing) => {
          requestStarted = timing.requestStarted;
        },
      }
    ),
    (error: unknown) => error instanceof TokenBucketAcquireTimeoutError
  );
  assert.equal(upstreamRan, false);
...
test("readiness circuit state 會在 cooldown 後開放既有 half-open probe", async () => {
  ...
  try {
    const scheduler = new RagicRequestScheduler();
    await assert.rejects(
      () => scheduler.runWrite("open-write-breaker", async () => {
        throw networkError("ECONNRESET");
      }),
      /ECONNRESET/
    );
    assert.equal(scheduler.getStats().writeCircuitState, "open");

    await new Promise((resolve) => setTimeout(resolve, 5));
    assert.equal(
      scheduler.getReadinessCircuitStates().writeCircuitState,
      "half-open"
    );
  } finally {
...
test("global token 排隊時 mutation 會先於已排隊的 sync 放行", async () => {
  const scheduler = new RagicRequestScheduler({
    globalRatePerSecond: 20,
    globalBurstCapacity: 1,
  });
  await scheduler.runRead("consume-initial-token", async () => undefined, "user");

  const started: string[] = [];
  const syncTask = scheduler.runRead(
    "queued-sync",
    async () => {
      started.push("sync");
    },
  ...
  const mutationTask = scheduler.runRead(
    "queued-mutation",
    async () => {
  ...
  await Promise.all([syncTask, mutationTask]);
  assert.deepEqual(started, ["mutation", "sync"]);
});

test("read overall budget 會限制 global token queue，不沿用一般 queue timeout", async () => {
  const scheduler = new RagicRequestScheduler({
    globalRatePerSecond: 1,
    globalBurstCapacity: 1,
  });
  await scheduler.runRead("consume-initial-token", async () => undefined);
