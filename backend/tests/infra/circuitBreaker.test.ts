...
test("readiness admission 只在 cooldown 到期後把 OPEN 推進到 HALF_OPEN", () => {
  let now = 0;
  const breaker = buildBreaker({ failureThreshold: 1, cooldownMs: 1000, now: () => now });
  breaker.recordFailure();

  now = 999;
  assert.equal(breaker.getAdmissionState(), "open");
  now = 1000;
  assert.equal(breaker.getAdmissionState(), "half-open");
});
