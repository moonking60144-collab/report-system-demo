...
test("ragicClient.clearFormCache 會清掉 work report options cache", async (t) => {
  ...
  const sourceFormPath = getFormConfig("901").linkedFields?.machineId?.sourceFormPath ?? "";
  let callCount = 0;
  ...
  t.mock.method(ragicClient, "getFormDataWithOptions", async () => {
    ...
    return {
      "1": {
        _ragicId: "1",
        "demo_machine_code": "M1",
        "demo_machine_name": callCount === 1 ? "舊機台" : "新機台",
      },
    };
  });

  const first = await service.getFormOptions("901", ["machineId"], "user");
  assert.equal(first.machineId?.[0]?.display, "舊機台");

  const cached = await service.getFormOptions("901", ["machineId"], "user");
  assert.equal(cached.machineId?.[0]?.display, "舊機台");
  assert.equal(callCount, 1);

  ragicClient.clearFormCache(sourceFormPath);

  const refreshed = await service.getFormOptions("901", ["machineId"], "user");
...
test("getFormOptions 同 form/fields 但不同 priority 不共用 in-flight", async (t) => {
  ...
  const backgroundRead = service.getFormOptions("901", ["machineId"], "background");
  const userRead = service.getFormOptions("901", ["machineId"], "user");
  ...
  gates[0].resolve({
    "1": { _ragicId: "1", "demo_machine_code": "M1", "demo_machine_name": "背景機台" },
  });
  gates[1].resolve({
    "1": { _ragicId: "1", "demo_machine_code": "M1", "demo_machine_name": "使用者機台" },
  });
...
test("ragicClient.clearFormCache 會清掉 operator option cache", async (t) => {
  ...
  const first = await getOperatorOptionMap("901", config);
  assert.equal(first.get("OP1"), "舊人員");

  const cached = await getOperatorOptionMap("901", config);
  assert.equal(cached.get("OP1"), "舊人員");
  assert.equal(callCount, 1);

  ragicClient.clearFormCache(sourceFormPath);

  const refreshed = await getOperatorOptionMap("901", config);
