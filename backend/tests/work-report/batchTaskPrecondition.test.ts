...
import { env, resolveWorkReportDataPath } from "../../src/config/env";
...
test("批次新增 worker 前置檢查失敗時不建立任何列", async () => {
  ...
  const task = workReportBatchCreateTaskService.requestBatchCreate({
    ...
    rows: Array.from({ length: 6 }, () => ({ payload: { machineId: "MB50" } })),
    beforeRun: async () => {
  ...
  assert.equal(finishedTask.batchRequestedCount, 6);
  assert.equal(finishedTask.batchCreatedCount, 0);
  assert.equal(finishedTask.batchFailedCount, 0);
...
test("批次新增透過共用 mutation queue 等待 sync", async () => {
  ...
  const rowKeyRepository: BatchCreateRowKeyRepository = {
    ...
    deleteByReservationIdentity: async () => 0,
  ...
  try {
    task = workReportBatchCreateTaskService.requestBatchCreate({
      formId,
      entryId,
      rows: [{ payload: { machineId: "MB50" }, clientRowKey }],
      rowKeyRepository,
      createRow: async () => {
        createRowCalls += 1;
        return { rowId: "1001" };
  ...
  assert.equal(finishedTask.batchRequestedCount, 1);
  assert.equal(finishedTask.batchCreatedCount, 1);
  assert.equal(finishedTask.batchFailedCount, 0);
  assert.equal(createRowCalls, 1);
});

test("部分成功的收尾重試等待 sync，完成後不提供資料列進度", async () => {
  await workReportTaskRegistryService.initialize();
  const formId = "901";
  ...
  const sourceTask = workReportBatchCreateTaskService.requestBatchCreate({
    ...
    entryId,
    rows: [
      { payload: { machineId: "MB50" }, clientRowKey },
      { payload: { machineId: "MA51" }, clientRowKey: `${clientRowKey}-2` },
    ],
    rowKeyRepository,
    createRow: async () => {
      createCalls += 1;
      if (createCalls === 2) throw new Error("row creation failed");
      return { rowId: "1001" };
    },
  ...
  assert.equal(failedSourceTask.batchRequestedCount, 2);
  assert.equal(failedSourceTask.batchCreatedCount, 1);
  assert.equal(failedSourceTask.batchFailedCount, 1);
  ...
  assert.equal(finishedRetryTask.batchRequestedCount, null);
  assert.equal(finishedRetryTask.batchCreatedCount, 1);
  assert.equal(finishedRetryTask.batchFailedCount, 0);
...
test("獨立 modal 新增與編輯都透過共用 mutation queue 等待 sync", async () => {
  ...
  try {
    createTask = createReportTaskService.enqueue({
      ...
      queueKey: `901:modal-create-${suffix}`,
      worker: async () => {
        createWorkerCalls += 1;
        return { rowId: "2001" };
      },
    ...
    updateTask = createReportTaskService.enqueue({
      ...
      queueKey: `902:modal-update-${suffix}`,
      worker: async () => {
        updateWorkerCalls += 1;
        return { rowId: "2002" };
      },
...
test("批次新增狀態前置檢查會擋已結案工令", async (t) => {
  ...
  await assert.rejects(
    () => workReportService.assertBatchCreateEntryAcceptsReports("901", "closed-entry"),
    (error: unknown) =>
  ...
  assert.deepEqual(readMock.mock.calls[0]?.arguments, [
    resolveWorkReportDataPath("901", config.ragicPath),
    "closed-entry",
    false,
    {
      priority: "mutation",
...
test("批次新增狀態前置檢查允許未結案工令", async (t) => {
  ...
  const result = await workReportService.assertBatchCreateEntryAcceptsReports("901", "open-entry");
  assert.equal(readMock.mock.callCount(), 1);
...
test("批次新增狀態前置檢查讀取失敗時會 fail closed", async (t) => {
  ...
  await assert.rejects(
    () => workReportService.assertBatchCreateEntryAcceptsReports("901", "unknown-entry"),
    (error: unknown) =>
      error instanceof HttpError &&
      error.statusCode === 409 &&
...
test("批次新增狀態不可確認時 worker 不建立任何列", async (t) => {
  ...
  const task = workReportBatchCreateTaskService.requestBatchCreate({
    ...
    rows: [{ payload: { machineId: "MB50" } }],
    beforeRun: async () => {
      await workReportService.assertBatchCreateEntryAcceptsReports("901", "unknown-entry");
    },
    createRow: async () => {
      createRowCalls += 1;
      return { rowId: "1001" };
...
test("刪除 worker 使用 mutation live snapshot 並在成功刪除後回傳 audit", async () => {
  const order: string[] = [];
  ...
  const task = workReportBatchDeleteTaskService.requestBatchDelete({
    ...
    deleteRow: async (rowId) => {
      ...
      order.push("snapshot");
      order.push("delete");
      return { rowId, beforeSnapshot: snapshot };
    },
