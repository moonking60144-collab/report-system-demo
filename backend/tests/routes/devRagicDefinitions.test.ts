...
async function buildFixture() {
  ...
  await writeFile(
    ...
    "var entry = param.getUpdatedEntry();\nvar targetFieldId = 9001108;\n",
    "utf-8"
  );
  const nuiLines = Array.from({ length: 26 }, (_, index) => `# filler ${index + 1}`);
  nuiLines[12] = "D,1,2,9001105,編號,text=1";
  nuiLines[23] = "D,7,6,9001108,測試,text=1&f=F6*D6+123456";
  await writeFile(
    nuiFilePath,
    `${nuiLines.join("\n")}\n`,
    "utf-8"
...
function knowledgeGovernanceService(
  submitCandidate: DevAiKnowledgeGovernanceService["submitCandidate"]
): DevAiKnowledgeGovernanceService {
  const unused = async (): Promise<never> => {
    throw new Error("unused knowledge governance method");
  };
  return {
    submitCandidate,
    listCandidates: unused,
    getCandidate: unused,
    updateCandidate: unused,
    approveCandidate: unused,
    rejectCandidate: unused,
  };
}
...
async function withTestServer(
  ...
  overrides: {
    ...
    devAiKnowledgeGovernanceService?: DevAiKnowledgeGovernanceService;
    devAiKnowledgeCompilerService?: DevAiKnowledgeCompilerService;
    devAiKnowledgeBaseService?: DevAiKnowledgeBaseService;
    reExport?: {
  ...
  app.use(
    ...
    createDevRagicDefinitionsRouter({
      ...
      devAiKnowledgeGovernanceService: overrides.devAiKnowledgeGovernanceService,
      devAiKnowledgeCompilerService: overrides.devAiKnowledgeCompilerService,
      devAiKnowledgeBaseService: overrides.devAiKnowledgeBaseService,
...
test("POST ai/formula/suggest 會把公式 occurrence 完整傳入 suggestion service", async () => {
  const fixture = await buildFixture();
  let received: Parameters<RagicFormulaAiSuggestionService["suggestFormula"]>[0] | null = null;
  const formulaAiSuggestionService: RagicFormulaAiSuggestionService = {
    async suggestFormula(request) {
      received = request;
      return {} as Awaited<ReturnType<RagicFormulaAiSuggestionService["suggestFormula"]>>;
    },
  };
  try {
    await withTestServer(
    ...
    await withTestServer(
      ...
      async (baseUrl) => {
        const res = await fetch(`${baseUrl}/api/dev/ragic-definitions/ai/chat`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ question: "DemoCo 是什麼？" }),
        });
        assert.equal(res.status, 401);
        assert.equal(called, false);
      },
  ...
  try {
    await withTestServer(
      ...
      async (baseUrl) => {
        ...
        assert.equal(receivedRequest?.fieldId, "9001108");
      },
      { devAiChatService }
    );
  } finally {
...
test("POST ai/feedback 無 token 會先被 Dev auth 擋下", async () => {
  const fixture = await buildFixture();
  let called = false;
  const devAiKnowledgeGovernanceService = knowledgeGovernanceService(
    async () => {
      called = true;
      throw new Error("should not be called");
    }
...
test("POST ai/feedback 只建立 pending candidate 並帶 dev actor", async () => {
  const fixture = await buildFixture();
  let seenActor: string | null | undefined;
  const devAiKnowledgeGovernanceService = knowledgeGovernanceService(
    async (request, options) => {
      assert.equal(request.payload.kind, "chat-answer");
      assert.equal(request.payload.question, "這個範例系統是什麼？");
      assert.equal(request.payload.answer, "依公開示範資料回答。");
  ...
  try {
    await withTestServer(
      ...
      async (baseUrl) => {
        ...
        assert.equal(body.data.candidate.id, "candidate-route");
...
test("GET ai/knowledge/runtime-status 回傳唯讀檢索狀態", async () => {
  const fixture = await buildFixture();
  let called = false;
  const devAiKnowledgeBaseService: DevAiKnowledgeBaseService = {
    async search() {
    ...
    await withTestServer(
      ...
      async (baseUrl) => {
        const res = await fetch(
          `${baseUrl}/api/dev/ragic-definitions/ai/knowledge/runtime-status`,
          { headers: { Authorization: `Bearer ${VALID_TOKEN}` } }
        );
        assert.equal(res.status, 200);
        const body = await res.json();
        assert.equal(body.data.retrievalMode, "lexical");
        assert.equal(body.data.index.state, "not-required");
        assert.equal(called, true);
      },
      ...
      async (baseUrl) => {
        ...
        await writeFile(
          ...
          Array.from({ length: 26 }, (_, index) =>
            index === 23
              ? "D,7,6,9001108,測試,text=1&f=F6*D6%2B654321"
              : index === 12
                ? "D,1,2,9001105,編號,text=1"
                : `# filler ${index + 1}`
          ).join("\n"),
          "utf-8"
        );
...
test("GET search 可用 fieldId 找到欄位、公式、欄位設定與 workflow 引用", async () => {
  ...
  try {
    await withTestServer(fixture, async (baseUrl) => {
      const res = await fetch(
        `${baseUrl}/api/dev/ragic-definitions/search?fieldId=9001108`,
        { headers: { Authorization: `Bearer ${VALID_TOKEN}` } }
      );
      assert.equal(res.status, 200);
      const body = await res.json();
      ...
      async (baseUrl) => {
        ...
        const responsePromise = fetch(
          `${baseUrl}/api/dev/ragic-definitions/formula/siblings?formPath=${encodeURIComponent(
            "default/devtest/51"
          )}&fieldId=9001108&formulaKind=formula&includeFreshness=true`,
          {
            headers: { Authorization: `Bearer ${VALID_TOKEN}` },
...
test("POST formula/dry-run 不會丟掉 position 與 sourceLine occurrence", async () => {
  const fixture = await buildFixture();
  try {
    await withTestServer(fixture, async (baseUrl) => {
      const res = await fetch(`${baseUrl}/api/dev/ragic-definitions/formula/dry-run`, {
      ...
      assert.equal(body.data.position, "G6");
      assert.equal(body.data.sourceLine, 999);
      assert.ok(
        body.data.blockers.some((blocker: string) =>
          blocker.includes("RAGIC_FORMULA_OCCURRENCE_STALE")
        ),
        "ROUTE_MUST_PRESERVE_FORMULA_OCCURRENCE"
      );
    });
  } finally {
    await rm(fixture.root, { recursive: true, force: true });
...
test("POST formula/dry-run 舊 .nui 若用 %2B，新 preview 正規化為 raw +", async () => {
  ...
  try {
    await writeFile(
      ...
      Array.from({ length: 26 }, (_, index) =>
        index === 23
          ? "D,7,6,9001108,測試,text=1&f=F6*D6%2B123456"
          : `# filler ${index + 1}`
      ).join("\n"),
      "utf-8"
    );
...
test("POST formula/dry-run 欄位存在但沒有既有公式時可新增 f= attr", async () => {
  ...
  try {
    await withTestServer(fixture, async (baseUrl) => {
      ...
      assert.match(body.data.oldLinePreview, /D,1,2,9001105,編號,text=1/);
      assert.match(body.data.newLinePreview, /D,1,2,9001105,編號,text=1&f=G6/);
    });
  } finally {
    await rm(fixture.root, { recursive: true, force: true });
  }
...
test("POST formula/dry-run 實際 .nui 與 baseline 公式不同時禁止", async () => {
  ...
  try {
    await writeFile(
      ...
      Array.from({ length: 26 }, (_, index) =>
        index === 23
          ? "D,7,6,9001108,測試,text=1&f=F6*D6%2B999999"
          : `# filler ${index + 1}`
      ).join("\n"),
      "utf-8"
    );
...
test("POST formula/apply 欄位存在但沒有既有公式時新增 f= attr 並 re-export 驗證", async () => {
  ...
  try {
    await withTestServer(fixture, async (baseUrl) => {
      ...
      assert.equal(body.data.verifiedFormula.fieldId, "9001105");
      assert.equal(body.data.verifiedFormula.nuiFormula, "G6");

      const nui = await readFile(fixture.nuiFilePath, "utf-8");
      assert.match(nui, /D,1,2,9001105,編號,text=1&f=G6/);
      const formulas = JSON.parse(
        await readFile(join(fixture.formDir, "formulas.json"), "utf-8")
      );
      assert.ok(
        formulas.some(
          (formula: { fieldId: string; nuiFormula: string }) =>
            formula.fieldId === "9001105" && formula.nuiFormula === "G6"
        )
      );
    });
  } finally {
