...
import activityLogEfficiencyReportsRouter from "../../src/routes/activityLogEfficiencyReports";
import { errorHandler } from "../../src/middleware/errorHandler";
import { activityLogEfficiencyReportArchiveService } from "../../src/services/activityLog/activityLogEfficiencyReportArchiveService";
...
test("GET /api/downtime/efficiency-reports 回分頁歷史版本", async (t) => {
  t.mock.method(
    activityLogEfficiencyReportArchiveService,
    "listSnapshots",
...
test("既有 monthly-csv endpoint 改由 archive service 回傳檔案", async (t) => {
  ...
  t.mock.method(activityLogEfficiencyReportArchiveService, "getOrCreateCurrentCsv", async () => ({
    snapshot: snapshot(),
    filePath,
...
test("analysis-xlsx endpoint 驗證出勤天數後交給 archive service", async (t) => {
  let callCount = 0;
  t.mock.method(activityLogEfficiencyReportArchiveService, "getOrCreateCurrentAnalysis", async () => {
    callCount += 1;
