...
test("GET search 過濾 by q", async () => {
  ...
  await withTestServer(env, async (baseUrl) => {
    const res = await fetch(`${baseUrl}/api/dev/ragic-fields/search?q=901`, {
      headers: { Authorization: `Bearer ${VALID_TOKEN}` },
    });
    assert.equal(res.status, 200);
    const body = await res.json();
    assert.equal(body.data.length, 1);
    assert.equal(body.data[0].fieldId, "9001040");
  });
});
...
test("GET search 用 fieldId 精確比對", async () => {
  ...
  await withTestServer(env, async (baseUrl) => {
    const res = await fetch(
      `${baseUrl}/api/dev/ragic-fields/search?fieldId=9001040`,
      { headers: { Authorization: `Bearer ${VALID_TOKEN}` } }
    );
    const body = await res.json();
    assert.equal(body.data.length, 1);
