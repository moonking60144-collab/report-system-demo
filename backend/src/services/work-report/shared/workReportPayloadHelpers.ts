...
export function buildActivityLogFallbackWritePayload(
  normalizedPayload: ReportWritePayload,
  workOrderNo: string,
  reportType: string,
  depUnit: string,
  ...
  const fieldEntries: Array<[string, unknown]> = [
    ...
    [env.UPSTREAM_ACTIVITY_LOG_REMARK_FIELD_ID, normalizedPayload.remark],
    [env.UPSTREAM_ACTIVITY_LOG_WORK_ORDER_FIELD_ID, workOrderNo],
    [env.UPSTREAM_ACTIVITY_LOG_TYPE_FIELD_ID, reportType],
    [env.UPSTREAM_ACTIVITY_LOG_PROCESS_FIELD_ID, normalizedPayload.processCode],
    [env.UPSTREAM_ACTIVITY_LOG_DEP_FIELD_ID, depUnit],
    [env.UPSTREAM_ACTIVITY_LOG_PROD_TYPE_FIELD_ID, prodType],
  ];
