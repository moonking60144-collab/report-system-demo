...
export async function runCreateRecalculateFlow(
  ...
  if (!recalculateCompleted) {
    const fallbackPayload = buildActivityLogFallbackWritePayload(
      normalizedPayload,
      workOrderNo,
      resolvedReportType,
    ...
    try {
      await measureStage("fallback", () =>
        simulateActivityLogRowSave(activityLogWritePath, rowId, fallbackPayload)
      );
    ...
    if (
      ...
      if (computedTotalWorkTime !== null) {
        ...
        try {
          await measureStage("fallback", () =>
            writeComputedTotalWorkTime(activityLogWritePath, rowId, totalWorkTimeFieldId, computedTotalWorkTime)
          );
