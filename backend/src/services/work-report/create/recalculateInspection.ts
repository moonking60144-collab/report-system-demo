...
export function evaluatePostCreateRecalculateNeed(
  ...
  const shiftTypeCandidates = [config.subtableFields.shiftType, "9001006"].filter(
    (candidate): candidate is string => Boolean(candidate)
  );
  ...
  const totalWorkTimeCandidates = [
    ...
    "9001034",
    "demo_total_work_time",
    "Total Work Time總工時(Hr)",
  ].filter((candidate): candidate is string => Boolean(candidate));
  const normalWorkHourCandidates = ["9001059", "[實際]時間-正常班(Hr)", "[實際]時間-正常班 (Hr)"];
