...
import {
  createRagicLiveReadMeta,
  resolveSqliteReadMeta,
} from "./readModelState";
...
export class WorkReportReportsReadService {
  ...
  private shouldUseStructuredReportQuery(options: ReportQueryOptions): boolean {
    return Boolean(
      options.status ||
        ...
        options.keyword ||
        Object.keys(options.columnFilters ?? {}).length > 0 ||
        (options.filterGroup?.conditions.length ?? 0) > 0 ||
  ...
  private async getStructuredReports(
    ...
    return {
      ...this.support.filterSortAndPaginateReports(fullResult.data, options),
      meta: {
  ...
  private async tryGetReportsFromSqlite(
    ...
    try {
      ...
      const result = await workReportSqliteRepository.getReports(formId, {
        limit: options.limit,
        offset: options.offset,
    ...
    try {
      ...
      return {
        ...
        meta: {
          ...
          ...resolveSqliteReadMeta(syncState),
          expiresAt: null,
  ...
  private async tryGetStructuredReportsFromSqlite(
    ...
    try {
      ...
      const result = await workReportSqliteRepository.getReports(formId, {
        ...
        excludeTestCustomerPart: options.excludeTestCustomerPart,
        excludeSortOrder99: options.excludeSortOrder99,
        entryId: options.entryId,
