import { env, resolveWorkReportDataPath } from "../../config/env";
...
export class WorkReportOptionsReadService {
  ...
  async getRawPreview(formId: string, limit: number): Promise<RagicRow[]> {
    ...
    const rawData = await ragicClient.getFormData(
      resolveWorkReportDataPath(formId, config.ragicPath)
    );
  ...
  private buildMachineDefault(
    ...
    const machineCode = this.pickFirstNonEmptyText(row.data["demo_machine_code"]);
    if (!machineCode) {
      return undefined;
    }

    const processCategoryCode = this.pickFirstNonEmptyText(row.data["demo_process_category"]);
    const processCategoryName =
      this.pickFirstNonEmptyText(row.data["demo_process_name"]) ??
      this.pickFirstNonEmptyText(row.data["製程類別名稱"]);
    const processCode =
      this.parseProcessCode(row.data["demo_process_code"]) ??
      this.parseProcessCode(row.data["9001038"]);
    const mainOperatorId = this.pickFirstNonEmptyText(row.data["demo_primary_operator_id"]);
    const mainOperatorName = this.pickFirstNonEmptyText(row.data["demo_primary_operator_name"]);
    const machineSpec = this.pickFirstNonEmptyText(row.data["機台規格說明"]);
    const machineSpeed = this.pickFirstNonEmptyText(row.data["機台分速"]);
    const status = this.pickFirstNonEmptyText(row.data["demo_status"]);
  ...
  private buildOperatorGroup(
    ...
    const label =
      this.pickFirstNonEmptyText(row.data["demo_department_group"]) ??
