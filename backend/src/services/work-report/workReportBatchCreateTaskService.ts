...
const log = createLogger("work-report-batch-create");
...
interface RequestBatchCreateInput {
  ...
  createRow: (payload: Record<string, unknown>, reservation: BatchCreateRowKeyRecord) => Promise<{ rowId: string }>;
...
export async function createRowWithIdempotency(input: {
  ...
  if (!reserveResult.reserved) {
    ...
    log.warn({
      event: "row-key.reserve-conflict",
      key: clientRowKey,
      existing: reserved,
      requested: { formId, entryId },
    });
  ...
  try {
    result = await createRow(row.payload, reserved);
  } catch (error) {
    if (shouldMarkCreateResultIndeterminate(error)) {
      ...
      await rowKeyRepo
        ...
        .catch((markError) => {
          log.warn({
            event: "row-key.mark-indeterminate-failed",
            key: clientRowKey,
            formId,
            entryId,
      ...
      await rowKeyRepo.releasePending({ clientRowKey, formId, entryId, reservationToken: reserved.reservationToken }).catch((releaseError) => {
        log.warn({
          event: "row-key.release-pending-failed",
          key: clientRowKey,
          formId,
          entryId,
...
class WorkReportBatchCreateTaskService {
  ...
  private syncTaskToRegistry(task: WorkReportBatchCreateTask): void {
    ...
    workReportTaskRegistryService.upsertTask({
      ...
      // Finalize-only retries do not process rows; the message retains the original totals.
      batchRequestedCount: task.retryMode === "finalize-only" ? null : task.requestedCount,
      batchCreatedCount: task.createdCount,
      batchFailedCount: rowCreateFailedItems.length,
