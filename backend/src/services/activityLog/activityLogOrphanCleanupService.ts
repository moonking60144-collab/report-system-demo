...
} from "./activityLogOrphanPolicy";

const log = createLogger("activityLog-orphan-cleanup-service");

/**
 * activity log 孤兒背景清理 service。
 *
 * 判定條件 / createdAt 解析都走 `activityLogOrphanPolicy` 共用模組（跟 audit 腳本共用一份），
...
export interface ActivityLogOrphanCleanupOptions {
  /** 掃最近幾天內的 entry */
  daysBack: number;
...
export interface ActivityLogOrphanCleanupStats {
  fetched: number;
  /** 符合全部判定條件的筆數（含太新的） */
  matched: number;
...
export async function runActivityLogOrphanCleanupOnce(
  options: ActivityLogOrphanCleanupOptions
): Promise<ActivityLogOrphanCleanupStats> {
  const activityLogPath =
    resolveWritePath("903", env.UPSTREAM_ACTIVITY_LOG_PATH) ?? env.UPSTREAM_ACTIVITY_LOG_PATH;
  if (!activityLogPath.trim()) {
    throw new Error("activity log path 未設定，無法執行 orphan cleanup");
  }
  ...
  const stats: ActivityLogOrphanCleanupStats = {
    fetched: 0,
    matched: 0,
    skippedTooRecent: 0,
    deleted: 0,
  ...
  while (toDelete.length < options.maxPerRun) {
    const page = await ragicClient.getFormPage(
      activityLogPath,
      {
        limit: pageSize,
        offset,
        where: [`${env.UPSTREAM_ACTIVITY_LOG_DATE_FIELD_ID},>=,${dateFrom}`],
      },
      false,
    ...
    for (const row of rows) {
      if (!isActivityLogOrphan(row.data, { creatorAccount: options.creatorAccount })) {
      ...
      const createdAtMs = parseActivityLogCreatedAtMs(row.data);
