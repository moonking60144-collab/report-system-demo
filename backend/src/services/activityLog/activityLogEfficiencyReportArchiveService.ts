...
import {
  ACTIVITY_LOG_PIVOT_CALCULATION_VERSION,
  activityLogPivotAnalysisExportService,
  type ActivityLogPivotTemplateBundle,
} from "./activityLogPivotAnalysisExportService";
import {
  inspectActivityLogPublishedCsv,
  efficiencyReportRetentionCutoffMonth,
  previousCompleteTaipeiMonth,
} from "./activityLogPublishedCsv";
...
interface ActivityLogEfficiencyReportArchiveServiceDeps {
  repository?: EfficiencyReportArchiveRepository;
  archiveDir?: string;
  fetchSource?: () => Promise<Buffer>;
  loadTemplate?: () => Promise<ActivityLogPivotTemplateBundle>;
  buildWorkbook?: (
    csvText: string,
    templateBody: Buffer,
...
export class ActivityLogEfficiencyReportArchiveService {
  private readonly repository: EfficiencyReportArchiveRepository;
  private readonly archiveDir: string;
  private readonly fetchSource: () => Promise<Buffer>;
  private readonly loadTemplate: () => Promise<ActivityLogPivotTemplateBundle>;
  ...
  constructor(deps: ActivityLogEfficiencyReportArchiveServiceDeps = {}) {
    ...
    this.loadTemplate =
      deps.loadTemplate ?? (() => activityLogPivotAnalysisExportService.loadTemplateBundle());
    this.buildWorkbook =
      deps.buildWorkbook ??
      ((csvText, templateBody, attendanceDays) =>
        activityLogPivotAnalysisExportService.buildWorkbook(csvText, templateBody, attendanceDays));
  ...
  private async createOrReuseSource(
    ...
    const inspection = inspectActivityLogPublishedCsv(
  ...
  private async createArtifact(
    ...
    template: ActivityLogPivotTemplateBundle,
    ...
    const artifact: EfficiencyReportArtifactRecord = {
      ...
      calculationVersion: ACTIVITY_LOG_PIVOT_CALCULATION_VERSION,
      reportFingerprint,
      xlsxRelativePath,
  ...
  private assertArtifactRebuildCompatible(
    ...
    if (
      ...
      artifact.calculationVersion !== ACTIVITY_LOG_PIVOT_CALCULATION_VERSION ||
...
export const activityLogEfficiencyReportArchiveService = new ActivityLogEfficiencyReportArchiveService();
