...
export const ACTIVITY_LOG_WORK_ORDER_REQUIRED_TYPES: readonly string[] = [
  "PROC-A",
  "PROC-B",
  "PROC-C",
  "PROC-D",
...
const WORK_ORDER_REQUIRED_TYPES_SET = new Set(ACTIVITY_LOG_WORK_ORDER_REQUIRED_TYPES);

export interface ActivityLogOrphanPolicyOptions {
 ...
 */
export function isActivityLogOrphan(
  record: RagicRecord,
  options: ActivityLogOrphanPolicyOptions
): boolean {
  const workOrderNo = pickRagicField(record, ACTIVITY_LOG_FIELD_KEYS.workOrderNo);
  if (workOrderNo !== "") return false;

  const plannedDowntime = pickRagicField(record, ACTIVITY_LOG_FIELD_KEYS.plannedDowntime);
  if (plannedDowntime === "Yes") return false;

  const reportType = pickRagicField(record, ACTIVITY_LOG_FIELD_KEYS.reportType);
  if (!WORK_ORDER_REQUIRED_TYPES_SET.has(reportType)) return false;

  const creator = pickRagicField(record, ACTIVITY_LOG_FIELD_KEYS.creatorAccount);
  if (!options.creatorAccount || creator !== options.creatorAccount) return false;

  const erpPartNo = pickRagicField(record, ACTIVITY_LOG_FIELD_KEYS.erpPartNo).toLowerCase();
 ...
 */
export function parseActivityLogCreatedAtMs(record: RagicRecord): number | null {
  const raw = pickRagicField(record, ACTIVITY_LOG_FIELD_KEYS.createdAt);
  if (!raw) return null;
