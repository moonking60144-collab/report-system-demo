...
export const ACTIVITY_LOG_PUBLISHED_COLUMN_COUNT = 65;
export const ACTIVITY_LOG_PUBLISHED_DATE_COLUMN_INDEX = 5;

export function parseActivityLogPublishedCsv(text: string): string[][] {
  const rows: string[][] = [];
  let row: string[] = [];
  let field = "";
  let inQuotes = false;
...
export interface InspectedActivityLogPublishedCsv {
  sourceHash: string;
  rowCount: number;
  rows: string[][];
}

export function inspectActivityLogPublishedCsv(
  csvText: string,
  expectedPeriodMonth: string,
  maxRows: number
): InspectedActivityLogPublishedCsv {
  const rows = parseActivityLogPublishedCsv(csvText).filter((row) =>
  ...
  if (header.length < ACTIVITY_LOG_PUBLISHED_COLUMN_COUNT) {
    throw new HttpError(
      502,
      `Ragic 發佈 CSV 欄位數為 ${header.length}，預期 ${ACTIVITY_LOG_PUBLISHED_COLUMN_COUNT} 欄。`,
      "PUBLISHED_CSV_HEADER_INVALID"
    );
  }

  const normalizedRows = dataRows.map((row, rowIndex) => {
    if (row.length < ACTIVITY_LOG_PUBLISHED_COLUMN_COUNT) {
      throw new HttpError(
        502,
        `Ragic 發佈 CSV 第 ${rowIndex + 2} 列只有 ${row.length} 欄，預期至少 ${ACTIVITY_LOG_PUBLISHED_COLUMN_COUNT} 欄。`,
        "PUBLISHED_CSV_ROW_INVALID"
      );
    }
    const normalized = row.map((value) => value.trim());
    const actualMonth = normalizeDateMonth(normalized[ACTIVITY_LOG_PUBLISHED_DATE_COLUMN_INDEX] ?? "");
