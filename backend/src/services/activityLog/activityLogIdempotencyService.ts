...
import {
  ...
  type ActivityLogClientRowKeyRecord,
} from "../../storage/sqlite/activityLogClientRowKeyRepository";

/**
 * activity log 寫入的 idempotency 包裝器。
 *
 * 使用情境：使用者按送出 → network timeout / UI 卡住 → 使用者又按一次。
 * 每次 click 如果 frontend 重用同一個 clientRowKey，這支 service 就能把多次 call
 * 收斂成「第一次寫 Ragic、之後都回同一筆 entryId」，避免 activity log 出現重複 entry。
 *
...
const inflightByKey = new Map<
  string,
  {
    ...
    promise: Promise<ActivityLogIdempotencyResult>;
  }
>();
...
export interface ActivityLogIdempotencyCreateResult {
  entryId: string | null;
}

export interface ActivityLogIdempotencyInput {
  /** 前端產的 UUID，同一次送出流程重用同一個 */
  clientRowKey: string | null | undefined;
  /** 來源標記，用於 debug / 稽核，例如 "downtime"、"work-report-901" */
  source: string;
  /** operation、target 與 canonical payload 的穩定 fingerprint。 */
  operationFingerprint?: string;
  /** 真正打 Ragic 寫入 + 後續處理的 callback；回 entryId 或 null */
  create: (reservation?: ActivityLogClientRowKeyRecord) => Promise<ActivityLogIdempotencyCreateResult>;
}

export interface ActivityLogIdempotencyResult {
  entryId: string | null;
...
function getHttpErrorUpstreamStatus(error: HttpError): number | undefined {
  ...
  const status = upstreamDetail?.status ?? upstreamDetail?.httpStatus;
...
export function isActivityLogCreateWriteIndeterminateError(error: unknown): boolean {
  const code = getErrorCode(error);
  const message = getErrorMessage(error);

  if (code === "ACTIVITY_LOG_WRITE_INDETERMINATE") {
    return true;
  }
...
export const isActivityLogMutationWriteIndeterminateError =
  isActivityLogCreateWriteIndeterminateError;

function throwIndeterminateActivityLogWrite(
  status: "pending" | "indeterminate",
  errorMessage?: string
): never {
  const message =
    status === "pending"
      ? "activity log 寫入仍在處理中，已暫停同 clientRowKey 重送，請稍後重新整理確認結果。"
      : `activity log 寫入結果尚未確認，已暫停同 clientRowKey 重送，請先確認是否已建立。${
          errorMessage ? ` 原因：${errorMessage}` : ""
        }`;
  throw new HttpError(409, message, "ACTIVITY_LOG_WRITE_INDETERMINATE");
}
...
function assertSameIdempotencyIdentity(input: {
  ...
  if (
    ...
    throw new HttpError(
      409,
      "activity log idempotency key 已用於不同的來源、目標或 payload，請重新整理後再送出。",
      "ACTIVITY_LOG_IDEMPOTENCY_KEY_CONFLICT"
    );
  }
}

export async function checkOrCreateActivityLogEntry(
  input: ActivityLogIdempotencyInput
): Promise<ActivityLogIdempotencyResult> {
  ...
  const task = (async (): Promise<ActivityLogIdempotencyResult> => {
    // 1. 先 reserve pending。這一步要早於 Ragic create，才能擋 backend 重啟或
    // Cloudflare/timeout 造成的「寫入可能成功但回應未知」重送。
    const reserveResult = await activityLogClientRowKeyRepository.reservePending({
      clientRowKey: key,
      source: input.source,
    ...
    if (!reserveResult.reserved) {
      ...
      if (existing?.status === "pending" || existing?.status === "indeterminate") {
        throwIndeterminateActivityLogWrite(existing.status, existing.errorMessage);
      }
      throw new HttpError(
        409,
        "activity log 寫入 idempotency key 已被占用，但查不到可重用的 entryId，請重新整理後再試。",
        "ACTIVITY_LOG_WRITE_INDETERMINATE"
      );
    }

    // 2. 沒映射就真的打 Ragic（create 內部可能再失敗）
    let result: ActivityLogIdempotencyCreateResult;
    try {
      result = await input.create(reserveResult.record ?? undefined);
    } catch (error) {
      if (isActivityLogCreateWriteIndeterminateError(error)) {
        await activityLogClientRowKeyRepository.markIndeterminate({
          clientRowKey: key,
          source: input.source,
          errorMessage: getErrorMessage(error),
        });
      } else {
        await activityLogClientRowKeyRepository.releasePending({
          clientRowKey: key,
          source: input.source,
        });
      }
    ...
    if (result.entryId) {
      const confirmedCount = await activityLogClientRowKeyRepository.confirm({
        clientRowKey: key,
      ...
      if (confirmedCount !== 1) {
        throw new HttpError(
          409,
          `activity log 已建立 entry ${result.entryId}，但 idempotency reservation 已變更；請先重新整理確認結果。`,
          "ACTIVITY_LOG_WRITE_INDETERMINATE"
        );
      }
    } else {
