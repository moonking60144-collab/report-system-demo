...
import { runWorkReportEntryProjection } from "./workReportEntryProjectionQueue";
...
export class WorkReportMutationProjectionService {
  ...
  async applyQueuedSortOrderAfterMutation(
    ...
    return runWorkReportEntryProjection(formId, entryId, () =>
      this.applyQueuedSortOrderNow(formId, entryId, sortOrder, enqueuedSeq)
    );
  }

  private async applyQueuedSortOrderNow(
    formId: string,
    entryId: string,
    sortOrder: number,
    enqueuedSeq: number
  ...
  async applyQueuedProjectionAfterMutation(
    ...
    return runWorkReportEntryProjection(formId, entryId, (waitedForSync) =>
      this.applyQueuedProjectionNow(formId, entryId, reason, enqueuedSeq,
        waitedForSync ? undefined : observedEntry)
    );
  }

  private async applyQueuedProjectionNow(
    formId: string,
    entryId: string,
    reason: ProjectionReason,
    enqueuedSeq: number,
    ...
    try {
      const record = await this.deps.refreshEntry(formId, entryId, observedEntry);
      const snapshotAt = new Date().toISOString();
