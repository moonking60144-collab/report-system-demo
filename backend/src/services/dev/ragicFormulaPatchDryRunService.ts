...
import {
  RagicFormulaOccurrenceError,
  resolveRagicFormulaOccurrence,
} from "./ragicFormulaOccurrence";
...
function extractReferencedPositions(
  formula: string,
  knownPositions: ReadonlySet<string>
): string[] {
  const refs = new Set<string>();
  for (const token of tokenizeFormula(formula)) {
    if (!token.isCellRef) continue;
    const position = token.text.toUpperCase();
    if (knownPositions.has(position)) refs.add(position);
  }
  return [...refs];
}
...
function detectFormulaCycle(params: {
  ...
  for (const field of params.detail.fields) {
    knownPositions.add(field.position.toUpperCase());
  }

  for (const formula of params.detail.formulas) {
    knownPositions.add(formula.position.toUpperCase());
  }

  const graph = new Map<string, Set<string>>();
  for (const formula of params.detail.formulas) {
    const formulaPosition = formula.position.toUpperCase();
    if (
      formulaPosition === params.targetPosition.toUpperCase() &&
      formula.formulaKind === params.targetFormulaKind
    ) {
      continue;
    }
    const expression = formula.nuiFormula;
    const refs = extractReferencedPositions(expression, knownPositions);
    if (refs.length === 0) continue;
    const edges = graph.get(formulaPosition) ?? new Set<string>();
    for (const ref of refs) edges.add(ref);
    graph.set(formulaPosition, edges);
  }

  const targetPosition = params.targetPosition.toUpperCase();
  const targetRefs = extractReferencedPositions(params.newFormula, knownPositions);
  graph.set(targetPosition, new Set(targetRefs));
  ...
  const findCycle = (
    ...
    for (const nextPosition of graph.get(currentPosition) ?? []) {
      if (nextPosition === targetPosition) {
        return [...path, nextPosition];
      }
      if (visiting.has(nextPosition)) continue;
      visiting.add(nextPosition);
      const cycle = findCycle(nextPosition, [...path, nextPosition], visiting);
      visiting.delete(nextPosition);
      if (cycle) return cycle;
    }
    return null;
  };

  const cycle = findCycle(targetPosition, [targetPosition], new Set([targetPosition]));
  if (!cycle) return null;
  return `公式會造成循環參照：${cycle.join(" -> ")}，請先調整公式依賴後再試算`;
}
...
export function createRagicFormulaPatchDryRunService(
  ...
  async function dryRunFormulaPatch(
    ...
    const hasPosition = Boolean(normalizedInput.position?.trim());
    const sourceLineProvided = normalizedInput.sourceLine !== undefined;
    const hasSourceLine =
      typeof normalizedInput.sourceLine === "number" &&
      Number.isInteger(normalizedInput.sourceLine) &&
      normalizedInput.sourceLine > 0;
    if (sourceLineProvided && !hasSourceLine) {
      result.blockers.push("RAGIC_FORMULA_OCCURRENCE_LOCATOR_INVALID：sourceLine 必須是正整數");
      return result;
    }
    if (hasPosition !== sourceLineProvided) {
      result.blockers.push(
        "RAGIC_FORMULA_OCCURRENCE_LOCATOR_INCOMPLETE：position 與 sourceLine 必須一起提供"
      );
      return result;
    }
    ...
    let occurrence;
    try {
      occurrence = resolveRagicFormulaOccurrence(detail, normalizedInput);
    } catch (error) {
      if (!(error instanceof RagicFormulaOccurrenceError)) throw error;
      result.blockers.push(`${error.code}：${error.message}`);
      return result;
    }
    if (!occurrence) {
    ...
    const { formula, field } = occurrence;

    result.fieldName = formula?.fieldName ?? field?.fieldName ?? null;
    result.position = occurrence.position;
    result.sourceLine = occurrence.sourceLine;
    result.oldFormula = formula?.nuiFormula ?? null;

    const cycleBlocker = detectFormulaCycle({
      detail,
      targetPosition: occurrence.position,
    ...
    if (result.position && parsedLine.position !== result.position) {
      result.blockers.push(
        `RAGIC_FORMULA_OCCURRENCE_STALE：sourceLine position 不一致：baseline=${result.position}，actual=${parsedLine.position}`
      );
    }
