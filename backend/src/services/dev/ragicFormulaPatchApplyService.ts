...
import { resolveRagicFormulaFromList } from "./ragicFormulaOccurrence";
import type {
...
async function readFormulaFromFilesystem(
  ...
  return resolveRagicFormulaFromList(formulas, {
    fieldId,
    formulaKind,
    position,
    sourceLine,
...
async function patchNuiFormulaBytes(params: {
  ...
  if (parsedLine.position !== params.expectedPosition) {
    throw new Error(
      `Ragic 現況已不同步，sourceLine position 不一致：expected=${params.expectedPosition}，actual=${parsedLine.position}`
    );
  }
...
export function createRagicFormulaPatchApplyService(
  ...
  async function applyFormulaPatch(
    ...
    if (!dryRun.builderFilePath || !dryRun.position || !dryRun.sourceLine) {
    ...
    const backupFilePath = join(
      backupRoot,
      `${uniqueFileStamp()}__${safeFilePart(input.formPath)}__${safeFilePart(input.fieldId)}__${safeFilePart(dryRun.position)}__L${dryRun.sourceLine}.nui`
    );
    const position = dryRun.position;
  ...
  async function applyFormulaPatchBatch(
    ...
    const writable = results.map(
      (target) =>
        ...
        Boolean(target.dryRun.position) &&
    ...
    try {
      await runWithDefinitionsWriteLock(async () => {
        ...
        for (let index = 0; index < inputs.length; index += 1) {
          ...
          const backupFilePath = join(
            backupRoot,
            `${stamp}__batch${index}__${safeFilePart(input.formPath)}__${safeFilePart(input.fieldId)}__${safeFilePart(target.dryRun.position as string)}__L${target.dryRun.sourceLine}.nui`
          );
          await mkdir(dirname(backupFilePath), { recursive: true });
          await copyFile(builderFilePath, backupFilePath);
          ...
          await patchNuiFormulaBytes({
            ...
            formulaKind: input.formulaKind,
            expectedFieldId: input.fieldId,
            expectedPosition: target.dryRun.position as string,
        ...
        for (let index = 0; index < inputs.length; index += 1) {
          ...
          const verifiedFormula =
            (await readFormulaFromFilesystem(
              ...
              input.fieldId,
              input.formulaKind,
              target.dryRun.position as string,
              target.dryRun.sourceLine as number
            )) ?? null;
        ...
        for (let index = 0; index < inputs.length; index += 1) {
          ...
          try {
            await appendAudit(auditFilePath, {
              ...
              fieldId: input.fieldId,
              formulaKind: input.formulaKind,
              position: target.dryRun.position,
      ...
      try {
        await appendAudit(auditFilePath, {
          ...
          targets: inputs.map((input, index) => ({
            ...
            fieldId: input.fieldId,
            formulaKind: input.formulaKind,
            position: results[index]?.dryRun.position ?? input.position,
            sourceLine: results[index]?.dryRun.sourceLine ?? input.sourceLine,
