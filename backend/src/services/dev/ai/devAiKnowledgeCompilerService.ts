...
const MANIFEST_FILE_NAME = ".compiled-manifest";
...
interface CompiledKnowledgeManifest {
  schemaVersion: "dev-ai-compiled-knowledge.v1";
  sourceHash: string;
  compiledAt: string;
...
export interface DevAiKnowledgeCompilerServiceDeps {
  ...
  afterFileWritten?: (fileName: string, index: number) => Promise<void> | void;
}
...
async function readApprovedExamples(filePath: string): Promise<ParsedApprovedExamples> {
  ...
  if (!raw.trim()) return { entries: [], malformed: 0, sourceHash };
      ...
      try {
        ...
        return [{
          ...
          metadata: {
            ...
            position:
              typeof metadata.position === "string" ? maskSecrets(metadata.position) : null,
            sourceLine:
              typeof metadata.sourceLine === "number" &&
              Number.isSafeInteger(metadata.sourceLine) &&
              metadata.sourceLine > 0
                ? metadata.sourceLine
                : null,
            formulaKind:
              typeof metadata.formulaKind === "string" ? maskSecrets(metadata.formulaKind) : null,
            domain: typeof metadata.domain === "string" ? maskSecrets(metadata.domain) : null,
          },
        }];
      } catch {
        malformed += 1;
  ...
  return { entries, malformed, sourceHash };
}
...
async function readCompiledManifest(
  compiledDir: string
): Promise<CompiledKnowledgeManifest | null> {
  try {
    const parsed = JSON.parse(
      await readFile(path.join(compiledDir, MANIFEST_FILE_NAME), "utf8")
    ) as Partial<CompiledKnowledgeManifest>;
    if (
      parsed.schemaVersion !== "dev-ai-compiled-knowledge.v1" ||
      typeof parsed.sourceHash !== "string" ||
      typeof parsed.compiledAt !== "string" ||
    ...
    return parsed as CompiledKnowledgeManifest;
  } catch (error) {
...
function renderMarkdown(params: {
  ...
  params.entries.forEach((entry, index) => {
    ...
    if (entry.metadata.position) lines.push(`- position: ${entry.metadata.position}`);
    if (entry.metadata.sourceLine) lines.push(`- sourceLine: ${entry.metadata.sourceLine}`);
    if (entry.metadata.formulaKind) lines.push(`- formulaKind: ${entry.metadata.formulaKind}`);
    if (entry.metadata.domain) lines.push(`- domain: ${entry.metadata.domain}`);
    lines.push("");
    lines.push("### 整理內容");
    lines.push("");
...
async function listCompiledFiles(
  ...
  manifest: CompiledKnowledgeManifest | null;
  ...
  const manifest = await readCompiledManifest(compiledDir);
  ...
  const manifestFiles = new Map(
    (manifest?.files ?? []).map((file) => [file.path, file.contentHash])
  );
  const matchesManifest =
    files.length === 2 &&
    manifestFiles.size === 2 &&
    (
      await Promise.all(
        files.map(async (file) => {
          const contentHash = manifestFiles.get(file.path);
          return Boolean(
            contentHash &&
              contentHash === sha256(await readFile(path.join(compiledDir, file.path), "utf8"))
          );
        })
      )
    ).every(Boolean);
...
export function createDevAiKnowledgeCompilerService(
  ...
  async function buildStatus(): Promise<DevAiKnowledgeStatusResult> {
    ...
    const needsCompile =
      ...
      (!compiled.matchesManifest || compiled.manifest?.sourceHash !== parsed.sourceHash);
  ...
  async function doCompile(
    ...
    for (const [index, target] of targets.entries()) {
      ...
      await writeTextAtomically(filePath, target.content);
      ...
      await deps.afterFileWritten?.(target.fileName, index);
    }

    await writeTextAtomically(
      path.join(compiledKnowledgeDir, MANIFEST_FILE_NAME),
      `${JSON.stringify({
        schemaVersion: "dev-ai-compiled-knowledge.v1",
      ...
      } satisfies CompiledKnowledgeManifest, null, 2)}\n`
    );
  ...
  return {
    ...
    async compile(options = {}) {
      const next = compileChain.then(() =>
        publicationBarrier.withWrite(() => doCompile(options))
      );
