...
import { normalizeGeneratedDevAiThreadTitle } from "./devAiThreadTitle";
import type {
...
function buildPrompt(params: {
  ...
  return [
    ...
    "所有中文敘述必須使用繁體中文與台灣慣用詞。",
    "threadTitle 請根據使用者需求與公式建議產生 4 到 24 字的短標題，只能使用繁體中文或英文；不得使用簡體字、日文、韓文、emoji、引號或 Markdown。",
    ...
    JSON.stringify({
      ...
      position: params.request.position ?? null,
      sourceLine: params.request.sourceLine ?? null,
      ...
      try {
        ...
        const prompt = buildPrompt({
          request: {
            ...
            sourceLine: context.targetSourceLine,
          },
        ...
        const result: RagicFormulaAiSuggestResult = {
          ...
          model: config.model,
          threadTitle: modelOutput.threadTitle,
          formPath: request.formPath,
          fieldId: request.fieldId,
          position: context.targetPosition,
          sourceLine: context.targetSourceLine,
