...
interface ThreadRow {
  ...
  current_source_line: number | null;
...
function normalizeContext(context: DevAiThreadContext): DevAiThreadContext {
  return {
    ...
    position: context.position?.trim().toUpperCase() ?? "",
    ...(Number.isSafeInteger(context.sourceLine) && Number(context.sourceLine) > 0
      ? { sourceLine: Number(context.sourceLine) }
      : {}),
    formulaKind:
...
function mapThread(row: ThreadRow): DevAiThread {
  return {
    ...
    context: {
      ...
      ...(row.current_position ? { position: row.current_position } : {}),
      ...(row.current_source_line !== null
        ? { sourceLine: row.current_source_line }
        : {}),
...
async function initializeSchema(db: Database): Promise<void> {
  ...
  if (!existing.has("current_position")) {
    await db.exec("ALTER TABLE dev_ai_threads ADD COLUMN current_position TEXT");
  }
  if (!existing.has("current_source_line")) {
    await db.exec("ALTER TABLE dev_ai_threads ADD COLUMN current_source_line INTEGER");
  }
}
...
export function createDevAiThreadRepository(
  ...
  return {
    async createThread(input) {
      ...
      await db.run(
        `INSERT INTO dev_ai_threads (
          ...
          current_form_path, current_field_id, current_position,
          current_source_line, current_formula_kind, last_message_preview
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        id,
        input.ownerActor,
        input.title,
    ...
    async listArtifacts(ownerActor, threadId, limit = 100) {
      ...
      return rows.reverse().map(mapArtifact);
    },
      ...
      if (context) {
        await db.run(
          `UPDATE dev_ai_threads
           SET updated_at = ?, last_message_preview = ?, title = COALESCE(?, title),
               current_form_path = ?,
               current_field_id = ?,
               current_position = ?,
               current_source_line = ?,
               current_formula_kind = ?
