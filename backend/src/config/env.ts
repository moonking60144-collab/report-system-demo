...
import { resolveRagicMutationReadPolicy } from "../infra/ragicReadRetryPolicy";
...
const ENV_ALIASES: Array<[string, string]> = [
  ...
  ["UPSTREAM_FORM_901_PATH", "RAGIC_FORM_901_PATH"],
  ["UPSTREAM_FORM_901_TEST_PATH", "RAGIC_FORM_901_TEST_PATH"],
  ["UPSTREAM_FORM_901_CLOSE_ACTION_BUTTON_ID", "RAGIC_FORM_901_CLOSE_ACTION_BUTTON_ID"],
  ["UPSTREAM_FORM_901_REOPEN_ACTION_BUTTON_ID", "RAGIC_FORM_901_REOPEN_ACTION_BUTTON_ID"],
  ["UPSTREAM_FORM_902_PATH", "RAGIC_FORM_902_PATH"],
  ["UPSTREAM_FORM_902_TEST_PATH", "RAGIC_FORM_902_TEST_PATH"],
  ["UPSTREAM_FORM_902_CLOSE_ACTION_BUTTON_ID", "RAGIC_FORM_902_CLOSE_ACTION_BUTTON_ID"],
  ["UPSTREAM_FORM_902_REOPEN_ACTION_BUTTON_ID", "RAGIC_FORM_902_REOPEN_ACTION_BUTTON_ID"],
...
if (isDemoMode) {
  ...
  process.env.RAGIC_FORM_901_PATH ??= "/demo/work-orders/line-a";
  process.env.RAGIC_FORM_902_PATH ??= "/demo/work-orders/line-b";
  process.env.UPSTREAM_ACTIVITY_LOG_PATH ??= "/demo/activity-logs";
  process.env.RAGIC_FORM_901_TEST_PATH ??= "/demo/work-orders/line-a";
  process.env.RAGIC_FORM_902_TEST_PATH ??= "/demo/work-orders/line-b";
  process.env.UPSTREAM_ACTIVITY_LOG_TEST_PATH ??= "/demo/activity-logs";
  process.env.UPSTREAM_ACTIVITY_LOG_SAVE_ACTION_BUTTON_ID ??= "9003";
  process.env.RAGIC_FORM_901_CLOSE_ACTION_BUTTON_ID ??= "9001";
  process.env.RAGIC_FORM_901_REOPEN_ACTION_BUTTON_ID ??= "9002";
  process.env.RAGIC_FORM_902_CLOSE_ACTION_BUTTON_ID ??= "9001";
  process.env.RAGIC_FORM_902_REOPEN_ACTION_BUTTON_ID ??= "9002";
  process.env.UPSTREAM_ACTIVITY_LOG_WORK_ORDER_FIELD_ID ??= "9001047";
  process.env.UPSTREAM_ACTIVITY_LOG_TYPE_FIELD_ID ??= "9001065";
  process.env.UPSTREAM_ACTIVITY_LOG_PROCESS_FIELD_ID ??= "9001033";
  process.env.UPSTREAM_ACTIVITY_LOG_DEP_FIELD_ID ??= "9001037";
  process.env.UPSTREAM_ACTIVITY_LOG_PROD_TYPE_FIELD_ID ??= "9001031";
  process.env.UPSTREAM_ACTIVITY_LOG_REMARK_FIELD_ID ??= "9001029";
  process.env.UPSTREAM_ACTIVITY_LOG_DATE_FIELD_ID ??= "9001030";
  process.env.RAGIC_SOURCE_MACHINE ??= "/demo/reference/machines";
  process.env.RAGIC_SOURCE_OPERATOR ??= "/demo/reference/operators";
  process.env.RAGIC_SOURCE_PROCESS ??= "/demo/reference/processes";
  process.env.SQLITE_READ_FORMS ??= "901,902";
  process.env.SQLITE_AUTO_SYNC_FORMS ??= "901,902";
  ...
  process.env.ACTIVITY_LOG_SQLITE_AUTO_SYNC_ENABLED ??= "false";
  process.env.ACTIVITY_LOG_PLANNED_IDLE_SYNC_ENABLED ??= "false";
  process.env.ACTIVITY_LOG_WRITE_REVERIFY_ENABLED ??= "false";
  process.env.RAGIC_FIELD_INDEX_AUTO_REFRESH_ENABLED ??= "false";
  process.env.RUNTIME_HEALTH_LOG_ENABLED ??= "false";
  process.env.ACTIVITY_LOG_ORPHAN_CLEANUP_ENABLED ??= "false";
  process.env.DEV_AI_ENABLED ??= "false";
  process.env.DEV_AI_RETRIEVAL_MODE ??= "lexical";
  ...
  if (process.env.FLY_APP_NAME) {
    ...
    process.env.REPORT_FULL_CACHE_FILE ??= "/data/reports-901-full.v1.json";
    ...
    process.env.ACTIVITY_LOG_WRITE_REVERIFY_STORE_FILE ??= "/data/activityLog-write-reverify.v1.json";
...
const ragicMutationReadTimeoutMs = Math.max(
  1_000,
  Math.trunc(readNumberEnv("RAGIC_MUTATION_READ_TIMEOUT_MS", 10_000))
);
const ragicMutationReadMaxRetries = Math.max(
  0,
  Math.trunc(readNumberEnv("RAGIC_MUTATION_READ_MAX_RETRIES", 2))
);
const ragicGetRetryBaseDelayMs = Math.max(
  0,
  Math.trunc(readNumberEnv("RAGIC_GET_RETRY_BASE_DELAY_MS", 200))
);
const defaultRagicMutationReadPolicy = resolveRagicMutationReadPolicy({
  timeoutMs: ragicMutationReadTimeoutMs,
  maxRetries: ragicMutationReadMaxRetries,
  retryBaseDelayMs: ragicGetRetryBaseDelayMs,
});
const ragicMutationReadPolicy = resolveRagicMutationReadPolicy({
  timeoutMs: ragicMutationReadTimeoutMs,
  maxRetries: ragicMutationReadMaxRetries,
  retryBaseDelayMs: ragicGetRetryBaseDelayMs,
  configuredTotalBudgetMs: Math.trunc(
    readNumberEnv(
      "RAGIC_MUTATION_READ_TOTAL_BUDGET_MS",
      defaultRagicMutationReadPolicy.totalBudgetMs
    )
  ),
});
...
export const env = {
  ...
  // server 例：C:\Demo\RagicBuilder\cust\def\app。本機沒這目錄就留空。
  RAGIC_BUILDER_PATH: process.env.RAGIC_BUILDER_PATH ?? "",
  RAGIC_API_KEY: resolveRagicApiKey(),
  DEV_SOURCE_RAGIC_API_KEY:
    process.env.DEV_SOURCE_RAGIC_API_KEY?.trim() || resolveRagicApiKey(),
  /**
  ...
  RAGIC_FORM_901_PATH: readRequiredEnv("RAGIC_FORM_901_PATH"),
  RAGIC_FORM_901_TEST_PATH: process.env.RAGIC_FORM_901_TEST_PATH ?? "",
  RAGIC_FORM_901_CLOSE_ACTION_BUTTON_ID: process.env.RAGIC_FORM_901_CLOSE_ACTION_BUTTON_ID ?? "",
  RAGIC_FORM_901_REOPEN_ACTION_BUTTON_ID: process.env.RAGIC_FORM_901_REOPEN_ACTION_BUTTON_ID ?? "",
  RAGIC_FORM_902_PATH: process.env.RAGIC_FORM_902_PATH ?? "",
  RAGIC_FORM_902_TEST_PATH: process.env.RAGIC_FORM_902_TEST_PATH ?? "",
  RAGIC_FORM_902_CLOSE_ACTION_BUTTON_ID: process.env.RAGIC_FORM_902_CLOSE_ACTION_BUTTON_ID ?? "",
  RAGIC_FORM_902_REOPEN_ACTION_BUTTON_ID: process.env.RAGIC_FORM_902_REOPEN_ACTION_BUTTON_ID ?? "",
  UPSTREAM_ACTIVITY_LOG_PATH: process.env.UPSTREAM_ACTIVITY_LOG_PATH ?? "/demo/activity-logs",
  UPSTREAM_ACTIVITY_LOG_TEST_PATH: process.env.UPSTREAM_ACTIVITY_LOG_TEST_PATH ?? "",
  UPSTREAM_ACTIVITY_LOG_SAVE_ACTION_BUTTON_ID: readRequiredNumericIdEnv(
    "UPSTREAM_ACTIVITY_LOG_SAVE_ACTION_BUTTON_ID"
  ),
  // activity log 欄位 ID 全部強制必填：若 Ragic schema 改 ID、.env 沒同步，
  // 寧可 boot 直接 throw，也不要 baked-in 的值 silent mismatch 害系統繼續寫錯欄位
  UPSTREAM_ACTIVITY_LOG_WORK_ORDER_FIELD_ID: readRequiredNumericIdEnv("UPSTREAM_ACTIVITY_LOG_WORK_ORDER_FIELD_ID"),
  UPSTREAM_ACTIVITY_LOG_TYPE_FIELD_ID: readRequiredNumericIdEnv("UPSTREAM_ACTIVITY_LOG_TYPE_FIELD_ID"),
  UPSTREAM_ACTIVITY_LOG_PROCESS_FIELD_ID: readRequiredNumericIdEnv("UPSTREAM_ACTIVITY_LOG_PROCESS_FIELD_ID"),
  UPSTREAM_ACTIVITY_LOG_DEP_FIELD_ID: readRequiredNumericIdEnv("UPSTREAM_ACTIVITY_LOG_DEP_FIELD_ID"),
  UPSTREAM_ACTIVITY_LOG_PROD_TYPE_FIELD_ID: readRequiredNumericIdEnv("UPSTREAM_ACTIVITY_LOG_PROD_TYPE_FIELD_ID"),
  UPSTREAM_ACTIVITY_LOG_REMARK_FIELD_ID: readRequiredNumericIdEnv("UPSTREAM_ACTIVITY_LOG_REMARK_FIELD_ID"),
  UPSTREAM_ACTIVITY_LOG_DATE_FIELD_ID: readRequiredNumericIdEnv("UPSTREAM_ACTIVITY_LOG_DATE_FIELD_ID"),
  ...
  SQLITE_READ_FORMS: readStringListEnv("SQLITE_READ_FORMS", ["901", "902"]),
  ...
  ACTIVITY_LOG_SQLITE_AUTO_SYNC_ENABLED: readBooleanEnv("ACTIVITY_LOG_SQLITE_AUTO_SYNC_ENABLED", true),
  // 計畫停機統計 SQLite 同步：背景定時撈近半年 (P)計畫停機分進 SQLite，讓圖表查 SQLite 秒回。
  ACTIVITY_LOG_PLANNED_IDLE_SYNC_ENABLED: readBooleanEnv("ACTIVITY_LOG_PLANNED_IDLE_SYNC_ENABLED", true),
  ACTIVITY_LOG_PLANNED_IDLE_SYNC_INTERVAL_MS: Math.max(
    60_000,
    Math.trunc(readNumberEnv("ACTIVITY_LOG_PLANNED_IDLE_SYNC_INTERVAL_MS", 30 * 60 * 1000))
  ),
  ACTIVITY_LOG_PLANNED_IDLE_SYNC_STARTUP_DELAY_MS: Math.max(
    0,
    Math.trunc(readNumberEnv("ACTIVITY_LOG_PLANNED_IDLE_SYNC_STARTUP_DELAY_MS", 120_000))
  ),
  SQLITE_AUTO_SYNC_INTERVAL_MS: Math.max(
    60_000,
    Math.trunc(readNumberEnv("SQLITE_AUTO_SYNC_INTERVAL_MS", 60 * 60 * 1000))
  ),
  ...
  // 背景任務（Ragic callback refresh、mutation projection、activity log refresh）
  ...
  RAGIC_MUTATION_READ_TIMEOUT_MS: ragicMutationReadPolicy.timeoutMs,
  RAGIC_MUTATION_READ_TOTAL_BUDGET_MS: ragicMutationReadPolicy.totalBudgetMs,
  RAGIC_MUTATION_VERIFY_TIMEOUT_MS: Math.max(
    1_000,
    Math.trunc(
      readNumberEnv(
        "RAGIC_MUTATION_VERIFY_TIMEOUT_MS",
        readNumberEnv("RAGIC_MUTATION_READ_TIMEOUT_MS", 10_000)
      )
    )
  ),
  RAGIC_MUTATION_READ_MAX_RETRIES: ragicMutationReadPolicy.maxRetries,
  ...
  ACTIVITY_LOG_WRITE_VERIFY_TIMEOUT_MS: Math.max(
    1_000,
    Math.trunc(readNumberEnv("ACTIVITY_LOG_WRITE_VERIFY_TIMEOUT_MS", 10_000))
  ),
  ACTIVITY_LOG_WRITE_VERIFY_MAX_RETRIES: Math.max(
    0,
    Math.trunc(readNumberEnv("ACTIVITY_LOG_WRITE_VERIFY_MAX_RETRIES", 0))
  ),
  ACTIVITY_LOG_WRITE_REVERIFY_ENABLED: readBooleanEnv("ACTIVITY_LOG_WRITE_REVERIFY_ENABLED", true),
  ACTIVITY_LOG_WRITE_REVERIFY_STORE_FILE:
    process.env.ACTIVITY_LOG_WRITE_REVERIFY_STORE_FILE ?? "./.data/activityLog-write-reverify.v1.json",
  ACTIVITY_LOG_WRITE_REVERIFY_INTERVAL_MS: Math.max(
    10_000,
    Math.trunc(readNumberEnv("ACTIVITY_LOG_WRITE_REVERIFY_INTERVAL_MS", 60_000))
  ),
  ACTIVITY_LOG_WRITE_REVERIFY_STARTUP_DELAY_MS: Math.max(
    0,
    Math.trunc(readNumberEnv("ACTIVITY_LOG_WRITE_REVERIFY_STARTUP_DELAY_MS", 30_000))
  ),
  ACTIVITY_LOG_WRITE_REVERIFY_MAX_PER_RUN: Math.max(
    1,
    Math.trunc(readNumberEnv("ACTIVITY_LOG_WRITE_REVERIFY_MAX_PER_RUN", 20))
  ),
  ACTIVITY_LOG_WRITE_REVERIFY_MAX_ATTEMPTS: Math.max(
    1,
    Math.trunc(readNumberEnv("ACTIVITY_LOG_WRITE_REVERIFY_MAX_ATTEMPTS", 6))
  ),
  ACTIVITY_LOG_WRITE_REVERIFY_TIMEOUT_MS: Math.max(
    1_000,
    Math.trunc(readNumberEnv("ACTIVITY_LOG_WRITE_REVERIFY_TIMEOUT_MS", 30_000))
  ),
  ACTIVITY_LOG_WRITE_REVERIFY_MAX_RETRIES: Math.max(
    0,
    Math.trunc(readNumberEnv("ACTIVITY_LOG_WRITE_REVERIFY_MAX_RETRIES", 1))
  ),
  ACTIVITY_LOG_WRITE_REVERIFY_MAX_AGE_MS: Math.max(
    60_000,
    Math.trunc(readNumberEnv("ACTIVITY_LOG_WRITE_REVERIFY_MAX_AGE_MS", 24 * 60 * 60 * 1000))
  ),
  ACTIVITY_LOG_SQLITE_REFRESH_INTERVAL_MS: Math.max(
    10_000,
    Math.trunc(readNumberEnv("ACTIVITY_LOG_SQLITE_REFRESH_INTERVAL_MS", 60_000))
  ),
  ...
  RAGIC_GET_RETRY_BASE_DELAY_MS: ragicMutationReadPolicy.retryBaseDelayMs,
  ...
  REPORT_FULL_CACHE_FILE:
    process.env.REPORT_FULL_CACHE_FILE ?? "./.cache/reports-901-full.v1.json",
  ...
  DEV_AI_RETRIEVAL_MODE:
    process.env.DEV_AI_RETRIEVAL_MODE?.trim() || "lexical",
  DEV_AI_VECTOR_DB_FILE:
    process.env.DEV_AI_VECTOR_DB_FILE?.trim() ||
    path.join(
      path.dirname(
  ...
  DEV_AI_EMBEDDING_CACHE_DIR:
    process.env.DEV_AI_EMBEDDING_CACHE_DIR?.trim() ||
    path.join(
      path.dirname(
        process.env.DEV_AI_CONVERSATION_DB_FILE?.trim() ||
          "./.data/dev-ai/conversations.v1.sqlite3"
  ...
  DEV_AI_EMBEDDING_ALLOW_DOWNLOAD: readBooleanEnv(
    "DEV_AI_EMBEDDING_ALLOW_DOWNLOAD",
    false
  ),
  DEV_AI_EMBEDDING_TIMEOUT_MS: Math.max(
    1_000,
    Math.trunc(readNumberEnv("DEV_AI_EMBEDDING_TIMEOUT_MS", 180_000))
  ),
  ...
  DEV_AI_CONVERSATION_DB_FILE:
    process.env.DEV_AI_CONVERSATION_DB_FILE?.trim() ||
    "./.data/dev-ai/conversations.v1.sqlite3",
  DEV_AI_KNOWLEDGE_DB_FILE:
    process.env.DEV_AI_KNOWLEDGE_DB_FILE?.trim() ||
    "./.data/dev-ai/knowledge.v1.sqlite3",
  ...
  // activity log 孤兒背景清理：預設關閉，確認 2A/2B/2C 上線穩定後再手動 env 打開
  ACTIVITY_LOG_ORPHAN_CLEANUP_ENABLED: readBooleanEnv("ACTIVITY_LOG_ORPHAN_CLEANUP_ENABLED", false),
  // Creator filter：空值不給預設（避免寫死個人名稱在 prod config 裡）。
  // CLEANUP_ENABLED=true 時會在 bootstrap 強制檢查這個必須有值
  ACTIVITY_LOG_ORPHAN_CREATOR_ACCOUNT:
    process.env.ACTIVITY_LOG_ORPHAN_CREATOR_ACCOUNT?.trim() ?? "",
} as const;
...
export function resolveWritePath(formId: string, defaultPath: string): string | null {
  ...
  if (formId === "901") {
    return env.RAGIC_FORM_901_TEST_PATH || null;
  }

  if (formId === "902") {
    return env.RAGIC_FORM_902_TEST_PATH || null;
  }

  if (formId === "903") {
    return env.UPSTREAM_ACTIVITY_LOG_TEST_PATH || null;
  }

  return null;
}

export function resolveWorkReportDataPath(formId: string, defaultPath: string): string {
  return resolveWritePath(formId, defaultPath) ?? defaultPath;
}
