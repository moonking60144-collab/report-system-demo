...
import { ragicFormulaOccurrenceKey } from "../services/dev/ragicFormulaOccurrence";
import {
...
} from "../services/dev/ai/devAiKnowledgeBaseService";
...
function parseFormulaPatchBody(body: {
  ...
  const position =
    typeof body.position === "string" ? body.position.trim().toUpperCase() : "";
  const hasSourceLine =
    body.sourceLine !== undefined && body.sourceLine !== null && body.sourceLine !== "";
  const sourceLine = hasSourceLine ? Number(body.sourceLine) : null;
  ...
  if (hasSourceLine && (!Number.isSafeInteger(sourceLine) || (sourceLine as number) <= 0)) {
    throw new HttpError(400, "sourceLine 必須是正整數", "BAD_FORMULA_SOURCE_LINE");
  }
  if (Boolean(position) !== hasSourceLine) {
    throw new HttpError(
      400,
  ...
  return {
    ...
    fieldId,
    formulaKind,
    ...(position ? { position, sourceLine: sourceLine as number } : {}),
    newFormula,
  };
}
...
function parseFormulaAiSuggestBody(body: {
  ...
  const position =
    typeof body.position === "string" ? body.position.trim().toUpperCase() : "";
  const sourceLine = body.sourceLine === undefined ? undefined : Number(body.sourceLine);
  const rawFormulaKind = String(body.formulaKind ?? "formula").trim();
  const formulaKind =
    rawFormulaKind === "formula" || rawFormulaKind === "defaultFormula"
      ? rawFormulaKind
...
function parseDevAiFeedbackBody(body: {
  ...
  return {
    ...
    ...(typeof body.sourceLine === "number" ? { sourceLine: body.sourceLine } : {}),
...
export function createDevRagicDefinitionsRouter(
  ...
  const knowledgeGovernanceService =
    deps.devAiKnowledgeGovernanceService ?? devAiKnowledgeGovernanceService;
  const knowledgeCompilerService =
    deps.devAiKnowledgeCompilerService ?? devAiKnowledgeCompilerService;
  const knowledgeBaseService =
    deps.devAiKnowledgeBaseService ?? devAiKnowledgeBaseService;
  const verifyToken =
    deps.verifyToken ??
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      for (const target of targets) {
        const key = ragicFormulaOccurrenceKey(target);
        if (seen.has(key)) {
          throw new HttpError(
            400,
            `targets 內同一公式 occurrence 重複：${target.formPath} · ${target.fieldId} · ${target.position ?? "legacy"}`,
            "DUPLICATE_TARGET"
          );
        }
        seen.add(key);
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const result = await knowledgeGovernanceService.submitCandidate({
        payload: data,
        domain: data.kind === "formula-suggestion" ? "definitions" : "ragic",
      }, {
  ...
  router.get(
    "/ai/knowledge/runtime-status",
    asyncHandler(async (_req, res) => {
      if (!knowledgeBaseService.getRuntimeStatus) {
        throw new HttpError(
          503,
          "Dev AI 檢索狀態暫時無法取得",
          "DEV_AI_RUNTIME_STATUS_UNAVAILABLE"
        );
      }
      res.json({ data: await knowledgeBaseService.getRuntimeStatus() });
    })
  );
