...
import { getSqliteAutoSyncStatus } from "../services/work-report-sync/sqliteAutoSyncStatus";

const realtimeEventsRouter = Router();
realtimeEventsRouter.get("/auto-sync/status", (_req, res) => {
  res.json({ data: getSqliteAutoSyncStatus() });
});
