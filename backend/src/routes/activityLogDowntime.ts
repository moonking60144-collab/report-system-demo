import { parseActivityLogFieldPrecondition } from "../services/activityLog/activityLogFieldPrecondition";
import { randomUUID } from "crypto";
import { Router, type Response } from "express";
import { asyncHandler } from "./asyncHandler";
import { HttpError } from "../utils/httpError";
import { activityLogDowntimeService } from "../services/activityLog/activityLogDowntimeService";
import { activityLogDowntimeCreateTaskService } from "../services/activityLog/activityLogDowntimeCreateTaskService";
import { activityLogDowntimeCallbackRefreshService } from "../services/activityLog/activityLogDowntimeCallbackRefreshService";
import {
...
} from "../services/activityLog/activityLogDowntimeMutationQueue";
...
const activityLogDowntimeRouter = Router();
...
const DOWNTIME_TASK_SCOPES = ["mine", "all"] as const;
type DowntimeTaskScope = (typeof DOWNTIME_TASK_SCOPES)[number];
...
activityLogDowntimeRouter.get(
  "/downtime/options",
  asyncHandler(async (_req, res) => {
    const data = await activityLogDowntimeService.getOptions();
    res.json({
      data,
      meta: {
...
activityLogDowntimeRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await activityLogDowntimeService.listRecords({
      refresh,
      limit,
      offset,
    });
...
activityLogDowntimeRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const refresh = String(req.query.refresh ?? "").trim() === "1";
    const result = await activityLogDowntimeService.summarizePlannedIdleByMachine(monthParam, refresh);
    res.json({
...
// activity log 在 Ragic 端的 JavaScript Workflow Callback 會 POST 到這裡
// （headers: x-ragic-callback-token + Content-Type: application/json）
// body 格式：{ entryId, eventType, source }
// 用途：接住別人在 Ragic UI 直接改/刪/新增 activity log entry 的事件，
// 將本地 SQLite snapshot 同步到最新狀態。
// 路徑跟 901/902 的 `/api/forms/:formId/ragic-callback` 對齊；
// 由於 activityLogDowntimeRouter 在 server.ts 比 workReportRouter 先 mount，
// `POST /api/forms/903/ragic-callback` 會先被這裡攔到，不會走到 work-report 的 `/:formId` 路由。
activityLogDowntimeRouter.post(
  "/forms/903/ragic-callback",
  asyncHandler(async (req, res) => {
    ...
    const task = activityLogDowntimeCallbackRefreshService.enqueue({
...
activityLogDowntimeRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const actor = readTaskActorContext(req);
    await activityLogDowntimeCreateTaskService.initialize();
    const task = activityLogDowntimeCreateTaskService.enqueue({
      payload: input,
...
activityLogDowntimeRouter.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const expectedValues = parseActivityLogFieldPrecondition(body, patch);
    const fieldPrecondition = expectedValues ? { expectedValues, fieldPreconditionVersion: 1 as const } : {};
    ...
    if (readAsyncMutationFlag(req.query.async)) {
      ...
      const task = createReportTaskService.enqueue({
        ...
        formId: "903",
        entryId,
        queueKey: ACTIVITY_LOG_DOWNTIME_MUTATION_QUEUE_KEY,
        clientMutationId,
        ...
        worker: async () => {
          const updated = await activityLogDowntimeService.updateRecord(entryId, patch, {
            expectedSnapshotHash,
            ...fieldPrecondition,
    ...
    try {
      await runActivityLogDowntimeMutationExclusive(async () => {
        const updated = await activityLogDowntimeService.updateRecord(entryId, patch, {
          expectedSnapshotHash,
          ...fieldPrecondition,
        });
...
activityLogDowntimeRouter.delete(
  "/downtime/records/:entryId",
  asyncHandler(async (req, res) => {
    const entryId = String(req.params.entryId ?? "").trim();
    if (!/^\d+$/.test(entryId)) {
    ...
    if (readAsyncMutationFlag(req.query.async)) {
      ...
      const task = createReportTaskService.enqueue({
        ...
        actorLabel: actor.actorLabel ?? undefined,
        worker: async () => {
          const deleted = await activityLogDowntimeService.deleteRecord(entryId, {
            expectedSnapshotHash,
            deferProjection: true,
          });
          await safeInsertRecordAudit({
...
activityLogDowntimeRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const scope = (String(req.query.scope ?? "").trim() || "mine") as DowntimeTaskScope;
    ...
    if (!DOWNTIME_TASK_SCOPES.includes(scope)) {
      throw new HttpError(400, `不支援的任務範圍：${scope}`, "TASK_SCOPE_INVALID");
    }

    const tasks =
      scope === "all" || actorClientId
        ? workReportTaskRegistryService.listTasks({
            ...
            actorClientId: scope === "mine" ? actorClientId ?? undefined : undefined,
            limit,
          })
        : [];
...
activityLogDowntimeRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const task = workReportTaskRegistryService.getTask(taskId);
    if (!task || task.formId !== "903" || !DOWNTIME_TASK_TYPES.includes(task.taskType)) {
...
export default activityLogDowntimeRouter;
