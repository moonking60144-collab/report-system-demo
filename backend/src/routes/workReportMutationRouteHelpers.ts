...
const log = createLogger("work-report-mutation-projection");
const PROJECTION_ENQUEUE_MAX_ATTEMPTS = 2;
const PROJECTION_ENQUEUE_RETRY_DELAY_MS = 50;

type ProjectionEnqueueDeps = Pick<
  WorkReportRouterDeps,
  "enqueueSqliteProjectionAfterMutation" | "requestSync"
>;

type MutationProjectionHookDeps = ProjectionEnqueueDeps &
  Pick<WorkReportRouterDeps, "applyQueuedSqliteProjectionAfterMutation">;

type SortOrderProjectionHookDeps = ProjectionEnqueueDeps &
  Pick<WorkReportRouterDeps, "applyQueuedSortOrderSqliteAfterMutation">;

function isRetryableProjectionEnqueueError(error: unknown): boolean {
  const code =
...
async function waitProjectionEnqueueRetry(): Promise<void> {
  await new Promise<void>((resolveDelay) => {
    setTimeout(resolveDelay, PROJECTION_ENQUEUE_RETRY_DELAY_MS);
  });
}

async function enqueueSqliteProjectionAfterMutationSafe(
  deps: ProjectionEnqueueDeps,
  formId: string,
  entryId: string,
  reason: ProjectionReason
): Promise<number | null> {
  for (let attempt = 1; attempt <= PROJECTION_ENQUEUE_MAX_ATTEMPTS; attempt += 1) {
    try {
      return await deps.enqueueSqliteProjectionAfterMutation(formId, entryId, reason);
    } catch (error) {
      if (
        attempt < PROJECTION_ENQUEUE_MAX_ATTEMPTS &&
        isRetryableProjectionEnqueueError(error)
      ) {
        await waitProjectionEnqueueRetry();
        continue;
      }
      log.warn({
...
async function requestProjectionReconciliationSafe(
  deps: ProjectionEnqueueDeps,
  formId: string,
  entryId: string,
  reason: ProjectionReason
  ...
  try {
    await deps.requestSync(formId, {
      triggeredBy: "mutation-reconcile",
      waitForCompletion: false,
      queueIfRunning: true,
    });
    log.warn({
      event: "reconciliation-scheduled",
      formId,
      entryId,
      reason,
    });
    ...
    log.error({
      event: "reconciliation-schedule-failed",
      formId,
      entryId,
      reason,
      error: error instanceof Error ? error.message : String(error),
...
export async function runPostMutationHooks(
  deps: MutationProjectionHookDeps,
  formId: string,
  entryId: string,
  mutationType: "create" | "update" | "delete",
  observedEntry?: RagicRecord
): Promise<number> {
  const projectionEnqueueStartedAt = Date.now();
  ...
  if (projectionSeq === null) {
    await requestProjectionReconciliationSafe(deps, formId, entryId, mutationType);
    return Date.now() - projectionEnqueueStartedAt;
  }
  if (projectionSeq <= 0) {
...
export async function runPostSortOrderMutationHooks(
  deps: SortOrderProjectionHookDeps,
  formId: string,
  entryId: string,
  sortOrder: number
): Promise<number> {
  ...
  if (projectionSeq === null) {
    await requestProjectionReconciliationSafe(deps, formId, entryId, "update");
    return Date.now() - projectionEnqueueStartedAt;
  }
  if (projectionSeq <= 0) {
    publishMutationUpdated(formId, entryId);
