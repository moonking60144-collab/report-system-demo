...
const log = createLogger("activityLog-client-row-key-cleanup");
...
async function runOnce(): Promise<void> {
  ...
  const deleted = await activityLogClientRowKeyRepository.cleanupOlderThan(thresholdIso);
  if (deleted > 0) {
    log.info({ event: "cleaned", deleted, thresholdIso });
  }
}

export function startActivityLogClientRowKeyCleanup(): void {
  if (cleanupTimer || startupTimer) return;
  if (!env.SQLITE_ENABLED) return;

  startupTimer = setTimeout(() => {
    runBackgroundTask("activityLog-client-row-key-cleanup.startup", runOnce);
    startupTimer = null;
  }, 5 * 60 * 1000);
  startupTimer.unref();

  cleanupTimer = setInterval(() => {
    runBackgroundTask("activityLog-client-row-key-cleanup.interval", runOnce);
  }, CLEANUP_INTERVAL_MS);
  cleanupTimer.unref();
}

export function stopActivityLogClientRowKeyCleanup(): void {
  if (startupTimer) {
