...
import { runActivityLogOrphanCleanupOnce } from "../services/activityLog/activityLogOrphanCleanupService";

const log = createLogger("activityLog-orphan-cleanup");
...
async function runOnce(): Promise<void> {
  if (!env.ACTIVITY_LOG_ORPHAN_CLEANUP_ENABLED) return;

  const stats = await runActivityLogOrphanCleanupOnce({
    daysBack: DAYS_BACK,
    ageThresholdMs: AGE_THRESHOLD_MS,
    maxPerRun: MAX_PER_RUN,
    creatorAccount: env.ACTIVITY_LOG_ORPHAN_CREATOR_ACCOUNT,
  });
  ...
  if (hasNonWhitelisted) {
    log.warn({
      ...
      hint: "這些報工類別符合『workOrderNo 空 + 非計畫停機 + creator 符合 + ERP料號不含 test』但不在白名單；若確認該清理就加進 ACTIVITY_LOG_WORK_ORDER_REQUIRED_TYPES",
      types: stats.nonWhitelistedReportTypes,
    });
  }
}

export function startActivityLogOrphanCleanup(): void {
  if (cleanupTimer || startupTimer) return;
  if (!env.ACTIVITY_LOG_ORPHAN_CLEANUP_ENABLED) {
    log.info({ event: "disabled", reason: "ACTIVITY_LOG_ORPHAN_CLEANUP_ENABLED=false" });
    return;
  }
  if (!env.ACTIVITY_LOG_ORPHAN_CREATOR_ACCOUNT) {
    // 沒 creator filter 就讓條件變寬、可能誤殺他人手動建的 entry；
    // 所以強制要求設（跟 ENABLED 綁定）
    log.error({
      event: "refused-to-start",
      reason: "ACTIVITY_LOG_ORPHAN_CLEANUP_ENABLED=true 但 ACTIVITY_LOG_ORPHAN_CREATOR_ACCOUNT 沒設",
    });
    return;
  }
  if (env.RAGIC_WRITE_TARGET !== "prod") {
    ...
    log.error({
      event: "refused-to-start",
      reason: "ACTIVITY_LOG_ORPHAN_CLEANUP_ENABLED=true 但 RAGIC_WRITE_TARGET 不是 prod",
  ...
  startupTimer = setTimeout(() => {
    runBackgroundTask("activityLog-orphan-cleanup.startup", runOnce);
    startupTimer = null;
  }, STARTUP_DELAY_MS);
  startupTimer.unref();

  cleanupTimer = setInterval(() => {
    runBackgroundTask("activityLog-orphan-cleanup.interval", runOnce);
  }, CLEANUP_INTERVAL_MS);
  cleanupTimer.unref();
}

export function stopActivityLogOrphanCleanup(): void {
  if (startupTimer) {
