 ...
 *
 * waiter 可帶數字 priority（數字越小越優先）；同 priority 仍維持 FIFO。
 * 這只影響尚未取得 token 的 request，不能中斷已經送到 upstream 的 request。
 */
...
interface AcquireOptions {
  ...
  signal?: AbortSignal;
  /** 數字越小越優先；相同 priority 依進入順序 FIFO。 */
  priority?: number;
}
  ...
  acquire(options: AcquireOptions = {}): Promise<void> {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      const waiter: Waiter = {
        ...
        reject,
        priority,
        sequence: this.nextWaiterSequence,
      };
      this.nextWaiterSequence += 1;
      ...
      const insertionIndex = this.waiters.findIndex(
        (queued) =>
      ...
      if (insertionIndex < 0) {
        ...
        this.waiters.splice(insertionIndex, 0, waiter);
      }
      this.drainWaiters();
