...
// activityLog 的 downtime / planned_idle 兩張表用 CREATE TABLE IF NOT EXISTS 自建、不受此版號管控，
// 故新增 activityLog 表不需 bump（bump 反而會誤判 901/902 snapshot 過期、觸發整批 re-sync）。
export const READ_MODEL_SCHEMA_VERSION = 7;
...
CREATE TABLE IF NOT EXISTS work_report_entries (
  ...
  sort_order REAL,
  sort_order_99_auto_visible INTEGER NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS activityLog_downtime_records (
...
CREATE INDEX IF NOT EXISTS idx_activityLog_downtime_date
  ON activityLog_downtime_records (date_value);
CREATE INDEX IF NOT EXISTS idx_activityLog_downtime_machine
  ON activityLog_downtime_records (machine_id);
CREATE INDEX IF NOT EXISTS idx_activityLog_downtime_process
  ON activityLog_downtime_records (process_code);
CREATE INDEX IF NOT EXISTS idx_activityLog_downtime_operator
  ON activityLog_downtime_records (operator_id);
CREATE INDEX IF NOT EXISTS idx_activityLog_downtime_work_order
  ON activityLog_downtime_records (work_order_no);

CREATE TABLE IF NOT EXISTS activityLog_downtime_state (
...
CREATE TABLE IF NOT EXISTS activityLog_planned_idle_records (
  entry_id TEXT PRIMARY KEY,
  date_value TEXT,
  month_key TEXT,
  machine_id TEXT,
...
CREATE INDEX IF NOT EXISTS idx_activityLog_planned_idle_month
  ON activityLog_planned_idle_records (month_key);
CREATE INDEX IF NOT EXISTS idx_activityLog_planned_idle_machine
  ON activityLog_planned_idle_records (machine_id);

-- 計畫停機背景同步狀態：記錄上次同步時間與同步到的最舊月份。
-- 讓 summarize 能區分「這月真的 0 筆計畫停機」(oldest_month 內、不 fallback) vs「還沒同步」(fallback 即時撈)。
CREATE TABLE IF NOT EXISTS activityLog_planned_idle_state (
  id INTEGER PRIMARY KEY CHECK (id = 1),
  synced_at TEXT,
  oldest_month TEXT,
  total_records INTEGER NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS activityLog_planned_idle_month_state (
...
-- 稽核日誌：紀錄 901/902 工令報工 row 與 16 停機紀錄的 update / delete 操作
-- scope = 'work-report'：form 901/902 的子表 row（rowId 必填）
-- scope = 'downtime'：activity log entry 本身（rowId NULL）
...
-- activity log 寫入的 client-side idempotency key，擋 retry 風暴造成的 duplicate entry
-- 前端每次 create 產 UUID（901/902 直接 reuse clientMutationId；downtime 新產）
-- 後端寫 Ragic 前先 lookup：命中就回舊 entryId 不再打 Ragic；未命中才真的寫，寫完記錄映射
CREATE TABLE IF NOT EXISTS activityLog_client_row_keys (
  client_row_key TEXT PRIMARY KEY,
  entry_id TEXT NOT NULL,
  source TEXT NOT NULL,                    -- 'downtime' | 'work-report-901' | 'work-report-902' 等
...
CREATE INDEX IF NOT EXISTS idx_activityLog_client_row_keys_created_at
  ON activityLog_client_row_keys (created_at);
...
const WORK_REPORT_ENTRY_COLUMN_MIGRATIONS = [
  ...
  {
    name: "sort_order_99_auto_visible",
    sql: "ALTER TABLE work_report_entries ADD COLUMN sort_order_99_auto_visible INTEGER NOT NULL DEFAULT 0",
  },
  {
...
export async function initializeReadModelSchema(db: Database): Promise<void> {
  ...
  await ensureActivityLogClientRowKeyColumns(db);
  await ensureActivityLogProjectionRevisionColumns(db);
}
  ...
  try {
    ...
    await db.exec(`
      ...
      SELECT
        ...
        entry.sort_order,
        entry.sort_order_99_auto_visible,
...
async function ensureWorkReportGenerationIndexes(db: Database): Promise<void> {
  await db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_work_report_entries_form_sort_visibility
      ON work_report_entries (form_id, generation_id, sort_order, sort_order_99_auto_visible);
...
async function ensureBatchCreateRowKeyColumns(db: Database): Promise<void> {
  ...
  if (!rows.some(row => row.name === "reservation_token")) {
    await db.exec("ALTER TABLE batch_create_row_keys ADD COLUMN reservation_token TEXT");
  }
  const existingColumns = new Set(rows.map((row) => String(row.name)));
...
async function ensureActivityLogClientRowKeyColumns(db: Database): Promise<void> {
  const rows = await db.all<Array<{ name: string }>>("PRAGMA table_info(activityLog_client_row_keys)");
  const existingColumns = new Set(rows.map((row) => String(row.name)));
  if (!existingColumns.has("status")) {
    await db.exec("ALTER TABLE activityLog_client_row_keys ADD COLUMN status TEXT NOT NULL DEFAULT 'confirmed'");
  }
  if (!existingColumns.has("error_message")) {
    await db.exec("ALTER TABLE activityLog_client_row_keys ADD COLUMN error_message TEXT");
  }
  if (!existingColumns.has("operation_fingerprint")) {
    await db.exec("ALTER TABLE activityLog_client_row_keys ADD COLUMN operation_fingerprint TEXT");
  }
  if (!existingColumns.has("reservation_token")) {
    await db.exec("ALTER TABLE activityLog_client_row_keys ADD COLUMN reservation_token TEXT");
  }
  if (!existingColumns.has("updated_at")) {
    await db.exec("ALTER TABLE activityLog_client_row_keys ADD COLUMN updated_at TEXT");
    await db.exec(`
      UPDATE activityLog_client_row_keys
      SET updated_at = created_at
      WHERE updated_at IS NULL
    `);
  }
  await db.exec(`
    CREATE INDEX IF NOT EXISTS idx_activityLog_client_row_keys_status
      ON activityLog_client_row_keys (status, updated_at)
  `);
}

async function ensureActivityLogProjectionRevisionColumns(db: Database): Promise<void> {
  await ensureTableColumns(db, "activityLog_downtime_state", [
    {
      name: "revision",
      sql: "ALTER TABLE activityLog_downtime_state ADD COLUMN revision INTEGER NOT NULL DEFAULT 0",
    },
  ]);
  await ensureTableColumns(db, "activityLog_planned_idle_state", [
    {
      name: "full_revision",
      sql: "ALTER TABLE activityLog_planned_idle_state ADD COLUMN full_revision INTEGER NOT NULL DEFAULT 0",
    },
    {
      name: "projection_revision",
      sql: "ALTER TABLE activityLog_planned_idle_state ADD COLUMN projection_revision INTEGER NOT NULL DEFAULT 0",
    },
  ]);
  await ensureTableColumns(db, "activityLog_planned_idle_month_state", [
    {
      name: "revision",
      sql: "ALTER TABLE activityLog_planned_idle_month_state ADD COLUMN revision INTEGER NOT NULL DEFAULT 0",
    },
  ]);
}
