...
import { isReportColumnKey } from "../../types/workReport";
import { isReadyConsecutiveLineBRecord } from "../../services/work-report/shared/workReportSortOrderVisibility";
...
import {
  ...
  type EntryRowRecord,
  getReportColumnTokenSqlExpression,
  getReportColumnSqlExpression,
  parseRecordPayload,
...
const ENTRY_SNAPSHOT_COLUMN_COUNT = 19;
...
class WorkReportSqliteRepository {
  ...
  private async replaceFormSnapshotWithDb(
    ...
    for (const record of dedupedRecords) {
      ...
      entryPayloads.push({
        ...
        sortOrder99AutoVisible: isReadyConsecutiveLineBRecord(record) ? 1 : 0,
    ...
    await withWriteTransaction(async (db) => {
      if ((await this.getActiveGenerationIdWithDb(db, formId)) === generationId) {
        throw new Error("Cannot replace the active work-report generation");
      }
      await db.run(
    ...
    // Only the inactive generation is built here. Release the writer between
    // batches so durable mutation events do not wait for the whole snapshot.
    const entryInsertChunkSize = resolveSqliteInsertChunkSize(
      batchSize,
      ENTRY_SNAPSHOT_COLUMN_COUNT
    );
    for (const entryBatch of chunkArray(entryPayloads, batchSize)) {
      await withWriteTransaction(async (db) => {
        for (const entryChunk of chunkArray(entryBatch, entryInsertChunkSize)) {
    ...
    for (const rowBatch of chunkArray(rowPayloads, batchSize * 2)) {
      await withWriteTransaction(async (db) => {
        for (const rowChunk of chunkArray(rowBatch, rowInsertChunkSize)) {
  ...
  async getReports(
    ...
    const orderByClauses =
      options.sortRules && options.sortRules.length > 0
        ? [...options.sortRules.flatMap(buildReportColumnSortClauses), "rowid ASC"]
        : ["rowid ASC"];
  ...
  async getFacetCounts(
    ...
    for (const field of fields) {
      ...
      const fieldName: ReportColumnKey = field;
      const columnName = getReportColumnTokenSqlExpression(fieldName);
  ...
  async getColumnValues(
    ...
    const valueExpression = getReportColumnSqlExpression(field);
    const rows = await db.all<Array<{ raw_value: unknown }>>(
      `
      SELECT ${valueExpression} AS raw_value
      FROM work_report_entries
      WHERE ${whereClauses.join(" AND ")}
      `,
      ...whereParams
  ...
  async upsertEntrySnapshot(
    ...
    return withWriteTransaction(async (db) => {
      ...
      await db.run(
        ...
        ON CONFLICT (form_id, generation_id, entry_id) DO UPDATE SET
          work_order_no = excluded.work_order_no,
          customer_part_no = excluded.customer_part_no,
          machine_code = excluded.machine_code,
          filter_machine_code = excluded.filter_machine_code,
          status = excluded.status,
          ragic_unfinished_status = excluded.ragic_unfinished_status,
          site_running = excluded.site_running,
          start_schedule = excluded.start_schedule,
          sort_order = excluded.sort_order,
          sort_order_99_auto_visible = excluded.sort_order_99_auto_visible,
        ...
        isReadyConsecutiveLineBRecord(record) ? 1 : 0,
