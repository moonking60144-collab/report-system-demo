...
export interface BatchCreateRowKeyRepository {
  ...
  deleteByReservationIdentity(input: {
    clientRowKey: string; formId: string; entryId: string; reservationToken: string; ragicRowId: string;
  }): Promise<number>;
  ...
  return {
    async lookup(clientRowKey: string) {
      ...
      const row = await db.get<RowShape>(
        `SELECT client_row_key, form_id, entry_id, ragic_row_id, status, error_message, created_at, updated_at, reservation_token
      ...
      await runSerializedWrite(async (db) => {
        const result = await db.run(
          `INSERT INTO batch_create_row_keys
             (client_row_key, form_id, entry_id, ragic_row_id, status, error_message, created_at, updated_at, reservation_token)
           VALUES (?, ?, ?, '', 'pending', NULL, ?, ?, ?)
        ...
        const result = await db.run(
          `UPDATE batch_create_row_keys
           ...
           WHERE client_row_key = ?
             ...
             AND entry_id = ?
             AND status = 'pending'
             AND (? IS NULL OR reservation_token = ?)`,
          input.ragicRowId,
          updatedAt,
          trimmed,
          input.formId,
    ...
    async deleteByReservationIdentity(input) {
      if (!input.clientRowKey || !input.reservationToken || !input.ragicRowId) return 0;
      return runSerializedWrite(async db => {
        const result = await db.run(
          `DELETE FROM batch_create_row_keys WHERE client_row_key = ? AND form_id = ? AND entry_id = ?
           AND reservation_token = ? AND (ragic_row_id = '' OR ragic_row_id = ?)`,
          input.clientRowKey, input.formId, input.entryId, input.reservationToken, input.ragicRowId
        );
