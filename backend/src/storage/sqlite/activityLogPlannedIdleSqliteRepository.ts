...
class ActivityLogPlannedIdleSqliteRepository {
  ...
  async replaceAll(
    ...
    return withWriteTransaction(async (db) => {
      const globalState = await db.get<{ projection_revision: number }>(
        "SELECT projection_revision FROM activityLog_planned_idle_state WHERE id = 1"
      );
      if ((globalState?.projection_revision ?? 0) !== expectedProjectionRevision) {
        return "stale";
      }

      await db.exec("DELETE FROM activityLog_planned_idle_records");
      await db.exec("DELETE FROM activityLog_planned_idle_month_state");
    ...
    return withWriteTransaction(async (db) => {
      const globalState = await db.get<{
        ...
        `SELECT full_revision, projection_revision
         FROM activityLog_planned_idle_state
         WHERE id = 1`
      );
      const monthState = await db.get<{ revision: number }>(
        "SELECT revision FROM activityLog_planned_idle_month_state WHERE month_key = ?",
        monthKey
      );
      if (
      ...
      await db.run("DELETE FROM activityLog_planned_idle_records WHERE month_key = ?", monthKey);
      await this.insertManyWithDb(db, records, syncedAt);
      await this.upsertMonthStateWithDb(db, monthKey, syncedAt);
  ...
  async getRefreshBarrier(monthKey: string): Promise<PlannedIdleRefreshBarrier> {
    ...
    const globalState = await db.get<{
      ...
      "SELECT full_revision, projection_revision FROM activityLog_planned_idle_state WHERE id = 1"
    );
    const monthState = await db.get<{ revision: number }>(
      "SELECT revision FROM activityLog_planned_idle_month_state WHERE month_key = ?",
      monthKey
  ...
  async getMonthSyncedAt(monthKey: string): Promise<string | null> {
    ...
    const row = await db.get<{ synced_at: string | null }>(
      "SELECT synced_at FROM activityLog_planned_idle_month_state WHERE month_key = ?",
      monthKey
    );
  ...
  async aggregateByMonth(monthKey: string): Promise<PlannedIdleMachineAggregate[]> {
    ...
    >(
      ...
      SELECT
        ...
        (
          SELECT r2.prod_type FROM activityLog_planned_idle_records r2
        ...
        COUNT(*) AS count
      FROM activityLog_planned_idle_records r1
  ...
  private async commitFullStateWithCurrentCount(
    ...
    const countRow = await db.get<{ count: number }>(
      "SELECT COUNT(*) AS count FROM activityLog_planned_idle_records"
    );
    const totalRecords =
    ...
    await db.run(
      `
      INSERT INTO activityLog_planned_idle_state (
      ...
      ON CONFLICT(id) DO UPDATE SET
        ...
        full_revision = activityLog_planned_idle_state.full_revision + 1,
        projection_revision = activityLog_planned_idle_state.projection_revision + 1,
  ...
  private async upsertMonthStateWithDb(
    ...
    await db.run(
      `INSERT INTO activityLog_planned_idle_month_state (month_key, synced_at, revision)
       VALUES (?, ?, 1)
       ON CONFLICT(month_key) DO UPDATE SET
         synced_at = excluded.synced_at,
         revision = activityLog_planned_idle_month_state.revision + 1`,
      monthKey,
      syncedAt
    );
  }
  ...
  private async insertManyWithDb(
    ...
    for (let index = 0; index < records.length; index += chunkSize) {
      ...
      await db.run(
        `
        INSERT INTO activityLog_planned_idle_records (
...
export const activityLogPlannedIdleSqliteRepository = new ActivityLogPlannedIdleSqliteRepository();
