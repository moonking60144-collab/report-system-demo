...
interface ActivityLogDowntimeStateRow {
...
export interface StoredActivityLogDowntimeState {
...
export function buildActivityLogDowntimeSnapshotHash(rawJson: string | null | undefined): string | null {
  if (!rawJson) return null;
  return createHash("sha256").update(rawJson).digest("hex");
}

function serializeActivityLogDowntimeRecord(record: ActivityLogDowntimeRecord): string {
  const stored = {
    id: record.id,
    date: record.date,
...
export function buildActivityLogDowntimeRecordSnapshotHash(
  record: ActivityLogDowntimeRecord
): string {
  return createHash("sha256").update(serializeActivityLogDowntimeRecord(record)).digest("hex");
}

function toDowntimeInsertParams(record: ActivityLogDowntimeRecord, syncedAt: string): unknown[] {
  return [
    record.id,
    record.date,
    record.machineId,
    ...
    record.remark,
    record.workOrderNo,
    serializeActivityLogDowntimeRecord(record),
    syncedAt,
  ];
}

class ActivityLogDowntimeSqliteRepository {
  async listRecords(options: { limit?: number; offset?: number } = {}): Promise<ActivityLogDowntimeRecord[]> {
    ...
    const baseSql = `
      SELECT
        ...
        work_order_no,
        raw_json
      FROM activityLog_downtime_records
    ...
    return rows.map((row) => ({
      id: row.entry_id,
      snapshotHash: buildActivityLogDowntimeSnapshotHash(row.raw_json),
      date: row.date_value,
  ...
  async replaceSnapshot(records: ActivityLogDowntimeRecord[], syncedAt: string): Promise<void> {
    await withWriteTransaction(async (db) => {
      await db.exec("DELETE FROM activityLog_downtime_records");
   ...
   *
   * Caller（activityLogDowntimeService.fetchRecordsFromRagic）目前只抓**近 30 天**內的
  ...
  async syncSnapshot(
    ...
    return withWriteTransaction(async (db) => {
      const currentState = await db.get<{ revision: number }>(
        "SELECT revision FROM activityLog_downtime_state WHERE id = 1"
      );
      ...
      const existingRows = await db.all<Array<{ entry_id: string }>>(
        "SELECT entry_id FROM activityLog_downtime_records"
      );
      ...
      for (const entryId of toDelete) {
        await db.run("DELETE FROM activityLog_downtime_records WHERE entry_id = ?", entryId);
      }
      await this.upsertRecordsWithDb(db, records, syncedAt);
      await this.updateSnapshotStateWithDb(db, syncedAt);
      return "applied";
  ...
  async upsertRecord(record: ActivityLogDowntimeRecord, syncedAt: string): Promise<void> {
    await withWriteTransaction(async (db) => {
  ...
  async getSnapshotState(): Promise<StoredActivityLogDowntimeState | null> {
    const db = await sqliteClient.getReadDb();
    const row = await db.get<ActivityLogDowntimeStateRow>(
      `
      SELECT snapshot_at, total_records, revision, updated_at
      FROM activityLog_downtime_state
      WHERE id = 1
      `
    );
    if (!row) {
  ...
  async getRecordSnapshotHash(entryId: string): Promise<string | null> {
    ...
    const row = await db.get<{ raw_json: string | null }>(
      "SELECT raw_json FROM activityLog_downtime_records WHERE entry_id = ?",
      entryId
    );
    return buildActivityLogDowntimeSnapshotHash(row?.raw_json);
  }
  ...
  private async upsertRecordsWithDb(
    ...
    for (let index = 0; index < records.length; index += chunkSize) {
      ...
      await db.run(
        `
        INSERT INTO activityLog_downtime_records (
          entry_id,
          date_value,
          machine_id,
          process_code,
  ...
  private async updateSnapshotStateWithDb(db: Database, snapshotAt: string): Promise<void> {
    const countRow = await db.get<{ count: number }>(
      "SELECT COUNT(*) AS count FROM activityLog_downtime_records"
    );
    const totalRecords =
    ...
    await db.run(
      `
      INSERT INTO activityLog_downtime_state (id, snapshot_at, total_records, revision, updated_at)
      VALUES (1, ?, ?, 1, ?)
      ON CONFLICT(id)
      DO UPDATE SET
        snapshot_at = excluded.snapshot_at,
        total_records = excluded.total_records,
        revision = activityLog_downtime_state.revision + 1,
...
export const activityLogDowntimeSqliteRepository = new ActivityLogDowntimeSqliteRepository();
