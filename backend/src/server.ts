...
import {
  startActivityLogOrphanCleanup,
  stopActivityLogOrphanCleanup,
} from "./bootstrap/activityLogOrphanCleanup";
import {
  startActivityLogClientRowKeyCleanup,
  stopActivityLogClientRowKeyCleanup,
} from "./bootstrap/activityLogClientRowKeyCleanup";
import {
...
import {
  startActivityLogPlannedIdleSync,
  stopActivityLogPlannedIdleSync,
} from "./bootstrap/activityLogPlannedIdleSync";
import {
  flushActivityLogWriteReverify,
  startActivityLogWriteReverify,
  stopActivityLogWriteReverify,
} from "./bootstrap/activityLogWriteReverify";
import {
...
import { drainWorkReportEntryProjections } from "./services/work-report-sync/workReportEntryProjectionQueue";
...
import activityLogDowntimeRouter from "./routes/activityLogDowntime";
import activityLogEfficiencyReportsRouter from "./routes/activityLogEfficiencyReports";
...
import { devAiKnowledgeCandidateRepository } from "./services/dev/ai/devAiKnowledgeCandidateRepository";
...
app.use("/api", activityLogEfficiencyReportsRouter);
...
async function bootstrapServer(): Promise<void> {
  ...
  try {
    ...
    log.warn({
      event: "activityLog-downtime-create-task.initialize-failed",
      error: error instanceof Error ? error.message : String(error),
    });
  }

  try {
    await activityLogDowntimeCallbackRefreshService.initialize();
  } catch (error) {
    log.warn({
      event: "activityLog-downtime-callback.initialize-failed",
      error: error instanceof Error ? error.message : String(error),
    });
  }
  ...
  try {
    await activityLogEfficiencyReportArchiveService.initialize();
  } catch (error) {
    log.warn({
...
function registerGracefulShutdown(server: Server): void {
  const shutdown = createGracefulShutdownHandler({
    ...
    drainMutationQueue: async () => {
      await drainWorkReportEntryMutationQueue();
      await drainWorkReportEntryProjections();
    },
    drainCallbackQueue: async () => {
      await Promise.all([
        ragicCallbackRefreshService.drain(),
        activityLogDowntimeCallbackRefreshService.drain(),
      ]);
    },
    flushRegistries: async () => {
      ...
      await flushActivityLogWriteReverify();
    },
    closeServiceStores: async () => {
      await devAiKnowledgeBaseService.dispose?.();
      await devAiKnowledgeCandidateRepository.close();
    ...
    getCallbackQueueStats: () => {
      ...
      const activityLog = activityLogDowntimeCallbackRefreshService.getQueueStats();
      return {
        accepting: workReport.accepting && activityLog.accepting,
        activeKeyCount: workReport.activeKeyCount + activityLog.activeKeyCount,
        pendingTaskCount: workReport.pendingTaskCount + activityLog.pendingTaskCount,
      };
    },
