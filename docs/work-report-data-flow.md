...
這份文件固定 `work-report`、901/902 報工、activity log 停機紀錄與 Ragic/SQLite/task registry 的資料流邊界。後續修改時先對照這裡，避免把不同 task shape、retry payload、read model 與 Ragic 寫入流程混在一起。
...
- 後端 create 流程仍由 `activityLogDowntimeService.createRecord()` 負責：
  - Type / DEP / ProdType 等 create-time required fields。
  - duplicate check。
  - post-write verify。
  - action button。
  - SQLite refresh。
  - `checkOrCreateActivityLogEntry` idempotency。
...
- 目前是同步 mutation record，不是 queued retryable create task；但後端仍經過 `903:downtime:mutation` barrier。
...
- 任何 startup Ragic read job 都應具備：
  ...
  - 明確 priority lane。
- 目前啟動錯峰原則：
  - options prewarm：延遲啟動，901/902 串行。
...
- 不把 16 downtime payload 塞進 901/902 `taskRetryStore`。
- 不把 901/902 與 activity log 的 retry store / task shape 混用。兩者都需要穩定 durable create key，但各自維持 domain API 與 payload 契約。
